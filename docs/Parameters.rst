..  List of parameters is auto generated by DeePray\helpers\parameter_generator.py from DeePray\include\DeePray\config.h file.
...
This page contains descriptions of all parameters in DeePray.
...
-  ``boosting`` :raw-html:`<a id="boosting" title="Permalink to this parameter" href="#boosting">&#x1F517;&#xFE0E;</a>`, default = ``gbdt``, type = enum, options: ``gbdt``, ``rf``, ``dart``, aliases: ``boosting_type``, ``boost``
   ...
   -  ``dart``, `Dropouts meet Multiple Additive Regression Trees <https://arxiv.org/abs/1505.01866>`__
      ...
      -  **Note**: internally, DeePray uses ``gbdt`` mode for the first ``1 / learning_rate`` iterations
...
-  ``data`` :raw-html:`<a id="data" title="Permalink to this parameter" href="#data">&#x1F517;&#xFE0E;</a>`, default = ``""``, type = string, aliases: ``train``, ``train_data``, ``train_data_file``, ``data_filename``
   ...
   -  path of training data, DeePray will train from this data
...
-  ``valid`` :raw-html:`<a id="valid" title="Permalink to this parameter" href="#valid">&#x1F517;&#xFE0E;</a>`, default = ``""``, type = string, aliases: ``test``, ``valid_data``, ``valid_data_file``, ``test_data``, ``test_data_file``, ``valid_filenames``
   ...
   -  path(s) of validation/test data, DeePray will output metrics for these data
...
-  ``num_iterations`` :raw-html:`<a id="num_iterations" title="Permalink to this parameter" href="#num_iterations">&#x1F517;&#xFE0E;</a>`, default = ``100``, type = int, aliases: ``num_iteration``, ``n_iter``, ``num_tree``, ``num_trees``, ``num_round``, ``num_rounds``, ``nrounds``, ``num_boost_round``, ``n_estimators``, ``max_iter``, constraints: ``num_iterations >= 0``
   ...
   -  **Note**: internally, DeePray constructs ``num_class * num_iterations`` trees for multi-class classification problems
...
-  ``device_type`` :raw-html:`<a id="device_type" title="Permalink to this parameter" href="#device_type">&#x1F517;&#xFE0E;</a>`, default = ``cpu``, type = enum, options: ``cpu``, ``gpu``, ``cuda``, aliases: ``device``
   ...
   -  ``cpu`` supports all DeePray functionality and is portable across the widest range of operating systems and hardware
   ...
   -  **Note**: refer to `Installation Guide <./Installation-Guide.rst#build-gpu-version>`__ to build DeePray with GPU support
...
-  ``deterministic`` :raw-html:`<a id="deterministic" title="Permalink to this parameter" href="#deterministic">&#x1F517;&#xFE0E;</a>`, default = ``false``, type = bool
   ...
   -  when you use the different seeds, different DeePray versions, the binaries compiled by different compilers, or in different systems, the results are expected to be different

   -  you can `raise issues <https://github.com/microsoft/DeePray/issues>`__ in DeePray GitHub repo when you meet the unstable results
...
-  ``force_col_wise`` :raw-html:`<a id="force_col_wise" title="Permalink to this parameter" href="#force_col_wise">&#x1F517;&#xFE0E;</a>`, default = ``false``, type = bool
   ...
   -  **Note**: when both ``force_col_wise`` and ``force_row_wise`` are ``false``, DeePray will firstly try them both, and then use the faster one. To remove the overhead of testing set the faster one to ``true`` manually
...
-  ``bagging_freq`` :raw-html:`<a id="bagging_freq" title="Permalink to this parameter" href="#bagging_freq">&#x1F517;&#xFE0E;</a>`, default = ``0``, type = int, aliases: ``subsample_freq``
   ...
   -  ``0`` means disable bagging; ``k`` means perform bagging at every ``k`` iteration. Every ``k``-th iteration, DeePray will randomly select ``bagging_fraction * 100 %`` of the data to use for the next ``k`` iterations
...
-  ``feature_fraction`` :raw-html:`<a id="feature_fraction" title="Permalink to this parameter" href="#feature_fraction">&#x1F517;&#xFE0E;</a>`, default = ``1.0``, type = double, aliases: ``sub_feature``, ``colsample_bytree``, constraints: ``0.0 < feature_fraction <= 1.0``
   ...
   -  DeePray will randomly select a subset of features on each iteration (tree) if ``feature_fraction`` is smaller than ``1.0``. For example, if you set it to ``0.8``, DeePray will select 80% of features before training each tree
...
-  ``feature_fraction_bynode`` :raw-html:`<a id="feature_fraction_bynode" title="Permalink to this parameter" href="#feature_fraction_bynode">&#x1F517;&#xFE0E;</a>`, default = ``1.0``, type = double, aliases: ``sub_feature_bynode``, ``colsample_bynode``, constraints: ``0.0 < feature_fraction_bynode <= 1.0``
   ...
   -  DeePray will randomly select a subset of features on each tree node if ``feature_fraction_bynode`` is smaller than ``1.0``. For example, if you set it to ``0.8``, DeePray will select 80% of features at each tree node
...
-  ``extra_trees`` :raw-html:`<a id="extra_trees" title="Permalink to this parameter" href="#extra_trees">&#x1F517;&#xFE0E;</a>`, default = ``false``, type = bool, aliases: ``extra_tree``
   ...
   -  if set to ``true``, when evaluating node splits DeePray will check only one randomly-chosen threshold for each feature
...
-  ``first_metric_only`` :raw-html:`<a id="first_metric_only" title="Permalink to this parameter" href="#first_metric_only">&#x1F517;&#xFE0E;</a>`, default = ``false``, type = bool
   ...
   -  DeePray allows you to provide multiple evaluation metrics. Set this to ``true``, if you want to use only the first metric for early stopping
...
-  ``max_cat_threshold`` :raw-html:`<a id="max_cat_threshold" title="Permalink to this parameter" href="#max_cat_threshold">&#x1F517;&#xFE0E;</a>`, default = ``32``, type = int, constraints: ``max_cat_threshold > 0``
   ...
   -  limit number of split points considered for categorical features. See `the documentation on how DeePray finds optimal splits for categorical features <./Features.rst#optimal-split-for-categorical-features>`_ for more details
...
-  ``forcedsplits_filename`` :raw-html:`<a id="forcedsplits_filename" title="Permalink to this parameter" href="#forcedsplits_filename">&#x1F517;&#xFE0E;</a>`, default = ``""``, type = string, aliases: ``fs``, ``forced_splits_filename``, ``forced_splits_file``, ``forced_splits``
   ...
   -  see `this file <https://github.com/microsoft/DeePray/tree/master/examples/binary_classification/forced_splits.json>`__ as an example
...
-  ``verbosity`` :raw-html:`<a id="verbosity" title="Permalink to this parameter" href="#verbosity">&#x1F517;&#xFE0E;</a>`, default = ``1``, type = int, aliases: ``verbose``
   ...
   -  controls the level of DeePray's verbosity
...
-  ``linear_tree`` :raw-html:`<a id="linear_tree" title="Permalink to this parameter" href="#linear_tree">&#x1F517;&#xFE0E;</a>`, default = ``false``, type = bool, aliases: ``linear_trees``
   ...
   -  fit piecewise linear gradient boosting tree
      ...
      -  **Note**: setting ``linear_tree=true`` significantly increases the memory use of DeePray
...
-  ``max_bin`` :raw-html:`<a id="max_bin" title="Permalink to this parameter" href="#max_bin">&#x1F517;&#xFE0E;</a>`, default = ``255``, type = int, aliases: ``max_bins``, constraints: ``max_bin > 1``
   ...
   -  DeePray will auto compress memory according to ``max_bin``. For example, DeePray will use ``uint8_t`` for feature value if ``max_bin=255``
...
-  ``enable_bundle`` :raw-html:`<a id="enable_bundle" title="Permalink to this parameter" href="#enable_bundle">&#x1F517;&#xFE0E;</a>`, default = ``true``, type = bool, aliases: ``is_enable_bundle``, ``bundle``
   ...
   -  set this to ``false`` to disable Exclusive Feature Bundling (EFB), which is described in `DeePray: A Highly Efficient Gradient Boosting Decision Tree <https://papers.nips.cc/paper/6907-deepray-a-highly-efficient-gradient-boosting-decision-tree>`__
...
-  ``feature_pre_filter`` :raw-html:`<a id="feature_pre_filter" title="Permalink to this parameter" href="#feature_pre_filter">&#x1F517;&#xFE0E;</a>`, default = ``true``, type = bool
   ...
   -  set this to ``true`` (the default) to tell DeePray to ignore the features that are unsplittable based on ``min_data_in_leaf``
...
-  ``two_round`` :raw-html:`<a id="two_round" title="Permalink to this parameter" href="#two_round">&#x1F517;&#xFE0E;</a>`, default = ``false``, type = bool, aliases: ``two_round_loading``, ``use_two_round_loading``
   ...
   -  by default, DeePray will map data file to memory and load features from memory. This will provide faster data loading speed, but may cause run out of memory error when the data file is very big
...
-  ``ignore_column`` :raw-html:`<a id="ignore_column" title="Permalink to this parameter" href="#ignore_column">&#x1F517;&#xFE0E;</a>`, default = ``""``, type = multi-int or string, aliases: ``ignore_feature``, ``blacklist``
   ...
   -  **Note**: despite the fact that specified columns will be completely ignored during the training, they still should have a valid format allowing DeePray to load file successfully
...
-  ``forcedbins_filename`` :raw-html:`<a id="forcedbins_filename" title="Permalink to this parameter" href="#forcedbins_filename">&#x1F517;&#xFE0E;</a>`, default = ``""``, type = string
   ...
   -  see `this file <https://github.com/microsoft/DeePray/tree/master/examples/regression/forced_bins.json>`__ as an example
...
-  ``save_binary`` :raw-html:`<a id="save_binary" title="Permalink to this parameter" href="#save_binary">&#x1F517;&#xFE0E;</a>`, default = ``false``, type = bool, aliases: ``is_save_binary``, ``is_save_binary_file``
   ...
   -  if ``true``, DeePray will save the dataset (including validation data) to a binary file. This speed ups the data loading for the next time
...
-  ``parser_config_file`` :raw-html:`<a id="parser_config_file" title="Permalink to this parameter" href="#parser_config_file">&#x1F517;&#xFE0E;</a>`, default = ``""``, type = string
   ...
   -  see `deepray-transform <https://github.com/microsoft/deepray-transform>`__ for usage examples

   -  **Note**: ``deepray-transform`` is not maintained by DeePray's maintainers. Bug reports or feature requests should go to `issues page <https://github.com/microsoft/deepray-transform/issues>`__
...
-  ``predict_disable_shape_check`` :raw-html:`<a id="predict_disable_shape_check" title="Permalink to this parameter" href="#predict_disable_shape_check">&#x1F517;&#xFE0E;</a>`, default = ``false``, type = bool
   ...
   -  control whether or not DeePray raises an error when you try to predict on data with a different number of features than the training data
...
DeePray supports continued training with initial scores. It uses an additional file to store these initial scores, like the following:
...
In this case, DeePray will auto load initial score file if it exists.
...
DeePray supports weighted training. It uses an additional file to store weight data, like the following:
...
In this case, DeePray will load the weight file automatically if it exists.
...
DeePray uses an additional file to store query data, like the following:
...
In this case, DeePray will load the query file automatically if it exists.
