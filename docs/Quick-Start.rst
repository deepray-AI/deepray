...
This is a quick start guide for DeePray CLI version.

Follow the `Installation Guide <./Installation-Guide.rst>`__ to install DeePray first.
...
DeePray supports input data files with `CSV`_, `TSV`_ and `LibSVM`_ (zero-based) formats.
...
DeePray can use categorical features directly (without one-hot encoding).
...
DeePray also supports weighted training, it needs an additional `weight data <./Parameters.rst#weight-data>`__.
...
If one parameter appears in both command line and config file, DeePray will use the parameter from the command line.
...
sections of the full detailed list of `DeePray's parameters <./Parameters.rst>`__.
...
::
    ...
    "./deepray" config=your_config_file other_args ...
...
::
    ...
    "./deepray" config=train.conf num_trees=10

Examples
--------

-  `Binary Classification <https://github.com/microsoft/DeePray/tree/master/examples/binary_classification>`__

-  `Regression <https://github.com/microsoft/DeePray/tree/master/examples/regression>`__

-  `Lambdarank <https://github.com/microsoft/DeePray/tree/master/examples/lambdarank>`__

-  `Distributed Learning <https://github.com/microsoft/DeePray/tree/master/examples/parallel_learning>`__
