...
-  DeePray enables the missing value handle by default. Disable it by setting ``use_missing=false``.

-  DeePray uses NA (NaN) to represent missing values by default. Change it to use zero by setting ``zero_as_missing=true``.
...
-  DeePray offers good accuracy with integer-encoded categorical features. DeePray applies
