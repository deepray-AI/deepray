...
This guide describes distributed learning in DeePray. Distributed learning allows the use of multiple machines to produce a single model.

Follow the `Quick Start <./Quick-Start.rst>`__ to know how to use DeePray first.

How Distributed DeePray Works
------------------------------

This section describes how distributed learning in DeePray works. To learn how to do this in various programming languages and frameworks, please see `Integrations <#integrations>`__.

Choose Appropriate Parallel Algorithm
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

DeePray provides 3 distributed learning algorithms now.
...
This section describes how to run distributed DeePray training in various programming languages and frameworks. To learn how distributed learning in DeePray works generally, please see `How Distributed DeePray Works <#how-distributed-deepray-works>`__.

Apache Spark
^^^^^^^^^^^^

Apache Spark users can use `SynapseML`_ for machine learning workflows with DeePray. This project is not maintained by DeePray's maintainers.

See `this SynapseML example`_ for additional information on using DeePray on Spark.

.. note::

  ``SynapseML`` is not maintained by DeePray's maintainers. Bug reports or feature requests should be directed to https://github.com/microsoft/SynapseML/issues.
...
DeePray's Python package supports distributed learning via `Dask`_. This integration is maintained by DeePray's maintainers.
...
For sample code using ``deepray.dask``, see `these Dask examples`_.

Training with Dask
''''''''''''''''''

This section contains detailed information on performing DeePray distributed training using Dask.
...
The estimators in ``deepray.dask`` expect that matrix-like or array-like data are provided in Dask DataFrame, Dask Array, or (in some cases) Dask Series format. See `the Dask DataFrame documentation`_ and `the Dask Array documentation`_ for more information on how to create such data structures.
...
While setting up for training, ``deepray`` will concatenate all of the partitions on a worker into a single dataset. Distributed training then proceeds with one DeePray worker process per Dask worker.
...
When setting up data partitioning for DeePray training with Dask, try to follow these suggestions:
...
In most situations, you should not need to tell ``deepray.dask`` to use a specific Dask client. By default, the client returned by ``distributed.default_client()`` will be used.

However, you might want to explicitly control the Dask client used by DeePray if you have multiple active clients in the same session. This is useful in more complex workflows like running multiple training jobs on different Dask clusters.

DeePray's Dask estimators support setting an attribute ``client`` to control the client that is used.
...
.. code:: python
  ...
  dask_model = dp.DaskLGBMClassifier(client=client)

  # option 2: set_params() after construction
  dask_model = dp.DaskLGBMClassifier()
  dask_model.set_params(client=client)

Using Specific Ports
********************

At the beginning of training, ``deepray.dask`` sets up a DeePray network where each Dask worker runs one long-running task that acts as a DeePray worker. During training, DeePray workers communicate with each other over TCP sockets. By default, random open ports are used when creating these sockets.

If the communication between Dask workers in the cluster used for training is restricted by firewall rules, you must tell DeePray exactly what ports to use.

**Option 1: provide a specific list of addresses and ports**

DeePray supports a parameter ``machines``, a comma-delimited string where each entry refers to one worker (host name or IP) and a port that that worker will accept connections on. If you provide this parameter to the estimators in ``deepray.dask``, DeePray will not search randomly for ports.
...
.. code:: python
  ...
  dask_model = dp.DaskLGBMRegressor(machines=machines)
...
If you are only running one Dask worker process on each host, and if you can reliably identify a port that is open on every host, using ``machines`` is unnecessarily complicated. If ``local_listen_port`` is given and ``machines`` is not, DeePray will not search for ports randomly, but it will limit the list of addresses in the DeePray network to those Dask workers that have a piece of the training data.
...
.. code:: python
  ...
  dask_model = dp.DaskLGBMRegressor(local_listen_port=12400)

.. warning::

  Providing ``local_listen_port`` is slightly less fragile than ``machines`` because DeePray will automatically figure out which workers have pieces of the training data. However, using this method, training can fail if any of the following are true:
...
.. warning::
  ...
  Custom objective functions used with ``deepray.dask`` will be called by each worker process on only that worker's local data.
...
.. code:: python
  ...
  dask_model = dp.DaskLGBMRegressor(
...
The estimators from ``deepray.dask`` can be used to create predictions based on data stored in Dask collections. In that interface, ``.predict()`` expects a Dask Array or Dask DataFrame, and returns a Dask Array of predictions.
...
DeePray's Dask estimators can be pickled directly with ``cloudpickle``, ``joblib``, or ``pickle``.
...
The estimators available from ``deepray.dask`` can be converted to an instance of the equivalent class from ``deepray.sklearn``. Choosing this option allows you to use Dask for training but avoid depending on any Dask libraries at scoring time.
...
**Option 3: save the DeePray Booster**

The lowest-level model object in DeePray is the ``deepray.Booster``. After training, you can extract a Booster from the Dask estimator.
...
`Kubeflow Fairing`_ supports DeePray distributed training. `These examples`_ show how to get started with DeePray and Kubeflow Fairing in a hybrid cloud environment.

Kubeflow users can also use the `Kubeflow XGBoost Operator`_ for machine learning workflows with DeePray. You can see `this example`_ for more details.

Kubeflow integrations for DeePray are not maintained by DeePray's maintainers.

.. note::

  The Kubeflow integrations for DeePray are not maintained by DeePray's maintainers. Bug reports or feature requests should be directed to https://github.com/kubeflow/fairing/issues or https://github.com/kubeflow/xgboost-operator/issues.
...
By default, distributed learning with DeePray uses socket-based communication.
...
3. Run following command on all machines, you need to change ``your_config_file`` to real config file.
   ...
   For Windows: ``deepray.exe config=your_config_file``

   For Linux: ``./deepray config=your_config_file``
...
3. Run following command on one machine (not need to run on all machines), need to change ``your_config_file`` to real config file.
   ...
   .. code:: console
       ...
       mpiexec.exe /machinefile mlist.txt deepray.exe config=your_config_file

   For Linux:

   .. code:: console

       mpiexec --machinefile mlist.txt ./deepray config=your_config_file
...
`Ray`_ is a Python-based framework for distributed computing. The `lightgbm_ray`_ project, maintained within the official Ray GitHub organization, can be used to perform distributed DeePray training using ``ray``.

See `the lightgbm_ray documentation`_ for usage examples.

.. note::

  ``lightgbm_ray`` is not maintained by DeePray's maintainers. Bug reports or feature requests should be directed to https://github.com/ray-project/lightgbm_ray/issues.

Mars
^^^^

`Mars`_ is a tensor-based framework for large-scale data computation. DeePray integration, maintained within the Mars GitHub repository, can be used to perform distributed DeePray training using ``pymars``.

See `the mars documentation`_ for usage examples.

.. note::

  ``Mars`` is not maintained by DeePray's maintainers. Bug reports or feature requests should be directed to https://github.com/mars-project/mars/issues.

.. _Dask: https://docs.dask.org/en/latest/

.. _SynapseML: https://aka.ms/spark

.. _this SynapseML example: https://github.com/microsoft/SynapseML/blob/master/notebooks/features/deepray/DeePray%20-%20Overview.ipynb
...
.. _the Dask prediction example: https://github.com/microsoft/deepray/tree/master/examples/python-guide/dask/prediction.py
...
.. _these Dask examples: https://github.com/microsoft/deepray/tree/master/examples/python-guide/dask

.. _Kubeflow Fairing: https://www.kubeflow.org/docs/components/fairing/fairing-overview

.. _These examples: https://github.com/kubeflow/fairing/tree/master/examples/deepray

.. _Kubeflow XGBoost Operator: https://github.com/kubeflow/xgboost-operator

.. _this example: https://github.com/kubeflow/xgboost-operator/tree/master/config/samples/deepray-dist

.. _here: https://www.youtube.com/watch?v=iqzXhp5TxUY

.. _A simple distributed learning example: https://github.com/microsoft/deepray/tree/master/examples/parallel_learning

.. _lightgbm_ray: https://github.com/ray-project/lightgbm_ray

.. _Ray: https://ray.io/

.. _the lightgbm_ray documentation: https://docs.ray.io/en/latest/tune/api_docs/integration.html#deepray-tune-integration-deepray

.. _Mars: https://docs.pymars.org/en/latest/index.html

.. _the mars documentation: https://docs.pymars.org/en/latest/user_guide/learn/deepray.html
