...
def if_mkldnn_openmp(if_true, if_false = []):
    ...
    """
    return select({
        "@//third_party/mkl_dnn:build_with_mkldnn_openmp": if_true,
...
def if_onednn_v3(if_true, if_false = []):
    ...
    """
    return select({
        "@//third_party/mkl_dnn:build_with_onednn_v3": if_true,
        "//conditions:default": if_false,
    })

def if_mkldnn_aarch64_acl(if_true, if_false = []):
    return select({
        "@//third_party/mkl:build_with_mkl_aarch64": if_true,
        "//conditions:default": if_false,
    })

def if_mkldnn_aarch64_acl_openmp(if_true, if_false = []):
    return select({
        "@//third_party/mkl_dnn:build_with_mkl_aarch64_openmp": if_true,
        "//conditions:default": if_false,
    })
