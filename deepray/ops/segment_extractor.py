...
def _get_random(positions, random_fn):
  flat_random = random_fn(shape=tf.shape(positions.flat_values), minval=0, maxval=1, dtype=tf.float32)
...
def _random_int_from_range(minval, maxval, random_fn):
  ...
  float_maxval = tf.cast(maxval, tf.float32)
  return tf.cast(random_fn(tf.shape(maxval), minval=float_minval, maxval=float_maxval), maxval.dtype)
...
def _sample_from_other_batch(sentences, random_fn):
  ...
  # sentence.
  other_batch = random_fn(shape=[tf.size(sentences)], minval=0, maxval=sentences.nrows() - 1, dtype=tf.int64)
  ...
  # that we sampled.
  other_sentence = _random_int_up_to(tf.gather(sentences.row_lengths(), other_batch), random_fn)
  return sentences.with_values(tf.stack([other_batch, other_sentence], axis=1))


def get_sentence_order_labels(sentences, random_threshold=0.5, random_next_threshold=0.5, random_fn=tf.random.uniform):
  ...
  row_lengths_broadcasted = tf.expand_dims(positions.row_lengths(), -1) + 0 * positions
  ...
  # For every position j in a row, sample a position preceeding j or
  # a position which is [0, j-1]
  all_preceding = tf.ragged.map_flat_values(_random_int_up_to, positions, random_fn)
  ...
  rows_broadcasted = tf.expand_dims(tf.range(sentences.nrows()), -1) + 0 * positions
  ...
  valid_preceding_mask = tf.cast(tf.concat([all_zeros[:, :1], all_ones[:, 1:]], -1), tf.bool)
  valid_succeeding_mask = tf.cast(tf.concat([all_ones[:, :-1], all_zeros[:, -1:]], -1), tf.bool)
...
def get_next_sentence_labels(sentences, random_threshold=0.5, random_fn=tf.random.uniform):
  ...
  next_sentences_pos = (positions + 1) % tf.expand_dims(sentences.row_lengths(), 1)
  rows_broadcasted = tf.expand_dims(tf.range(sentences.nrows()), -1) + 0 * positions
