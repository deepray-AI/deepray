...
class DecodingModule(tf.Module, metaclass=abc.ABCMeta):
  ...
  def generate(
    ...
    def _generate_step(state):
      topk_seq, topk_log_probs, topk_ids, new_cache = self._grow_alive_seq(state, batch_size)
      new_finished_flags = self._finished_flags(topk_ids, state)
      alive_state = self._get_new_alive_state(topk_seq, topk_log_probs, new_finished_flags, new_cache)
      finished_state = self._get_new_finished_state(state, topk_seq, topk_log_probs, new_finished_flags, batch_size)
  ...
  @abc.abstractmethod
  def _grow_alive_seq(self, state: Dict[str, Any], batch_size: int) -> InternalState:
  ...
  @abc.abstractmethod
  def _finished_flags(self, topk_ids: tf.Tensor, state: Dict[str, Any]) -> tf.Tensor:
