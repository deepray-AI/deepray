...
class SequenceBeamSearch(tf.Module):
  ...
  def search(self, initial_ids, initial_cache):
    ...
    def _grow_alive_seq(state):
      ...
      topk_seq, new_cache = self._gather_beams([alive_seq, new_cache], topk_beam_indices, batch_size, beams_to_keep)
    ...
    def _get_new_finished_state(state, new_seq, new_log_probs, new_finished_flags):
      ...
      if not self.padded_decode:
        finished_seq = tf.concat([finished_seq, tf.zeros([batch_size, self.beam_size, 1], tf.int32)], axis=2)
