...
def sample_top_k(logits, top_k):
  ...
  """
  top_k = tf.clip_by_value(top_k, clip_value_min=1, clip_value_max=tf.shape(logits)[-1])
...
def sample_top_p(logits, top_p):
  ...
  range_for_gather = tf.tile(range_for_gather * logits_shape[1], [1, logits_shape[1]]) + sorted_indices
  flattened_logits = tf.reshape(logits, [-1])
  flattened_sorted_indices = tf.reshape(range_for_gather, [-1])
  sorted_logits = tf.reshape(tf.gather(flattened_logits, flattened_sorted_indices), [logits_shape[0], logits_shape[1]])
  ...
  top_p_logits = set_tensor_by_indices_to_value(logits, indices_to_remove, np.NINF)
  return top_p_logits
...
class SamplingModule(decoding_module.DecodingModule, metaclass=abc.ABCMeta):
  ...
  def __init__(
    ...
    self.sample_temperature = tf.convert_to_tensor(sample_temperature, dtype=tf.float32)
  ...
  def _grow_alive_seq(self, state: Dict[str, Any], batch_size: int) -> decoding_module.InternalState:
    ...
    candidate_log_probs = decoding_module.log_prob_from_logits(new_logits)
    ...
    if self.enable_greedy:
      ...
      topk_ids = tf.random.categorical(sampled_logits, dtype=tf.int32, num_samples=1)
      topk_log_probs = tf.gather(original_log_probs, topk_ids, axis=1, batch_dims=1)
    if self.padded_decode:
      topk_seq = tf.transpose(alive_seq, perm=[1, 0])
      topk_seq = tf.tensor_scatter_nd_update(topk_seq, [[i + 1]], tf.expand_dims(tf.squeeze(topk_ids, -1), 0))
  ...
  def _get_new_finished_state(
    ...
    if not self.padded_decode:
      finished_seq = tf.concat([finished_seq, tf.zeros([batch_size, 1], tf.int32)], axis=-1)
    ...
    new_seq = tf.multiply(new_seq, tf.cast(tf.logical_not(finished_flags), new_seq.dtype))
    new_scores = tf.multiply(new_scores, tf.cast(tf.logical_not(finished_flags), new_scores.dtype))

    finished_seq += tf.multiply(new_seq, tf.cast(new_finished_flags, new_seq.dtype))
  ...
  def _process_finished_state(self, finished_state: Dict[str, Any]) -> decoding_module.Output:
    ...
    if self.length_normalization_fn is not None:
      length_norm = self.length_normalization_fn(self.max_decode_length + 1, self.dtype)
      alive_log_probs = alive_log_probs / length_norm
    seq_cond = decoding_module.expand_to_same_rank(finished_cond, finished_seq)
    score_cond = decoding_module.expand_to_same_rank(finished_cond, finished_scores)
    finished_seq = tf.where(seq_cond, finished_seq, alive_seq)
    finished_scores = tf.where(score_cond, finished_scores, alive_log_probs)
    if self.extra_cache_output:
      return finished_seq, finished_scores, finished_state[decoding_module.StateKeys.INITIAL_OUTPUT_CACHE]
  ...
  def _continue_search(self, state) -> tf.Tensor:
    ...
    all_has_eos = tf.reduce_all(state[decoding_module.StateKeys.FINISHED_FLAGS], axis=None, name="search_finish_cond")
  ...
  def _finished_flags(self, topk_ids, state) -> tf.Tensor:
    ...
    new_finished_flags = tf.logical_or(new_finished_flags, state[decoding_module.StateKeys.FINISHED_FLAGS])
