...
from deepray.layers import attention
...
class CachedAttentionTest(keras_parameterized.TestCase):
  ...
  def test_masked_attention(self):
    ...
    # Invoke the data with a random set of mask data. This should mask at least
    # one element.
    mask_data = np.random.randint(2, size=(batch_size, from_seq_length, from_seq_length))
  ...
  def test_padded_decode(self):
    ...
    from_data = tf.zeros((batch_size, from_seq_length, 8), dtype=np.float32)
    decode_loop_step = 2
    mask_data = np.random.randint(2, size=(batch_size, from_seq_length, from_seq_length), dtype=np.int32)
    # Testing the invocation directly as Keras cannot consume inputs correctly.
    masked_output_data, cache = layer([from_data, from_data, mask_data, cache], decode_loop_step=decode_loop_step)
    self.assertEqual(masked_output_data.shape, (3, 4, 2, 2))
    self.assertEqual(cache["value"].shape, (3, 4, 2, 2))
