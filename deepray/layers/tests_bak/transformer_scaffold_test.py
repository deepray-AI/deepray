...
from deepray.layers import transformer_scaffold
...
class TransformerLayerTest(keras_parameterized.TestCase):
  ...
  def test_layer_invocation_with_mask(self):
    ...
    mask_data = np.random.randint(2, size=(batch_size, sequence_length, sequence_length))
  ...
  def test_layer_invocation_with_float16_dtype(self):
    ...
    # (the NN is too complex) but this will rule out structural runtime errors.
    batch_size = 6
    input_data = (10 * np.random.random_sample((batch_size, sequence_length, width))).astype(np.float16)
    # The attention mask should be of shape (batch, from_seq_len, to_seq_len),
    # which here is (batch, sequence_length, sequence_length)
    mask_data = np.random.randint(2, size=(batch_size, sequence_length, sequence_length))
    _ = model.predict([input_data, mask_data])
