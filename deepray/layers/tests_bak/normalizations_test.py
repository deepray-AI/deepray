...
def test_regularizations():
  layer = GroupNormalization(gamma_regularizer="l1", beta_regularizer="l1", groups=4, axis=2)
  layer.build((None, 4, 4))
  assert len(layer.losses) == 2
  max_norm = tf.keras.constraints.max_norm
  layer = GroupNormalization(groups=2, gamma_constraint=max_norm, beta_constraint=max_norm)
  layer.build((None, 3, 4))
...
def test_groupnorm_2d_different_groups():
  ...
  for i in groups:
    ...
    np.testing.assert_allclose(out.mean(axis=(0, 1), dtype=np.float32), (0.0, 0.0, 0.0), atol=1e-1)
    np.testing.assert_allclose(out.std(axis=(0, 1), dtype=np.float32), (1.0, 1.0, 1.0), atol=1e-1)
...
def test_groupnorm_convnet():
  ...
  np.testing.assert_allclose(np.mean(out, axis=(0, 2, 3), dtype=np.float32), (0.0, 0.0, 0.0), atol=1e-1)
  np.testing.assert_allclose(np.std(out, axis=(0, 2, 3), dtype=np.float32), (1.0, 1.0, 1.0), atol=1e-1)
...
def test_groupnorm_convnet_no_center_no_scale():
  np.random.seed(0x2020)
  model = tf.keras.models.Sequential()
  norm = GroupNormalization(axis=-1, groups=2, center=False, scale=False, input_shape=(3, 4, 4))
  model.add(norm)
  model.compile(loss="mse", optimizer="sgd")
  # centered and variance are  5.0 and 10.0, respectively
  x = np.random.normal(loc=5.0, scale=10.0, size=(1000, 3, 4, 4))
...
def calculate_frn(x, beta=0.2, gamma=1, eps=1e-6, learned_epsilon=False, dtype=np.float32):
  if learned_epsilon:
    eps = eps + 1e-4
...
def test_with_gamma(dtype):
  ...
  inputs = np.random.rand(28, 28, 1).astype(dtype)
  inputs = np.expand_dims(inputs, axis=0)
  frn = FilterResponseNormalization(beta_initializer="zeros", gamma_initializer="ones", dtype=dtype)
  frn.build((None, 28, 28, 1))
  observed = frn(inputs)
...
def test_with_epsilon(dtype):
  ...
  frn.build((None, 28, 28, 1))
  observed = frn(inputs)
  expected = calculate_frn(inputs, beta=0.5, gamma=1, learned_epsilon=True, dtype=dtype)
  np.testing.assert_allclose(expected[0], observed[0])
...
def test_eps_gards(dtype):
  ...
  input_layer = tf.keras.layers.Input(shape=(32, 32, 3))
  frn = FilterResponseNormalization(beta_initializer="ones", gamma_initializer="ones", learned_epsilon=True)
