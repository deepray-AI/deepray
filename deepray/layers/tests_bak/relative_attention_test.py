...
def _create_mock_attention_data(
  ...
  if include_state:
    ...
    state_data = dict(state=tf.random.normal(shape=(batch_size, memory_length, value_dim)))
  ...
  if include_mask:
    ...
    if two_stream:
      mask_data = dict(content_attention_mask=mask_data, query_attention_mask=mask_data)
    else:
