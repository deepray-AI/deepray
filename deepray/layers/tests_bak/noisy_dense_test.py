...
@pytest.mark.parametrize("input_shape", [(3, 2), (3, 4, 2), (None, None, 2), (3, 4, 5, 2)])
...
def test_noisy_dense_dtype(dtype):
  inputs = tf.convert_to_tensor(np.random.randint(low=0, high=7, size=(2, 2)), dtype=dtype)
...
def test_noisy_dense_with_policy():
  ...
  outputs = layer(inputs)
  output_signature = layer.compute_output_signature(tf.TensorSpec(dtype="float16", shape=(2, 2)))
