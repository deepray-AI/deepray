...
from deepray.layers import relative_attention
...
def _cache_memory(current_state, previous_state, memory_length, reuse_length=0):
  ...
  if memory_length is None or memory_length == 0:
    ...
    if previous_state is None:
      ...
      new_mem = tf.concat([previous_state, current_state], 1)[:, -memory_length:, :]
...
class TransformerXLBlock(tf.keras.layers.Layer):
  ...
  **Note: This layer is currently experimental.
  ...
  def call(
    ...
    attention_output = {}
    for attention_stream, input_stream, attention_key in zip(attention_streams, input_streams, attention_keys):
