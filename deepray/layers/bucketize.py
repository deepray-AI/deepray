...
class CategoryToIdLayer(tf.keras.layers.Layer):
  def __init__(self, vocabulary_list, values):
    ...
    init = tf.lookup.KeyValueTensorInitializer(
      keys=tf.constant(self.vocabulary_list, dtype=tf.int64),
      values=tf.constant(self.values, dtype=tf.int64))
...
class Hash:
  def __init__(self, hash_size: int, mask: bool = False):
    """
  ...
  def __call__(self, input_tensor: tf.Tensor) -> tf.Tensor:
    ...
    :param input_tensor: The input tensor to apply the hash function to.
    :return: The result of applying the hash function to the input tensor.
    """
    output_tensor = tf.math.floormod(input_tensor, self.hash_size)
    if self.mask:
...
class NumericaBucketIdLayer(tf.keras.layers.Layer):
  ...
  def call(self, inputs):
    id_tensor = tf.raw_ops.Bucketize(
      input=inputs, boundaries=self.bucket_boundaries
    )
    return id_tensor
