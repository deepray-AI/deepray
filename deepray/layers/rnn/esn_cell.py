...
class ESNCell(AbstractRNNCell):
  ...
  def build(self, inputs_shape):
    ...
    if input_size is None:
      raise ValueError("Could not infer input size from inputs.get_shape()[-1]. Shape received is %s" % inputs_shape)
    ...
    def _esn_recurrent_initializer(shape, dtype, partition_info=None):
      ...
      if self.use_norm2:
        ...
        recurrent_norm2 = tf.math.sqrt(tf.math.reduce_sum(tf.math.square(recurrent_weights)))
        is_norm2_0 = tf.cast(tf.math.equal(recurrent_norm2, 0), dtype)
        scaling_factor = tf.cast(self.spectral_radius, dtype) / (recurrent_norm2 + 1 * is_norm2_0)
      else:
        abs_eig_values = tf.abs(tf.linalg.eig(recurrent_weights)[0])
        scaling_factor = tf.math.divide_no_nan(tf.cast(self.spectral_radius, dtype), tf.reduce_max(abs_eig_values))
