...
def test_base_esn():
  units = 3
  expected_output = np.array([[2.77, 2.77, 2.77], [4.77, 4.77, 4.77], [6.77, 6.77, 6.77]], dtype=np.float32)
...
def test_esn_echo_state_property_norm2():
  use_norm2 = True
  units = 3
  cell = ESNCell(units=units, use_norm2=use_norm2, recurrent_initializer="ones", connectivity=1.0)
  cell.build((3, 3))
...
def test_esn_connectivity():
  ...
  np.testing.assert_allclose(np.asarray([actual_connectivity]), np.asanyarray([connectivity]), 1e-2)


def test_esn_keras_rnn():
  cell = ESNCell(10)
  seq_input = tf.convert_to_tensor(np.random.rand(2, 3, 5), name="seq_input", dtype=tf.float32)
