...
def test_output_size():
  ...
  mha = MultiHeadAttention(head_size=head_size, num_heads=num_heads, output_size=output_size)
...
def test_extra_dims_atten_coef():
  ...
  mha = MultiHeadAttention(head_size=head_size, num_heads=num_heads, return_attn_coef=True)
...
def test_compute_output_shape():
  ...
  output_shape = mha.compute_output_shape([(batch_size, 5, 9), (batch_size, 7, 11), (batch_size, 7, 13)])
...
def test_save_load_model():
  ...
  net, attn_coef = MultiHeadAttention(head_size=head_size, num_heads=num_heads, dropout=0.5,
                                      return_attn_coef=True)([inputs, inputs, inputs])
...
def test_fit_predict_eval():
  ...
  net = MultiHeadAttention(head_size=head_size, num_heads=num_heads, dropout=0.5)([inputs, inputs, inputs])
  net = tf.keras.layers.GlobalAveragePooling1D()(net)
  net = tf.keras.layers.Dense(10, activation="softmax")(net)
