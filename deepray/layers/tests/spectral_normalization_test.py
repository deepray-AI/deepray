...
from deepray.layers import spectral_normalization
...
def test_normalization():
  ...
  base_layer = tf.keras.layers.Conv2D(1, (2, 2), kernel_initializer=tf.constant_initializer(value=2))
  sn_layer = spectral_normalization.SpectralNormalization(base_layer)
  model = tf.keras.models.Sequential(layers=[inputs, sn_layer])
  ...
  for training in [False, True]:
    _ = model(tf.constant(np.ones((1, 2, 2, 1), dtype=np.float32)), training=training)
    if training:
...
def test_no_kernel():
  with pytest.raises(AttributeError):
    spectral_normalization.SpectralNormalization(tf.keras.layers.MaxPooling2D(2, 2)).build((2, 2))
