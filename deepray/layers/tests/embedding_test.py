...
from deepray.layers.embedding import Embedding, MultiHashEmbedding
...
class EmbeddingTest(test_combinations.TestCase):
    ...
    #         Embedding,
    #         kwargs={"embedding_dim": 4, "vocabulary_size": 10, "input_length": 2},
    #         input_shape=(3, 2),
    #         input_dtype="int32",
    ...
    #     )
        ...
        # test_utils.layer_test(
        #     Embedding,
        #     kwargs={"embedding_dim": 4, "vocabulary_size": 10, "mask_zero": True},
        #     input_shape=(3, 2),
        #     input_dtype="int32",
        #     expected_output_dtype="float32",
        # )
    ...
    def test_use_one_hot(self):
        ...
        layer = Embedding(
            vocabulary_size=100, embedding_dim=16, use_one_hot_matmul=True
        )
...
class MultiHashEmbeddingTest(test_combinations.TestCase):
    @test_combinations.run_all_keras_modes
    def test_embedding(self):
        if tf.test.is_gpu_available():
            self.skipTest("Only test embedding on CPU.")
    ...
    def test_embedding_correctness(self):
        layer = MultiHashEmbedding(embedding_dim=2, vocabulary_size=2)
        model = keras.models.Sequential([layer])
    ...
    def test_embedding_incorrect_dimension(self):
        with self.assertRaises(ValueError):
            MultiHashEmbedding(vocabulary_size=0, embedding_dim=1)

        with self.assertRaises(ValueError):
            MultiHashEmbedding(vocabulary_size=1, embedding_dim=0)

    @test_combinations.generate(
        test_combinations.combine(mode=["graph", "eager"])
    )
    def test_eager_gpu_cpu(self):
        l = MultiHashEmbedding(embedding_dim=2, vocabulary_size=2)
        l.build((None, 2))
        inputs = keras.backend.constant([[0, 1, 0]], dtype="int32")
        with tf.GradientTape() as tape:
            output = l(inputs)
    ...
    def test_mixed_precision_embedding(self):
        try:
            policy.set_global_policy("mixed_float16")
            layer = MultiHashEmbedding(vocabulary_size=5, embedding_dim=2)
            self.assertEqual(layer._dtype_policy.name, "mixed_float16")
            outputs = layer(np.array([0, 1, 2]))
            self.assertEqual(outputs.dtype, "float16")
        finally:
    ...
    def test_error_message_for_mask_zero_enabled_with_sparse_tensor(self):
        with self.assertRaisesRegex(
            ValueError,
            "`mask_zero` cannot be enabled when "
            "`tf.keras.layers.MultiHashEmbedding` is used with `tf.SparseTensor` "
            "input.",
        ):
            layer = MultiHashEmbedding(
                vocabulary_size=3,
    ...
    def test_use_one_hot(self):
        batch = 8
        input_length = 10
        layer = MultiHashEmbedding(vocabulary_size=100, embedding_dim=16)
        self.assertFalse(layer._use_one_hot_matmul)
