...
class BertEncoderTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def test_dict_outputs_network_creation(self, encoder_cls):
    ...
    test_network = encoder_cls(vocab_size=100, hidden_size=hidden_size, num_attention_heads=2, num_layers=3, **kwargs)
  ...
  def test_dict_outputs_network_creation_return_attention_scores(self, encoder_cls):
    ...
    expected_data_shape = [None, num_attention_heads, sequence_length, sequence_length]
  ...
  def test_embeddings_as_inputs(self):
    ...
    mask = tf.keras.Input(shape=(sequence_length,), dtype=tf.int32)
    type_ids = tf.keras.Input(shape=(sequence_length,), dtype=tf.int32)
    test_network.build(dict(input_word_ids=word_ids, input_mask=mask, input_type_ids=type_ids))
    embeddings = test_network.get_embedding_layer()(word_ids)
    # Calls with the embeddings.
    dict_outputs = test_network(dict(input_word_embeddings=embeddings, input_mask=mask, input_type_ids=type_ids))
    all_encoder_outputs = dict_outputs["encoder_outputs"]
    pooled = dict_outputs["pooled_output"]
...
class BertEncoderV2CompatibilityTest(tf.test.TestCase):
  ...
  def test_weights_forward_compatible(self):
    ...
    mask_data = np.random.randint(2, size=(batch_size, sequence_length))
    type_id_data = np.random.randint(num_types, size=(batch_size, sequence_length))
    data = dict(input_word_ids=word_id_data, input_mask=mask_data, input_type_ids=type_id_data)
    ...
    new_net._position_embedding_layer.set_weights(old_net._position_embedding_layer.get_weights())
    new_net._type_embedding_layer.set_weights(old_net._type_embedding_layer.get_weights())
    new_net._embedding_norm_layer.set_weights(old_net._embedding_norm_layer.get_weights())
    # embedding_dropout has no weights.

    if hasattr(old_net, "_embedding_projection"):
      new_net._embedding_projection.set_weights(old_net._embedding_projection.get_weights())
    # attention_mask_layer has no weights.

    new_net._pooler_layer.set_weights(old_net._pooler_layer.get_weights())

    for otl, ntl in zip(old_net._transformer_layers, new_net._transformer_layers):
