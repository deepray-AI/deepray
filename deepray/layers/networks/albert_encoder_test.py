...
class AlbertEncoderTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def test_network_creation(self, expected_dtype):
    ...
    kwargs = dict(vocab_size=100, hidden_size=hidden_size, num_attention_heads=2, num_layers=3)
    ...
    self.assertNotEmpty([x for x in test_network.weights if "embedding_projection/" in x.name])
    self.assertNotEmpty([x for x in test_network.weights if "transformer/" in x.name])
    self.assertEmpty([x for x in test_network.weights if "transformer/layer" in x.name])
