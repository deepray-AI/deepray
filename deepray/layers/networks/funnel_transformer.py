...
def _pool_and_concat(mask, unpool_length: int, strides: Union[Sequence[int], int], axes: Union[Sequence[int], int]):
  ...
  for axis, stride in zip(axes, strides):
    ...
    pool_tensor_shape = [slice(None)] * axis + [slice(unpool_length, None, stride)]
    pool_tensor = mask[pool_tensor_shape]
    mask = tf.concat((unpool_tensor, pool_tensor), axis=axis)
  return mask


def _create_truncated_avg_transforms(seq_length: int, pool_strides: Sequence[int]):
  ...
  for pool_stride in pool_strides:
    if pool_stride == 1:
      ...
      transform = [[1.0 if (i // pfac) == j else 0.0 for j in range(psl)] for i in range(sl)]
...
def _create_truncated_avg_masks(input_mask: tf.Tensor, pool_strides: Sequence[int], transforms: Sequence[tf.Tensor]):
...
class FunnelTransformerEncoder(tf.keras.layers.Layer):
  ...
  def call(self, inputs, output_range: Optional[tf.Tensor] = None):
    ...
    if self._pool_type in (_MAX, _AVG):
      ...
      # b/215659399.
      pooling_transforms = _create_truncated_avg_transforms(self._max_sequence_length, self._pool_strides)
      attention_masks = _create_truncated_avg_masks(mask, self._pool_strides, pooling_transforms)
      for i, layer in enumerate(self._transformer_layers):
