...
def _create_causal_attention_mask(seq_length, memory_length, dtype=tf.float32, same_length=False):
  ...
  causal_attention_mask = tf.concat([padding, upper_triangular - diagonal], 1)
  if same_length:
...
def _combine_masks(mask1, mask2, dtype, how="and"):
  ...
  return tf.cast(operator(tf.cast(mask1, tf.bool), tf.cast(mask2, tf.bool)), dtype=dtype)
...
def _compute_attention_mask(
  ...
  if data_mask is not None:
    ...
    state_mask = tf.ones([batch_size, tf.shape(data_mask)[1], memory_length], dtype=dtype)
    # state_mask: [B, 1, M] or [B, S, M]
    data_mask = tf.concat([state_mask, data_mask], 2)
    # data_mask: [B, 1, S + M] or [B, S, S + M]

    if attention_type == "uni":
      attention_mask = _combine_masks(causal_attention_mask, data_mask[:, None, :, :], dtype=dtype)
    else:
...
def _compute_segment_matrix(segment_ids, memory_length, batch_size, use_cls_mask):
  ...
  memory_padding = tf.zeros([batch_size, memory_length], dtype=segment_ids.dtype)
  ...
  if use_cls_mask:
    ...
    segment_matrix = tf.equal(segment_ids[:, :, None], padded_segment_ids[:, None, :])
    ...
    segment_matrix = tf.logical_not(tf.equal(segment_ids[:, :, None], padded_segment_ids[:, None, :]))
...
def _compute_positional_encoding(
  ...
  if bi_data:
    ...
    if dtype is not None and dtype != tf.float32:
      ...
      backward_position_sequence = tf.cast(backward_position_sequence, dtype=dtype)

    if clamp_length > 0:
      forward_position_sequence = tf.clip_by_value(forward_position_sequence, -clamp_length, clamp_length)
      backward_position_sequence = tf.clip_by_value(backward_position_sequence, -clamp_length, clamp_length)

    if batch_size is not None:
      forward_positional_encoding = position_encoding_layer(forward_position_sequence, batch_size // 2)
      backward_positional_encoding = position_encoding_layer(backward_position_sequence, batch_size // 2)
    else:
      forward_positional_encoding = position_encoding_layer(forward_position_sequence, None)
      backward_positional_encoding = position_encoding_layer(backward_position_sequence, None)

    relative_position_encoding = tf.concat([forward_positional_encoding, backward_positional_encoding], axis=0)
  else:
    forward_position_sequence = tf.range(beg, end, -1.0)
    if dtype is not None and dtype != tf.float32:
      forward_position_sequence = tf.cast(forward_position_sequence, dtype=dtype)
    ...
    relative_position_encoding = position_encoding_layer(forward_position_sequence, batch_size)
