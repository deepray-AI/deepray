...
class DynamicEmbedding(tf.keras.layers.Layer):
  ...
  def __init__(
    ...
    try:
      ...
      raise TypeError('embeddnig size must be convertible to integer, but get {}'.format(type(embedding_size)))
    ...
    with tf.name_scope('DynamicEmbedding'):
      self.params = de.get_variable(
          ...
          restrict_policy=kwargs.get('restrict_policy', None),
  ...
  def call(self, ids):
    ...
    if self.with_unique:
      with tf.name_scope(self.name + "/EmbeddingWithUnique"):
        ...
        unique_embeddings = de.shadow_ops.embedding_lookup(self.shadow, unique_ids)
  ...
  def get_config(self):
    ...
    config = {
        ...
        'devices': self.params.devices if self.keep_distribution else None,
        'name': self.name,
        'trainable': self.trainable,
        'bp_v2': self.params.bp_v2,
        'restrict_policy': _restrict_policy,
  ...
  def build(self, input_shape):
    ...
    for name, length, dim, voc_size, dtype, hash_size, multihash_factor, storage_type, bucket_boundaries in self.feature_map[
        ~(self.feature_map['ftype'].isin(["Label", "Weight"]))][[
            "name", "length", "dim", "voc_size", "dtype", "hash_size", "multihash_factor", "storage_type",
            "bucket_boundaries"
        ]].values:
      ...
      if self.fold_columns[name] not in self.embedding_layers:
        multihash_factor = self.feature_map.loc[self.feature_map['name'] == self.fold_columns[name]
                                               ]['multihash_factor'].values[0] if self.fold_columns[
