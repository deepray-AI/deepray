...
@contextmanager
def change_global_flag_for_multi_level_hash(is_multi_level, is_ignore_eflops_device_fn):
  """
...
def multi_level_hash_creator(
  ...
  def inner(next_creator, **kwargs):
    if "is_kvvar" in kwargs:
      ...
      kwargs["is_ignore_eflops_device_fn"] = (_IS_IGNORE_EFLOPS_DEVICE_FN or is_ignore_eflops_device_fn)
...
class _KvVariableStore(tf_variable_scope._VariableStore):  # pylint: disable=protected-access
  ...
  def get_kv_variable(
    ...
    if shape is not None:
      shape = tensor_shape.as_shape([10000]).concatenate(tensor_shape.as_shape(shape))
  ...
  def _get_partitioned_kv_variable(
    ...
    """
        ...
        """
    initializing_from_value = initializer is not None and isinstance(initializer, ops.Tensor)
    ...
    if not reuse or partitioner:
      ...
      if is_tf_1_13_or_higher():
        partitions = tf_variable_scope._call_partitioner(partitioner, shape, dtype)
      else:
      ...
      shards = partitions[0]
      shape = tensor_shape.as_shape([10000 * shards]).concatenate(embedding_dim)
    ...
    if name in self._partitioned_vars:
      ...
      if not key_dtype.is_compatible_with(existing_var._variable_list[0].key_dtype):  # pylint: disable=protected-access
        raise ValueError(
      ...
      if (partitions is not None and existing_var._get_partitions() != partitions):  # pylint: disable=protected-access
        raise ValueError(
            "Trying to reuse partitioned variable %s, but specified partitions "
            "%s and found partitions %s." % (name, partitions, existing_var._get_partitions())
        )  # pylint: disable=protected-access
    ...
    if "%s/part_0" % name in self._vars:
      if "%s/part_%d" % (name, num_slices - 1) not in self._vars:
        raise ValueError(
            ...
            "%s/part_0 was found, but %s/part_%d was not." % (num_slices, name, name, num_slices - 1)
        )
      if "%s/part_%d" % (name, num_slices) in self._vars:
        raise ValueError(
            "Partitioner returned a different partitioning than what was "
            "already found.  Partitioner returned %d shards, and shard "
            "%s/part_0 was found, but so was the extra shard %s/part_%d." % (num_slices, name, name, num_slices)
        )
    ...
    for i, (var_offset, var_shape) in enumerate(iter_slices(shape.as_list(), num_slices, slice_dim)):
...
def default_kv_variable_creator(next_creator=None, **kwargs):
  ...
  # Set trainable value based on synchronization value.
  # tf2 change
  synchronization = kwargs.get("synchronization", tf.VariableSynchronization.AUTO)
...
tf_variable_scope.default_variable_creator = variable_creator_wrapper(tf_variable_scope.default_variable_creator)
tf_variable_scope.variables.default_variable_creator_v2 = (
    variable_creator_wrapper(  # pylint: disable=line-too-long
        resource_variable_ops.default_variable_creator_v2
    )
)  # tf2.13 change
...
class _pure_kv_variable_scope:  # pylint: disable=protected-access, invalid-name
    ...
    if isinstance(self._name_or_scope, tf_variable_scope.VariableScope):
      ...
      if self._custom_getter is not None:
        variable_scope_object.set_custom_getter(
            tf_variable_scope._maybe_wrap_custom_getter(self._custom_getter, self._name_or_scope.custom_getter)
        )
  ...
  def __enter__(self):
    ...
    if isinstance(self._name_or_scope, tf_variable_scope.VariableScope):
      ...
      self._reuse = (self._reuse or self._old.reuse)  # Re-using is inherited by sub-scopes.
      ...
      if self._custom_getter is not None:
        variable_scope_object.set_custom_getter(
            tf_variable_scope._maybe_wrap_custom_getter(self._custom_getter, self._old.custom_getter)
        )
      if self._dtype is not None:
        variable_scope_object.set_dtype(self._dtype)
      if self._use_resource is not None:
...
def create_slot_wrapper(func):
  ...
  def wrapper(primary, val, scope, validate_shape, shape, dtype, copy_xla_sharding):
    shape = shape if callable(val) else None
    if isinstance(primary, kv_variable_ops.KvVariable):
      if shape is not None:
        shape = tensor_shape.as_shape([shape.as_list()[1] * int(primary.num_concat_opt_vars)])
      ...
      if primary.is_multi_level:
        ...
        with tf_variable_scope.variable_creator_scope(multi_level_hash_creator()):
...
# pylint:disable=protected-access
slot_creator._create_slot_var = create_slot_wrapper(slot_creator._create_slot_var)
