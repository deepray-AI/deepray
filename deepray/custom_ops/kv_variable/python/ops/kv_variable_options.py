...
def get_remote_option(storage_path):
  return KvOptions(
      ...
      configs={
          StorageType.REMOTE_STORAGE: KvStorageConfig(storage_path=storage_path),
      },
  )
...
class KvOptions:
  ...
  def __init__(self, combination=StorageCombination.MEM, configs=None):
    ...
    for storage_type in COMBINATION_MAPPING[combination]:
      if storage_type not in configs:
        raise ValueError("Storage type %s should in combination %s" % (storage_type, combination))
  ...
  def has_storage(self, storage_type):
    return (storage_type in COMBINATION_MAPPING[self.storage_option.combination])
