...
device_setter.STANDARD_PS_OPS = (device_setter.STANDARD_PS_OPS + utils.get_kv_variable_op_types())  # pylint: disable=bad-whitespace
...
def _BaseSaverBuilder_AddRestoreOps(  # pylint: disable=invalid-name
  ...
  for saveable in saveables:
    if isinstance(saveable, KvVariableSaveable):
      ...
      break
  if (first_kv_saveable is not None and first_kv_saveable.var.is_ignore_eflops_device_fn):
  ...
  for saveable in saveables:
    ...
    if isinstance(saveable, KvVariableSaveable):
      assign_ops.append(saveable.restore(saveable_tensors, shapes, filename_tensor))
    else:
      assign_ops.append(saveable.restore(saveable_tensors, shapes))
  if (one_kv_variable is not None and one_kv_variable.is_ignore_eflops_device_fn):
...
def enable_save_v3():
  ...
  def save_v2_hook(filename_tensor, tensor_names, tensor_slices, tensors, has_ev=None):  # pylint: disable=unused-argument
...
def get_or_create_do_full_export():
  """
    ...
    """
  try:
    do_full_export = ops.get_default_graph().get_tensor_by_name("do_full_export:0")
  except Exception:  # pylint: disable=broad-except
    with ops.name_scope(""):
      do_full_export = array_ops.placeholder_with_default(True, shape=(), name='do_full_export')
...
def get_or_create_is_loading_finished():
  """
    ...
    """
  try:
    is_loading_finished = ops.get_default_graph().get_tensor_by_name("is_loading_finished:0")
  except Exception:  # pylint: disable=broad-except
    with ops.name_scope(""):
      is_loading_finished = array_ops.placeholder_with_default(True, shape=(), name="is_loading_finished")
...
def get_or_create_first_n():
  ...
  try:
    ...
    with ops.name_scope(""):
      first_n = array_ops.placeholder_with_default(8, shape=(), name="first_n")
  return first_n
...
def _eager_safe_variable_handle(
  ...
  if exists:
    raise ValueError(
        "variable object with name '%s' already created. Use "
    ...
    try:
      ...
      with context.eager_mode():
        with ops.device(self._handle_device):
          gen_kv_variable_ops.destroy_kv_variable_op_v2(self._handle, ignore_lookup_error=True)
    except TypeError:
...
def add_inner_table_for_multi_hash(params, names):
  ...
  if isinstance(params, PartitionedVariable):
    ...
    for part, v in enumerate(var):
      v.add_multi_level_hash([f"{scope_name}{name}/part_{part}:0" for name in names], names)
  else:
    params.add_multi_level_hash([f"{scope_name}{name}:0" for name in names], names)
...
class KvVariable(resource_variable_ops.ResourceVariable):  # pylint: disable=abstract-method
  ...
  def __init__(
    ...
    self._is_ignore_eflops_device_fn = kwargs.get("is_ignore_eflops_device_fn", False)
    ...
    graph_embedding_num_shards = ops.get_collection(variable_scope.GRAPHSHARDS_KEY)[0]
  ...
  def add_multi_level_hash(self, names, origin_names):
    ...
    if self._is_multi_level:
      ...
      duplicate_names = set(origin_names) & set(self._multi_level_names_origin)
      if duplicate_names:
        raise ValueError("Found duplicate names when add sub hash "
      ...
      insert = gen_kv_variable_ops.append_kv_variable_for_multi_hash(self._handle, names_in_tensor)
  ...
  def _init_from_arguments(
    ...
    if (
        isinstance(initial_value, ops.Tensor) and hasattr(initial_value, "graph") and
        initial_value.graph.building_function
    ):
      raise ValueError(
    ...
    if not isinstance(collections, (list, tuple, set)):
      raise ValueError(
          ...
          " tuple, or set. Got %s of type %s" % (collections, type(collections))
      )
    ...
    with ops.init_scope():
      ...
      with ops.name_scope(name, "KvVariable", [] if init_from_fn else [initial_value]) as var_scope_name:
        ...
        if self._in_graph_mode:
          with ops.name_scope("IsInitialized"):
            self._is_initialized_op = (gen_kv_variable_ops.kv_variable_is_initialized_v2(self._handle))
          if initial_value is not None:
            with ops.name_scope("Init") as n, ops.colocate_with(self._handle):
              self._initializer_op = gen_kv_variable_ops.init_kv_variable_v2(
                  self._handle,
                  _try_guard_against_uninitialized_dependencies(n, initial_value),
                  name=n,
              )
    ...
    if not self._in_graph_mode:
      self._handle_deleter = EagerResourceDeleter(handle=self._handle, handle_device=self._handle.device)
  ...
  def _init_from_proto(self, variable_def, import_scope=None):
    ...
    self._handle = g.as_graph_element(ops.prepend_name_scope(variable_def.variable_name, import_scope=import_scope))
    self._shape = tensor_shape.as_shape([10000]).concatenate(self._handle.op.get_attr("value_shape"))
    self._handle_name = self._handle.name
    self._unique_id = self._handle_name
    self._initializer_op = g.as_graph_element(
        ops.prepend_name_scope(variable_def.initializer_name, import_scope=import_scope)
    )
    if (hasattr(variable_def, "initial_value_name") and variable_def.initial_value_name):
      self._initial_value = g.as_graph_element(
          ops.prepend_name_scope(variable_def.initial_value_name, import_scope=import_scope)
      )
    else:
    ...
    self._value_dtype = dtypes.as_dtype(self._handle.op.get_attr("value_dtype"))
    self._graph_element = g.get_tensor_by_name(self._handle.op.name + "/Read/ReadKvVariableOpV2:0")[1]
    if variable_def.HasField("save_slice_info_def"):
      self._save_slice_info = variables.Variable.SaveSliceInfo(
          save_slice_info_def=variable_def.save_slice_info_def,
          import_scope=import_scope,
  ...
  def storage_size_count(self):
    return gen_kv_variable_ops.kv_variable_size_v3(self._handle, name=self._handle.op.name + "/StorageSize")
  ...
  def total_count(self):
    ...
    return gen_kv_variable_ops.kv_variable_size_v2(self._handle, name=self._handle.op.name + "/Size")

  @property
  def total_freq(self):
    """Calculate the freq of kvvariable"""
    return gen_kv_variable_ops.kv_variable_frequency(self._handle, name=self._handle.op.name + "/Freq")

  def _read_variable_op(self):
    if hasattr(self, "_trainable") and self._trainable:
      tape.variable_accessed(self)
    result = gen_kv_variable_ops.read_kv_variable_op_v2(self._handle, self._key_dtype, self._value_dtype)
    return result
  ...
  def _smart_cond(self, pred, true_fn, false_fn):
    ...
    if (self._trainable is False and self.has_path() and not util.is_tfplus_opt_slot_var(self.handle.name)):
      logging.info(
  ...
  def increase_counting(self, indices, counts, name=None):
    ...
    def train_fn():
      return gen_kv_variable_ops.kv_variable_increase_count_v2(self._handle, indices, counts, name=name)
  ...
  def get_counting(self, indices, name=None):
    return gen_kv_variable_ops.kv_variable_get_count_v2(self._handle, indices, name=name)

  def apply_snapshot(self, filename_tensor, need_full_import, ckpt_name=None, name=None):
    if ckpt_name is None:
      return gen_kv_variable_ops.kv_variable_apply_snapshot(self._handle, filename_tensor, need_full_import, name=name)
  ...
  def to_proto(self, export_scope=None):
    ...
    if export_scope is None or self.handle.name.startswith(export_scope):
      ...
      var_def.snapshot_name = ops.strip_name_scope("kv_variable", export_scope)
  ...
  def is_initialized(self, name=None):
    return gen_kv_variable_ops.kv_variable_is_initialized_v2(self.handle, name)

  # pylint: disable=unused-argument
  def assign(self, value, use_locking=None, name=None, read_value=True):
    # TODO(tongsuo.ts): add implement later
    if not isinstance(value, KvVariable):
      raise ValueError("KvVariable does not implement assign() for type %s" % type(value))
  ...
  def load_remote_storage(self, table_name):
    return gen_kv_variable_ops.kv_variable_load_remote_table(self.handle, table_name)
  ...
  def export(self, name=None):
    ...
    if _ENABLE_DELTA_EXPORT:
      ...
      with ops.colocate_with(self._handle):
        ...
        if len(tensors) != 8:
          raise RuntimeError("KvVariable: Number of exported tensors is not equal to 8" % name)
        return OrderedDict((name + "-" + k, v) for k, v in tensors._asdict().items())
    with ops.colocate_with(self._handle):
      if tfplus_saver_mode() == 0:
        # Inference mode, only export keys, value, init-tables
        # and other three tensors will be empty
  ...
  def delete(self, indices, name=None):
    return gen_kv_variable_ops.kv_variable_delete(self._handle, indices, name=name)

  def get_timestamp(self, indices, name=None):
    return gen_kv_variable_ops.kv_variable_get_time_stamp(self._handle, indices, name=name)
  ...
  def delete_with_timestamp(self, threshold, name=None):
    """Delete the tensors whose last updated time exceed the threshold.
        ...
        """
    return gen_kv_variable_ops.kv_variable_delete_with_timestamp(self._handle, self._key_dtype, threshold, name=name)
...
class KvVariableSaveable(BaseSaverBuilder.SaveableObject):
  ...
  def restore(
    ...
    with ops.control_dependencies([restore_op]):
      ...
      init_op = gen_kv_variable_ops.init_kv_variable_v2(
          self._var._handle,
          variables._try_guard_against_uninitialized_dependencies(self.var.name, self._var._initial_value),
      )
    return init_op
...
class KvVariableSaveableV3(KvVariableSaveable):
    ...
    if var.is_multi_level:
      ...
      for tensor_dict in tensors_dict:
        for tensor_name, tensor in tensor_dict.items():
          self._restore_specs.append(BaseSaverBuilder.SaveSpec(tensor, "", tensor_name))
      ...
      self._restore_specs = [
          BaseSaverBuilder.SaveSpec(tensor, "", tensor_name) for tensor_name, tensor in tensors_dict.items()
      ]
      orig_ordering = [k.split("-")[-1] for k in tensors_dict.keys()]

      if tfplus_saver_mode() == 0:
        self._restore_specs = (self._restore_specs[:3] + self._restore_specs[6:])
        self._ordering = orig_ordering[:3] + orig_ordering[6:]
        self._empty_ordering = orig_ordering[3:6]
      else:
        self._ordering = orig_ordering
  ...
  def _restore_multi_level_hash(
    ...
    with ops.colocate_with(self.op.handle):
      with ops.control_dependencies(ops.get_collection(ops.GraphKeys.TABLE_INITIALIZERS)):
        return control_flow_ops.group(
            [gen_kv_variable_ops.kv_variable_import(self.op.handle, first_n=6, **kw) for kw in all_kwargs]
        )
...
class _UnreadVariable(KvVariable):
  ...
  def _read_variable_op(self):
    with ops.control_dependencies([self._parent_op]):
      return gen_kv_variable_ops.read_kv_variable_op_v2(self._handle, self._key_dtype, self._value_dtype)
...
tensor_conversion_registry.register_tensor_conversion_function(_UnreadVariable, _dense_var_to_tensor)
...
def _GatherGrad(op, grad):  # pylint: disable=invalid-name
  ...
  params_shape = ops.convert_to_tensor(tensor_shape.TensorShape(kv_op.get_attr("value_shape")))
...
def scatter_update(ref, indices, updates, use_locking=True, name=None):
  ...
  return orignal_scatter_update(ref, indices, updates, use_locking=use_locking, name=name)
...
def scatter_add(ref, indices, updates, use_locking=True, name=None):
  ...
  return orignal_scatter_add(ref, indices, updates, use_locking=use_locking, name=name)
...
if is_tf_1_13_or_higher():
  ...
  original_saveable_objects_for_op = (saveable_object_util.saveable_objects_for_op)
else:
  original_op_list_to_dict = (saveable_object_util.BaseSaverBuilder.OpListToDict)
  original_saveable_objects_for_op = (saveable_object_util.BaseSaverBuilder.SaveableObjectsForOp)
...
def op_list_to_dict(op_list, convert_variable_to_tensor=True):
  ...
  names_to_saveables.update(original_op_list_to_dict(orignal_tf_op_list, convert_variable_to_tensor))
...
if is_tf_1_13_or_higher():
  ...
  saveable_object_util.BaseSaverBuilder.OpListToDict = staticmethod(op_list_to_dict)
  saveable_object_util.BaseSaverBuilder.SaveableObjectsForOp = staticmethod(saveable_objects_for_op)
...
def register(self, candidate, name=None):
  ...
  if name in self._registry:
    (filename, line_number, function_name, _) = self._registry[name][registry._LOCATION_TAG]
    logging.vlog(
        1,
        "Registering two %s with name '%s'! "
        "(Previous registration was in %s %s:%d)" % (self._name, name, function_name, filename, line_number),
    )
