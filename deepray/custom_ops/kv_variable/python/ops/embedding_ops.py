...
def _embedding_lookup_and_transform(
  ...
  if is_kv_variable and not all(isinstance(p, kv_variable_ops.KvVariable) for p in params):
    ...
    if np == 1 and (not transform_fn or ids.get_shape().ndims == 1):  # pylint:disable=no-else-return
      ...
      with ops.colocate_with(params[0]):
        if is_kv_variable:
          ...
          gather_op = params[0].sparse_read_with_counts(flattern_ids, counts, name=name)
        else:
      ...
      if ids.get_shape().ndims > 1:
        result = array_ops.reshape(
            result,
            array_ops.concat([array_ops.shape(ids), params[0].get_shape()[1:]], 0),
        )
      ...
      if is_kv_variable:
        ...
        if dim_0_size.value:
          ...
          num_total_ids = math_ops.reduce_sum(math_ops.cast(tf.stack(dim_0_sizes), flat_ids.dtype))
...
def _embedding_lookup_sparse(
  ...
  with ops.name_scope(name, "embedding_lookup_sparse", params + [sp_ids]) as name:  # pylint:disable=redefined-argument-from-local
    segment_ids = sp_ids.indices[:, 0]
    if segment_ids.dtype != dtypes.int32:
      segment_ids = math_ops.cast(segment_ids, dtypes.int32)

    ids = sp_ids.values
    need_counts = (isinstance(params[0], kv_variable_ops.KvVariable) and params[0].enter_threshold > 0)
...
def safe_embedding_lookup_sparse(
  ...
  embedding_weights = [
      w if (isinstance(w, resource_variable_ops.ResourceVariable) and dtype in (None, w.dtype)) else
      ops.convert_to_tensor(w, dtype=dtype) for w in embedding_weights
  ]

  is_kv_variable = isinstance(embedding_weights[0], kv_variable_ops.KvVariable)
  if is_kv_variable and not all(isinstance(p, kv_variable_ops.KvVariable) for p in embedding_weights):
  ...
  with ops.name_scope(
    ...
    if original_rank_dim is not None and original_rank_dim > 2:
      ...
      final_result.set_shape(
          tensor_shape.unknown_shape((tensor_shape.Dimension(original_rank_dim) - 1).value
    ...
    if isinstance(origin_weights, ranking_utils.SUPPORTED_TYPES):
      ...
      ranking_utils.append_embedding_result_for_ranking(unique_name, final_result)
      ranking_utils.append_embedding_input_for_ranking(unique_name, origin_sparse_ids)
    return final_result
...
original_safe_embedding_lookup_sparse = (embedding_ops.safe_embedding_lookup_sparse)
original_safe_embedding_lookup_sparse_v2 = (embedding_ops.safe_embedding_lookup_sparse_v2)
...
def embedding_lookup_unique(params, ids, partition_strategy="mod", name=None):
  ...
  with ops.name_scope(name, "EmbeddingLookupUnique", [params, ids]):
    ...
    ids_flat = array_ops.reshape(ids, math_ops.reduce_prod(shape, keepdims=True))
    unique_ids, idx = array_ops.unique(ids_flat)
    unique_embeddings = _embedding_lookup_and_transform(params, unique_ids, partition_strategy)
    embeds_flat = array_ops.gather(unique_embeddings, idx)
    embed_shape = array_ops.concat([shape, array_ops.shape(unique_embeddings)[1:]], 0)
    embeds = array_ops.reshape(embeds_flat, embed_shape)
    embeds.set_shape(ids.get_shape().concatenate(unique_embeddings.get_shape()[1:]))
  if isinstance(params, ranking_utils.SUPPORTED_TYPES):
    ranking_utils.update_embedding_for_ranking(
        unique_name,
        params,
...
def insert_kv_embedding(params, ids, values, name=None):
  ...
  assert (ids.dtype == params[0].key_dtype
         ), "ids' dtype: {} not matched with embedding's {}".format(ids.dtype, params[0].key_dtype)
  assert (values.dtype == params[0].dtype
         ), "values' dtype: {} not matched with embedding's {}".format(values.dtype, params[0].dtype)
  assert (values.shape.as_list()[-1] == params[0].shape.as_list()[-1]
         ), "values' shape: {} not matched with " "embedding dimension {}".format(values.shape, params[0].shape)
  ...
  with ops.name_scope(name, "load_kv_embedding", params + [ids, values]):
    ...
    p_assignments = math_ops.cast(p_assignments, dtypes.int32)
    # Partition list of ids based on assignments into np separate lists
    gather_keys = data_flow_ops.dynamic_partition(ids, p_assignments, num_partition)
    gather_values = data_flow_ops.dynamic_partition(values, p_assignments, num_partition)

    scatter_update_ops = [
        kv_variable_ops.scatter_update(params[i], gather_keys[i], gather_values[i]) for i in range(num_partition)
    ]
...
def use_tfplus_embedding_ops():
  ...
  safe_embedding_lookup_sparse_v2 = (embedding_ops.safe_embedding_lookup_sparse_v2)
...
def use_tf_embedding_ops():
  ...
  embedding_ops.safe_embedding_lookup_sparse = (original_safe_embedding_lookup_sparse)
  ...
  embedding_ops.safe_embedding_lookup_sparse_v2 = (original_safe_embedding_lookup_sparse_v2)
  global safe_embedding_lookup_sparse_v2
...
def _change_embedding_to_eflops_gen():
  ...
  def _is_eflops_mode():
    ...
    try:
      ...
      return False
    return all(globals()[name] is getattr(eflops_embedding_ops, name) for name in embedding_ops_name)
