...
class RectifiedAdamOptimizer(tf_adam.AdamOptimizer):
  ...
  def _prepare(self):
    ...
    self._total_steps_t = ops.convert_to_tensor(initial_total_steps, name="total_step")
    ...
    self._weight_decay_t = ops.convert_to_tensor(initial_weight_decay, name="weight_decay")
  ...
  def _tfplus_apply_dense_shared(self, grad, var):
    ...
    if self._initial_total_steps > 0:
      ...
      warmup_steps = total_steps * math_ops.cast(self._warmup_proportion_t, var_dtype)
    ...
    if self._amsgrad:
      ...
      vhat_t = state_ops.assign(vhat, tf.maximum(vhat, v_t), use_locking=self._use_locking)
    ...
    var_update = state_ops.assign_sub(var, var_t * lr_t, use_locking=self._use_locking)
  ...
  def _tfplus_apply_sparse_shared(self, grad, var, indices):
    ...
    with ops.control_dependencies([m_updates]):
      m_t = state_ops.scatter_update(m, indices, m_updates, use_locking=self._use_locking)
    if self._use_nesterov:
      logger.info(f"use_nesterov is: {self._use_nesterov}")
    ...
    with ops.control_dependencies([v_updates]):
      v_t = state_ops.scatter_update(v, indices, v_updates, use_locking=self._use_locking)

    if self._amsgrad:
      vhat = self.get_slot(var, "vhat")
      vhat_updates = tf.maximum(v_updates, array_ops.gather(vhat, indices))
      with ops.control_dependencies([vhat_updates]):
        vhat_t = state_ops.scatter_update(vhat, indices, vhat_updates, use_locking=self._use_locking)
    ...
    with ops.control_dependencies([m_updates, v_updates]):
      ...
      if self._initial_weight_decay > 0.0:
        var_updates += math_ops.cast(self._weight_decay_t, var_dtype) * array_ops.gather(var, indices)
      var_update = state_ops.scatter_sub(var, indices, var_updates * lr_t, use_locking=self._use_locking)
