...
class AdamOptimizer(tf_adam.AdamOptimizer):
  ...
  def _create_slots(self, var_list):
    ...
    for v in var_list:
      if not isinstance(v, KvVariable):
        ...
        if (self._version == 2 or v.has_path() or v.kv_options is variable_scope.default_kv_option()):
  ...
  def _tfplus_apply_sparse_shared(self, grad, var, indices):
    ...
    if enabled_concat_opt_vars:
      ...
      m_values = array_ops.slice(m_v_values, [0, 0], [indices_length, embedding_dim])
      v_values = array_ops.slice(m_v_values, [0, embedding_dim], [indices_length, embedding_dim])
    else:
    ...
    if enabled_concat_opt_vars:
      with ops.control_dependencies([m_updates, v_updates]):
        ...
        m_v_t = state_ops.scatter_update(m_v, indices, m_v_updates, use_locking=self._use_locking)
    else:
    ...
    with ops.control_dependencies([m_updates, v_updates]):
      ...
      var_update_op = state_ops.scatter_sub(var, indices, var_updates, use_locking=self._use_locking)
    if enabled_concat_opt_vars:
