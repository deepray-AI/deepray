...
class GroupAdamOptimizer(tf_adam.AdamOptimizer):
  ...
  def __init__(
    ...
    if initial_accumulator_value < 0.0:
      raise ValueError("initial_accumulator_value %f needs to be be positive or zero" % initial_accumulator_value)
      ...
      if not isinstance(v, KvVariable):
        ...
        with ops.colocate_with(v):
          if v.has_path() and self._version < 3:
            logging.warning("All slot KvVariables with ssd storage need to be merged")
          if (self._version >= 3 or v.has_path() or v.kv_options is variable_scope.default_kv_option()):
  ...
  def _resource_apply_sparse(self, grad, var, indices):
    if not isinstance(var, KvVariable):
      return super(GroupAdamOptimizer, self)._resource_apply_sparse(grad, var, indices)
    m, v, linear, m_v_linear = None, None, None, None
    if (self._version >= 3 or var.has_path() or var.kv_options is variable_scope.default_kv_option()):
    ...
    if self._version == 2:
      return (
          gen_kv_variable_ops.kv_variable_group_sparse_apply_adam_new_v2(
              var.handle,
              linear.handle,
              m.handle,
              v.handle,
