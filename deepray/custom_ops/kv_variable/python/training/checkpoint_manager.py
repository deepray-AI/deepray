...
class CheckpointStateManager(object):  # pylint: disable=useless-object-inheritance
  ...
  def __init__(self, checkpoint_dir, latest_filename=None):
    ...
    self._ckpt_state_ext = self.load_checkpoint_state_ext(latest_filename=latest_filename)
  ...
  def update_latest_full_checkpoint(self, full_checkpoint_path):
    ...
    if self._ckpt_state_ext is None:
      ...
      prev_current_full_path = (self._ckpt_state_ext.current_full_checkpoint_path)
      prev_all_delta_paths = (self._ckpt_state_ext.all_valid_delta_checkpoint_paths)
    ...
    if prev_current_full_path is not None:
      self._ckpt_state_ext.history_versions[prev_current_full_path].deltas.extend(prev_all_delta_paths)
  ...
  def add_delta_checkpoint(self, delta_checkpoint_path):
    if self._ckpt_state_ext is None:
      raise ValueError("current ckpt_ext_state is None, can not add delta checkpoint %s" % delta_checkpoint_path)

    if self._ckpt_state_ext.current_full_checkpoint_path is None:
      raise ValueError(
          "current_full_checkpoint_path is None,\
          can not add delta checkpoint %s" % delta_checkpoint_path
      )
    self._ckpt_state_ext.all_valid_delta_checkpoint_paths.append(delta_checkpoint_path)
    self.update_checkpiont_state_ext()
  ...
  def load_checkpoint_state_ext(self, latest_filename=None):
    """
        ...
        """
    ckpt = None
    coord_checkpoint_filename = self.get_checkpoint_ext_filename(latest_filename)
    f = None
    ...
    try:
      if file_io.file_exists(coord_checkpoint_filename):
        ...
        if not ckpt.current_full_checkpoint_path:
          raise ValueError("Invalid ext checkpoint state loaded from " + checkpoint_dir)
  ...
  def get_all_relative_delta_checkpoints(self, full_checkpoint_path):
    ...
    if self._ckpt_state_ext is None:
      return None
    if (full_checkpoint_path == self._ckpt_state_ext.current_full_checkpoint_path):
      return self._ckpt_state_ext.all_valid_delta_checkpoint_paths

    delta_list = self._ckpt_state_ext.history_versions.get(full_checkpoint_path, None)
  ...
  def update_checkpiont_state_ext(self):
    ...
    file_io.atomic_write_string_to_file(ckpt_ext_file, text_format.MessageToString(ckpt_state_ext))
  ...
  def remove_old_checkpoint(self, removed_full_ckpt_path, meta_graph_suffix="meta"):
    """
    ...
    if (removed_full_ckpt_path == self._ckpt_state_ext.current_full_checkpoint_path):
      raise ValueError("Can not remove %s which is the only verion checkpoint" % removed_full_ckpt_path)
    to_be_removed = [removed_full_ckpt_path]
    delta_list = self._ckpt_state_ext.history_versions.get(removed_full_ckpt_path, None)
    ...
    for ckpt in to_be_removed:
      tf_checkpoint_management.remove_checkpoint(ckpt, meta_graph_suffix=meta_graph_suffix)
      self.remove_snapshot_directory(ckpt)
    self.update_checkpiont_state_ext()
    logging.info("Delete old full ckpt %s and all relative delta versions." % removed_full_ckpt_path)
...
def process_non_abs_path(ckpt, func, checkpoint_dir):
  ...
  if not os.path.isabs(ckpt.current_full_checkpoint_path):
    ckpt.current_full_checkpoint_path = func(checkpoint_dir, ckpt.current_full_checkpoint_path)
  ...
  for full in ckpt.history_versions:
    ...
    if not os.path.isabs(full):
      ...
      history_with_full_path[new_path
