 ...
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    if (!init_token) {
      ...
      auto creator = [ctx, this, enter_threshold, &value_shape,
                      &storage_option](tfplus::KvVariableInterface** ret) {
        KvVariableInterface* container = new KvVariable<K, V>(
            this->cinfo_.name(), value_shape, enter_threshold, storage_option);
...
REGISTER_KERNEL_BUILDER(
    Name("KvVariableFullOrDeltaImportV2").Device(DEVICE_CPU),
    KvVariableFullOrDeltaImportOp);
