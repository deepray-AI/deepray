...
class TableManager {
 public:
  ...
  TableManager(StorageOption storage_option, const std::string& variable_name,
    ...
    src.setZero();
    auto table =
        new MemStorageTable<K, V>(-1, embedding_dim_, ev_table_, variable_name);
    writeable_storage_table_ = table;
    storage_tables_.push_back(table);
  }

  ~TableManager() { delete ev_table_; }
  KvMap* GetKVMap() { return ev_table_; }

  bool HasMemTable() { return with_ev_table_; }

  inline bool SSDStorageEneabled() { return false; }
  ...
  void ClassifyLocalOrRemoteKey(
      const Tensor& keys, std::vector<std::pair<K, size_t>>* local_keys,
      std::vector<std::pair<K, size_t>>* non_local_keys) {
    const auto& keys_flat = keys.flat<K>();
  ...
  std::string RemoteStorageTableName() const { return ""; }

  bool RemoteStorageEnabled() { return false; }
  ...
  Status TriggerTransferSSD(bool with_lock) { return ::tensorflow::OkStatus(); }

  Status InitRemoteTable(std::string remote_table_name) {
    return ::tensorflow::OkStatus();
  }

  void CopyToWritableStorage(const K& key, EVContext<V>* context) { return; }
