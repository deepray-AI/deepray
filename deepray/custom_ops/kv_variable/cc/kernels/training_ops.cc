...
#include "./kv_variable.h"
#include "./kv_variable_interface.h"
#include "./utility.h"
...
tf_mutex* GetTrainingVariableMutex(OpKernelContext* ctx, int input, bool sparse,
  ...
  void Compute(OpKernelContext* ctx) override TF_NO_THREAD_SAFETY_ANALYSIS {
    ...
    OP_REQUIRES(ctx, inner_dim > 0,
                errors::InvalidArgument(
                    "Inner dimension should be greater than zero."));
    if (N > 0) {
      auto DoWork = [this, ctx, &table_var, &table_accum, inner_dim, &indices,
                     &grad, &lr](int64_t start_i, int64_t limit_i) {
        auto grad_flat = grad.flat_outer_dims<T>();
        auto indices_flat = indices.flat<Tindex>();
        T lr_scalar = lr.scalar<T>()();
        const int64_t embedding_dim_size = grad.dim_size(1);
...
class ApplyAdaBeliefOp : public OpKernel {
 public:
  explicit ApplyAdaBeliefOp(OpKernelConstruction* ctx) : OpKernel(ctx) {
    OP_REQUIRES_OK(ctx, ctx->GetAttr("use_locking", &use_exclusive_lock_));
  }
  void Compute(OpKernelContext* ctx) override TF_NO_THREAD_SAFETY_ANALYSIS {
    uint64_t stime = tensorflow::Env::Default()->NowMicros();
...
#define REGISTER_KERNELS(D, T)                                          \
  REGISTER_KERNEL_BUILDER(                                              \
      Name("ApplyAdaBelief").Device(DEVICE_##D).TypeConstraint<T>("T"), \
      ApplyAdaBeliefOp<D##Device, T>);                                  \
  REGISTER_KERNEL_BUILDER(Name("ResourceApplyAdaBelief")                \
                              .HostMemory("var")                        \
                              .HostMemory("m")                          \
                              .HostMemory("v")                          \
                              .Device(DEVICE_##D)                       \
...
#define REGISTER_KERNELS(D, T)                                     \
  REGISTER_KERNEL_BUILDER(                                         \
      Name("ApplyLamb").Device(DEVICE_##D).TypeConstraint<T>("T"), \
      ApplyLambOp<D##Device, T>);                                  \
  REGISTER_KERNEL_BUILDER(Name("ResourceApplyLamb")                \
                              .HostMemory("var")                   \
                              .HostMemory("m")                     \
                              .HostMemory("v")                     \
                              .Device(DEVICE_##D)                  \
...
class ApplyLambHessianOp : public OpKernel {
 public:
  explicit ApplyLambHessianOp(OpKernelConstruction* ctx) : OpKernel(ctx) {
    OP_REQUIRES_OK(ctx, ctx->GetAttr("use_locking", &use_exclusive_lock_));
  }
...
#define REGISTER_KERNELS(D, T)                                            \
  REGISTER_KERNEL_BUILDER(                                                \
      Name("ApplyLambHessian").Device(DEVICE_##D).TypeConstraint<T>("T"), \
      ApplyLambHessianOp<D##Device, T>);                                  \
  REGISTER_KERNEL_BUILDER(Name("ResourceApplyLambHessian")                \
                              .HostMemory("var")                          \
                              .HostMemory("m")                            \
                              .HostMemory("v")                            \
                              .Device(DEVICE_##D)                         \
...
struct ApplyAdaDQH {
  void operator()(const Device& d, typename TTypes<T>::Flat var,
    ...
    var.device(d) -= m_new * alpha /
                     v.sqrt().cwiseMax(
                         epsilon() * Eigen::numext::sqrt(T(1) - beta2_power()));
    m.device(d) += (grad - m) * (T(1) - beta1());
  }
};
 ...
 public:
  ...
  void Compute(OpKernelContext* ctx) override TF_NO_THREAD_SAFETY_ANALYSIS {
    ...
    OP_REQUIRES_OK(ctx,
                   LookupResource(ctx, HandleFromInput(ctx, 0), &table_var));
    core::ScopedUnref unref_me_var(table_var);
    OP_REQUIRES_OK(ctx, LookupResource(ctx, HandleFromInput(ctx, 1), &table_m));
    core::ScopedUnref unref_me_m(table_m);
    ...
    if (N > 0) {
      auto DoWork = [this, ctx, inner_dim, &table_var, &table_m, &table_v,
                     &indices, &grad, &lr, &beta1_power, &beta2_power, &beta1,
                     &beta2, &epsilon](int64_t start_i, int64_t limit_i) {
        const int64_t first_dim_size = indices.dim_size(0);
        const int64_t embedding_dim_size = grad.dim_size(1);
        auto grad_flat = grad.flat_outer_dims<T>();
        auto indices_flat = indices.flat<Tindex>();
...
#define COMPUTE_ADADQH(grad_to_use)                                        \
  ...
  v = beta2_scalar * v + (static_cast<T>(1) - beta2_scalar) * h.square();  \
  var -= m_new * alpha /                                                   \
         v.sqrt().cwiseMax(                                                \
             epsilon_scalar *                                              \
             Eigen::numext::sqrt(static_cast<T>(1) - beta2_power_scalar)); \
    ...
    if (N > 0) {
      ...
      for (Tindex i = 0; i < N; i++) {
        ...
        const T alpha = lr_scalar *
                        Eigen::numext::sqrt(T(1) - beta2_power_scalar) /
...
#define REGISTER_KERNELS(T, Tindices)                                   \
  REGISTER_KERNEL_BUILDER(Name("SparseApplyAdaDQH")                     \
                              .Device(DEVICE_CPU)                       \
                              .TypeConstraint<T>("T")                   \
                              .TypeConstraint<Tindices>("Tindices"),    \
                          SparseApplyAdaDQHOp<CPUDevice, T, Tindices>); \
  REGISTER_KERNEL_BUILDER(Name("ResourceSparseApplyAdaDQH")             \
                              .Device(DEVICE_CPU)                       \
                              .TypeConstraint<T>("T")                   \
                              .TypeConstraint<Tindices>("Tindices"),    \
                          SparseApplyAdaDQHOp<CPUDevice, T, Tindices>);
 ...
 public:
  ...
  void Compute(OpKernelContext* ctx) override TF_NO_THREAD_SAFETY_ANALYSIS {
    ...
    const T epsilon_adjust =
        epsilon_scalar *
        Eigen::numext::sqrt(static_cast<T>(1) - beta2_power_scalar);
    const T last_epsilon_adjust =
        epsilon_scalar * Eigen::numext::sqrt(static_cast<T>(1) -
                                             beta2_power_scalar / beta2_scalar);
    auto l21_norm = l21_scalar * Eigen::numext::sqrt(static_cast<T>(inner_dim));

    if (N > 0) {
      auto DoWork = [this, ctx, &table_var, &table_m, &table_v, &table_linear,
                     &indices, &grad, &beta1_power_scalar, &beta2_power_scalar,
                     &beta1_scalar, &beta2_scalar, &epsilon_adjust,
                     &last_epsilon_adjust, &l1_scalar, &l2_scalar, &l21_norm,
                     &alpha, first_dim_size,
...
#define REGISTER_KERNELS(T, Tindices)                                      \
  REGISTER_KERNEL_BUILDER(Name("SparseApplyAdaBelief")                     \
                              .Device(DEVICE_CPU)                          \
                              .TypeConstraint<T>("T")                      \
                              .TypeConstraint<Tindices>("Tindices"),       \
                          SparseApplyAdaBeliefOp<CPUDevice, T, Tindices>); \
  REGISTER_KERNEL_BUILDER(Name("ResourceSparseApplyAdaBelief")             \
                              .Device(DEVICE_CPU)                          \
                              .TypeConstraint<T>("T")                      \
                              .TypeConstraint<Tindices>("Tindices"),       \
                          SparseApplyAdaBeliefOp<CPUDevice, T, Tindices>);
...
#define REGISTER_KERNELS(T, Tindices)                                       \
  REGISTER_KERNEL_BUILDER(Name("SparseApplyAdaHessian")                     \
                              .Device(DEVICE_CPU)                           \
                              .TypeConstraint<T>("T")                       \
                              .TypeConstraint<Tindices>("Tindices"),        \
                          SparseApplyAdaHessianOp<CPUDevice, T, Tindices>); \
  REGISTER_KERNEL_BUILDER(Name("ResourceSparseApplyAdaHessian")             \
                              .Device(DEVICE_CPU)                           \
                              .TypeConstraint<T>("T")                       \
                              .TypeConstraint<Tindices>("Tindices"),        \
                          SparseApplyAdaHessianOp<CPUDevice, T, Tindices>);
 ...
 public:
  ...
  void Compute(OpKernelContext* ctx) override TF_NO_THREAD_SAFETY_ANALYSIS {
    ...
    if (N > 0) {
      auto DoWork = [this, ctx, inner_dim, &table_var, &table_m, &table_v,
                     &table_linear, &indices, &grad, &lr, &beta1_power,
                     &beta2_power, &beta1, &beta2, &epsilon, &l1, &l2, &l21,
...
#define COMPUTE_GROUPADADQHHG(grad_to_use)                                     \
  ...
  auto deno =                                                                  \
      (y + y.constant(static_cast<T>(2) * l2_scalar * lr_scalar)).square();    \
  auto indicator = (v.constant(epsilon_adjust) >= v.sqrt())                    \
                       .select(v.constant(static_cast<T>(1)),                  \
                               v.constant(static_cast<T>(0)));                 \
  if (l1_linear_norm > l21_norm) {                                             \
    l1_linear_norm = static_cast<T>(1) - l21_norm / l1_linear_norm;            \
    ...
    eps_hg = -lr_scalar *                                                      \
             Eigen::numext::sqrt(static_cast<T>(1) -                           \
                                 beta2_power_scalar / beta2_scalar) /          \
             y * indicator * l1_linear_norm * l1_linear;                       \
  } else {                                                                     \
    lr_hg.setZero();                                                           \
...
#undef REGISTER_KERNELS
  ...
  void operator()(const Device& d, typename TTypes<T>::Flat var,
                  ...
                  typename TTypes<T>::ConstScalar beta1,
                  typename TTypes<T>::ConstScalar beta2,
                  typename TTypes<T>::ConstScalar epsilon, const bool& sam,
                  typename TTypes<T>::Flat delta,
    ...
    const T epsilon_adjust =
        epsilon() * Eigen::numext::sqrt(T(1) - beta2_power() / beta2());

    auto deno = v.sqrt().cwiseMax(epsilon_adjust);
    auto indicator = (v.constant(epsilon_adjust) >= v.sqrt())
                         .select(v.constant(T(1)), v.constant(T(0)));
...
class ComputeAdaDQHHGOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* ctx) override TF_NO_THREAD_SAFETY_ANALYSIS {
    ...
    const Device& device = ctx->template eigen_device<Device>();
    ComputeAdaDQHHG<Device, T>()(
        device, var.flat<T>(), m.flat<T>(), v.flat<T>(), grad.flat<T>(),
        lr.scalar<T>(), beta1_power.scalar<T>(), beta2_power.scalar<T>(),
        beta1.scalar<T>(), beta2.scalar<T>(), epsilon.scalar<T>(),
        sam.scalar<bool>()(), delta.flat<T>(), alpha.scalar<T>(),
        lr_hg->flat<T>(), eps_hg->flat<T>());
...
class KvVariableGroupSparseApplyAdamV4Op : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* ctx) override TF_NO_THREAD_SAFETY_ANALYSIS {
    ...
    if (N > 0) {
      auto DoWork = [this, ctx, &table_var, &table_m_v_linear, &indices, &grad,
                     &beta1_power_scalar, &beta2_power_scalar, &beta1_scalar,
                     &beta2_scalar, &epsilon_scalar, &l1_scalar, &l2_scalar,
                     &alpha, &l21_norm, value_bytes, embedding_dim_size,
                     first_dim_size](int64_t start_i, int64_t limit_i) {
        auto grad_flat = grad.flat_outer_dims<T>();
        auto indices_flat = indices.flat<Tindex>();
        // prepare phstore variables
...
#define COMPUTE_ADAM(grad_to_use)                                             \
  ...
  if (beta1_scalar > beta1_power_scalar) {                                    \
    ...
    linear +=                                                                 \
        alpha * m - (new_v_sqrt + new_v_sqrt.constant(epsilon_scalar)) * var; \
  }                                                                           \
  auto l1_reg_adjust = linear.cwiseMin(l1_scalar).cwiseMax(-l1_scalar);       \
  auto l1_linear = l1_reg_adjust - linear;                                    \
  using TensorScalar = Eigen::Tensor<T, 0, Eigen::RowMajor>;                  \
