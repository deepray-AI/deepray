...
Status WriteTensor(const Tensor& val, tsl::BufferedWritableFileEV* out,
...
// REQUIRES: val.dtype() == DT_STRING
Status WriteStringTensor(const Tensor& val, tsl::BufferedWritableFileEV* out,
...
Status WriteVariantTensor(const Tensor& val, tsl::BufferedWritableFileEV* out,
                          size_t* bytes_written, uint32* crc32c) {
  // On-disk format:
  //   [varint64 len1][bytes variant1][4 byte checksum]
  //   ..
...
// new size.
Status PadAlignment(tsl::BufferedWritableFileEV* out, int alignment,
                    int64_t* size) {
...
BundleWriter::BundleWriter(Env* env, StringPiece prefix, const Options& options)
  ...
  if (!status_.ok()) return;
  out_ = std::make_unique<tsl::BufferedWritableFileEV>(
