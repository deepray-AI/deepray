...
#ifndef XLA_TSL_LIB_IO_BUFFERED_FILE_EV_H_
#define XLA_TSL_LIB_IO_BUFFERED_FILE_EV_H_
...
namespace tsl {
class BufferedWritableFileEV : public WritableFile {
 public:
  explicit BufferedWritableFileEV(std::unique_ptr<WritableFile> file,
                                  int64_t buffer_size = kDefaultBufferSize)
      : file_(std::move(file)) {
    buffer_.resize(buffer_size);
  }
  ~BufferedWritableFileEV() override { Close().IgnoreError(); }
  ...
  absl::Status AppendSegment(StringPiece data) {
    TF_RETURN_IF_ERROR(Flush());
    memcpy(&buffer_[0], data.data(), data.size());
    crc32_ = crc32c::Extend(crc32_, &buffer_[0], data.size());
 ...
 private:
  ...
  BufferedWritableFileEV(const BufferedWritableFileEV&) = delete;
  void operator=(const BufferedWritableFileEV&) = delete;
};

}  // namespace tsl
#endif  // XLA_TSL_LIB_IO_BUFFERED_FILE_EV_H_
