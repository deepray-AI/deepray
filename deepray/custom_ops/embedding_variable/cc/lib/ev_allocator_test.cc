...
TEST(EVAllocator, TestUseAllocation) {
  auto allocator = ev_allocator();
  CHECK_EQ(allocator->Name(), "ev_allocator");  // 确认返回的是EVAllocator
}
...
//   size_t allocated_size = allocator->BatchAllocateRaw(size, 4, 4, ptrs);
...
//   size_t allocated_size = allocator->BatchAllocateRaw(size, 4, 8, ptrs);
...
//   size_t allocated_size = allocator->BatchAllocateRaw(size, 4, 4096, ptrs);
...
TEST(EVAllocator, TestMultiThreadAllocate40B) {
  ...
  LOG(INFO) << "fragment ratio:" << (double)(real - compare) * 100.0 / compare
            << "%";
}
...
//     size_t total = 0;
//     for (int i = 0; i < loop_size; ++i) {
//       auto alloc_num = allocator->BatchAllocateRaw(batch_size, alignment,
//                                                    allocate_size, ptrs);
//       total += alloc_num;
...
//   double t1 = getResident() * getpagesize() / 1024.0 / 1024.0;
//   size_t real = t1 - t0;
//   size_t compare = (size_t)THREAD_NUM * allocate_size * loop_size *
//   batch_size /
//                    1024.0 / 1024.0;
//   LOG(INFO) << "memory real: " << real << "MB";
//   LOG(INFO) << "memory theoretic: " << compare << "MB";
//   LOG(INFO) << "fragment ratio:" << (double)(real - compare) * 100.0 /
//   compare
//             << "%";
// }
