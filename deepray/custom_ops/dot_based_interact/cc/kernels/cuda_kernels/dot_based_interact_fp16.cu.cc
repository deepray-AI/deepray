...
#include "dot_based_interact_shared_utils.h"
...
template <uint WARPS_PER_BLOCK, uint THREADBLOCK_SIZE, uint M_BLOCKS,
          uint K_BLOCKS, uint SMEM_STRIDE, uint SMEM_STRIDE_ACC, uint WARP_SIZE,
          uint WARP_SIZE_LOG_2, uint TILE_DIM, uint TILE_DIM_LOG_2,
          bool IS_ALIGNED>
__launch_bounds__(THREADBLOCK_SIZE) __global__
    void dotBasedInteractFwdKernelFP16(
        const __half* __restrict input, __half* __restrict output,
        ...
        uint smem_rows_per_warp,  // how many rows are multiplied in a single
                                  // pass
        uint output_size,  // this is with padding
        uint num_row_steps, uint num_col_steps, uint padding_size) {
  // right now, there are two solutions implemented for padding out the matrices
  // to have dims that are multiples of 16 first, they are both padded out to be
  // multiples: we can just take it and continue second, in the multiplication
  // we inset the second tile so that it overlaps with the first one, then skip
  // the doubled up part when unwinding the resulting matrix
  ...
  if (IS_ALIGNED) {  // TODO use contexpr if when makefile is upgraded to C++17
    for (uint idx = lane_id; idx < (num_cols >> 2); idx += WARP_SIZE) {
      for (int i = 0; i < num_rows; ++i, sample_input += num_cols) {
        ((float2*)(shmem + i * SMEM_STRIDE))[idx] =
            ((float2*)sample_input)[idx];  // todo will this not read OOB for
                                           // last sample?
      }
    }
  } else {
  ...
  for (uint idx = lane_id; idx < (num_cols_after_padding >> 2);
       idx += WARP_SIZE) {
  ...
  wmma::fragment<wmma::accumulator, TILE_DIM, TILE_DIM, TILE_DIM, float>
  ...
  for (int k_step = 0; k_step < num_col_steps; k_step++) {
    wmma::fragment<wmma::matrix_a, TILE_DIM, TILE_DIM, TILE_DIM, half,
                   wmma::row_major>
        a[M_BLOCKS];
    wmma::fragment<wmma::matrix_b, TILE_DIM, TILE_DIM, TILE_DIM, half,
                   wmma::col_major>
        b[M_BLOCKS];
    for (int j = 0; j < M_BLOCKS; j++) {
    ...
    for (int i = 0; i < M_BLOCKS; i++) {
      for (int j = 0; j < M_BLOCKS; j++) {
        wmma::mma_sync(
            acc[i][j], a[i], b[j],
            acc[i][j]);  // todo we can get a speedup by skipping one of the
                         // multiplications - they are redundant
      }
    }
  }
  ...
  for (int i = 0; i < M_BLOCKS; i++) {
    for (int j = 0; j < M_BLOCKS; j++) {
      ...
      wmma::store_matrix_sync(tile_ptr, acc[i][j], SMEM_STRIDE_ACC,
                              wmma::mem_row_major);
    }
  }
  ...
  for (int i = 0; i < num_rows; ++i, ++srcLine) {
    ...
    for (uint idx = lane_id; idx < i; idx += WARP_SIZE) {
      ...
      gmem_interact_output[offset + idx] =
          __float2half(shmem_store[srcLine * SMEM_STRIDE_ACC + idx]);
    }
  }
...
template <uint WARPS_PER_BLOCK, uint THREADBLOCK_SIZE, uint ROW_TILES_PER_STEP,
          uint COL_TILES_PER_STEP, uint WARP_SIZE, uint WARP_SIZE_LOG_2,
          uint TILE_DIM, uint TILE_DIM_LOG_2, bool IS_ALIGNED>
__launch_bounds__(THREADBLOCK_SIZE) __global__
    void dotBasedInteractBwdKernelFP16(
        const __half* __restrict input, const __half* __restrict upstream_grad,
        half __restrict* grad, half __restrict* bottom_mlp_grad,
        uint batch_size, uint num_rows, uint num_cols,
        ...
        uint interaction_ugrad_2D_size_elems, uint interaction_ugrad_2D_stride,
        uint input_size_elems, uint input_stride, uint num_row_steps,
        uint num_col_steps, uint row_tiles_per_step,
  ...
  uint gmem_ugrad_sample_offset =
      sample_id * (num_cols + interaction_ugrad_size_with_padding);
  ...
  if (IS_ALIGNED) {
#pragma unroll
    for (uint idx = lane_id; idx < (interaction_ugrad_size >> 3);
         idx += WARP_SIZE) {
      ((float4*)smem_in)[idx] = ((float4*)gmem_ugrad_interactions)[idx];
    }
    uint offset = (interaction_ugrad_size >> 3) << 3;
    for (uint idx = lane_id + offset; idx < interaction_ugrad_size;
         idx += WARP_SIZE) {
...
#pragma unroll 2
  ...
  for (int col_step = 0; col_step < num_col_steps; col_step++) {
    for (uint i = 0; i < ROW_TILES_PER_STEP; i++) {
      const half* tile_ptr =
          smem_in + ((i * input_stride + col_step) << TILE_DIM_LOG_2);
    ...
    if (gmem_grad_col < num_cols) {
      for (uint i = 0; i < num_rows; i++) {
        gmem_grad[i * num_cols + gmem_grad_col] =
            __float2half(smem_out[(i << TILE_DIM_LOG_2) + lane_id]);
      }
    }
  }
  if (IS_ALIGNED) {
