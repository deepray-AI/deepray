...
template <uint WARPS_PER_BLOCK, uint THREADBLOCK_SIZE, uint WARP_SIZE,
          uint WARP_SIZE_LOG_2, uint TILE_LENGTH, uint TILE_LENGTH_LOG_2,
          uint TILE_WIDTH, uint TILE_WIDTH_LOG_2, uint ROW_TILES_PER_STEP>
__launch_bounds__(THREADBLOCK_SIZE) __global__
    void dotBasedInteractTF32FwdKernel(
        ...
        uint smem_elems_per_warp, uint output_size, uint num_row_steps,
        uint num_col_steps, uint smem_stride, uint smem_stride_acc,
        uint padding_size) {
  ...
  wmma::fragment<wmma::accumulator, kWmmaM, kWmmaN, kWmmaK, float>
      acc[ROW_TILES_PER_STEP][ROW_TILES_PER_STEP];
  ...
  for (int k_step = 0; k_step < num_col_steps; k_step++) {
    ...
    for (int j = 0; j < ROW_TILES_PER_STEP; j++) {
      int base_row =
          (j < ROW_TILES_PER_STEP - 1) ? j * 16 : num_rows_after_padding - 16;
      const float* tile_ptr =
          shmem + (base_row * smem_stride + k_step * kWmmaK);
  ...
  for (int i = 0; i < num_rows; ++i, ++src_line) {
    ...
    if (lane_id < i) {
      ...
      gmem_interact_output[offset + lane_id] =
          shmem[src_line * smem_stride_acc + lane_id];
    }
  }
...
template <uint WARPS_PER_BLOCK, uint THREADBLOCK_SIZE, uint WARP_SIZE,
          uint WARP_SIZE_LOG_2, uint FRAG_A_ROWS, uint FRAG_B_COLS,
          uint TILE_LENGTH, uint TILE_LENGTH_LOG_2, uint TILE_WIDTH,
          uint TILE_WIDTH_LOG_2>
__launch_bounds__(THREADBLOCK_SIZE) __global__
    void dotBasedInteractTF32BwdKernel(
        const float* __restrict input, const float* __restrict upstream_grad,
        float* __restrict grad, float* __restrict bottom_mlp_grad,
        uint batch_size, uint num_rows, uint num_cols,
        uint num_rows_after_padding, uint num_cols_after_padding,
        ...
        uint input_size_elems, uint input_stride,
        uint shared_mem_per_warp_size_elems, uint num_k_steps,
        uint num_n_steps) {
  // The only support sizes for TF32.
  const uint kWmmaM = 16;
  const uint kWmmaN = 16;
  ...
  float* smem_out = &shared_mem_float[smem_warp_offset + input_size_elems +
...
#pragma unroll
  for (uint idx = lane_id; idx < (interaction_ugrad_size >> 2);
       idx += WARP_SIZE) {
...
#pragma unroll 2
  ...
  for (uint n = 0; n < num_n_steps; n++) {
    ...
    for (uint k = 0; k < num_k_steps; k++) {
      for (uint i = 0; i < FRAG_A_ROWS; i++) {
        const float* mat_a_tile_ptr =
            smem_ugrad +
            (i << TILE_LENGTH_LOG_2) * interaction_ugrad_2D_stride +
            (k << TILE_WIDTH_LOG_2);
        wmma::load_matrix_sync(a[i], mat_a_tile_ptr,
                               interaction_ugrad_2D_stride);
      }
      for (uint j = 0; j < FRAG_B_COLS; j++) {
        const float* mat_b_tile_ptr = smem_in +
                                      (k << TILE_WIDTH_LOG_2) * input_stride +
                                      ((2 * n + j) << TILE_LENGTH_LOG_2);
    ...
    for (uint i = 0; i < FRAG_A_ROWS; i++) {
      for (uint j = 0; j < FRAG_B_COLS; j++) {
        float* out_tile_ptr = smem_out + (i << TILE_LENGTH_LOG_2) * out_stride +
                              (j << TILE_LENGTH_LOG_2);
        wmma::store_matrix_sync(out_tile_ptr, acc[i][j], out_stride,
                                wmma::mem_row_major);
      }
    }
