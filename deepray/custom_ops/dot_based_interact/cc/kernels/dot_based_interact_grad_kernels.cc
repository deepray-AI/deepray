...
struct DotBasedInteractGradFunctor {
  void operator()(const Device& d, const T* input, const T* upstream_grad,
                  T* grad, T* bottom_mlp_grad, int64 batch_size, int64 num_rows,
                  int64 num_cols);
};

template <>
struct DotBasedInteractGradFunctor<GPUDevice, float> {
  void operator()(const GPUDevice& d, const float* input,
                  const float* upstream_grad, float* grad,
                  float* bottom_mlp_grad, int64 batch_size, int64 num_rows,
                  int64 num_cols) {
    int major = d.majorDeviceVersion();
    if (major >= 8) {
      dotBasedInteractAmpereTF32Bwd(input, upstream_grad, grad, bottom_mlp_grad,
                                    batch_size, num_rows, num_cols, d.stream());
    } else if (major == 7) {
      dotBasedInteractVoltaF32Bwd(input, upstream_grad, grad, bottom_mlp_grad,
                                  batch_size, num_rows, num_cols, d.stream());
    }
  }
};
...
struct DotBasedInteractGradFunctor<GPUDevice, half> {
  void operator()(const GPUDevice& d, const half* input,
                  const half* upstream_grad, half* grad, half* bottom_mlp_grad,
                  int64 batch_size, int64 num_rows, int64 num_cols) {
    int major = d.majorDeviceVersion();
    if (major >= 8) {
      dotBasedInteractAmpereF16Bwd(input, upstream_grad, grad, bottom_mlp_grad,
                                   batch_size, num_rows, num_cols, d.stream());
    } else if (major == 7) {
      dotBasedInteractVoltaF16Bwd(input, upstream_grad, grad, bottom_mlp_grad,
                                  batch_size, num_rows, num_cols, d.stream());
    }
  }
};
...
class DotBasedInteractGradOp : public OpKernel {
 public:
  explicit DotBasedInteractGradOp(OpKernelConstruction* context)
      : OpKernel(context) {}
  ...
  void Compute(OpKernelContext* context) override {
    ...
    OP_REQUIRES_OK(context,
                   context->allocate_output(0, input_shape, &grad_tensor));
    ...
    OP_REQUIRES(
        context, device.majorDeviceVersion() >= 7,
        errors::InvalidArgument("GPU not supported (need Volta or higher)"));

    DotBasedInteractGradFunctor<Device, T>()(
        device, input_tensor.flat<T>().data(),
        upstream_grad_tensor.flat<T>().data(), grad_tensor->flat<T>().data(),
        bottom_mlp_grad_tensor->flat<T>().data(), batch_size, num_rows,
        num_cols);
  }
};
