...
struct DotBasedInteractFunctor<GPUDevice, float> {
  void operator()(const GPUDevice& d, const float* input,
                  const float* bottom_mlp_output, float* output,
                  int64 batch_size, int64 num_rows, int64 num_cols) {
    int major = d.majorDeviceVersion();
    if (major >= 8) {
      dotBasedInteractAmpereTF32Fwd(input, bottom_mlp_output, output,
                                    batch_size, num_rows, num_cols, d.stream());
    } else if (major == 7) {
      dotBasedInteractVoltaF32Fwd(input, bottom_mlp_output, output, batch_size,
                                  num_rows, num_cols, d.stream());
    }
  }
};
...
struct DotBasedInteractFunctor<GPUDevice, half> {
  void operator()(const GPUDevice& d, const half* input,
                  const half* bottom_mlp_output, half* output, int64 batch_size,
                  int64 num_rows, int64 num_cols) {
    int major = d.majorDeviceVersion();
    if (major >= 8) {
      dotBasedInteractAmpereF16Fwd(input, bottom_mlp_output, output, batch_size,
                                   num_rows, num_cols, d.stream());
    } else if (major == 7) {
      dotBasedInteractVoltaF16Fwd(input, bottom_mlp_output, output, batch_size,
                                  num_rows, num_cols, d.stream());
    }
  }
};
...
class DotBasedInteractOp : public OpKernel {
 public:
  explicit DotBasedInteractOp(OpKernelConstruction* context)
      : OpKernel(context) {}
  ...
  void Compute(OpKernelContext* context) override {
    ...
    DotBasedInteractFunctor<Device, T>()(
        ...
        output_tensor->flat<T>().data(), batch_size, num_rows, num_cols);
  }
};
