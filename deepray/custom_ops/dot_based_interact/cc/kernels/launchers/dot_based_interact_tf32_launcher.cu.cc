...
#include "../cuda_kernels/dot_based_interact_tf32.cu.cc"
#include "../launchers/dot_based_interact_fp32_launcher.cu.cc"
...
inline void dotBasedInteractTF32Fwd(const void* input,
  ...
  const uint kSkewFloat = 4;  // Ensures we are 16 byte align as required by
  ...
  if (float4_predicate) {
    dotBasedInteractTF32FwdKernel<kWarpsPerBlock, kThreadBlockSize, kWarpSize,
                                  kWarpSizeLog2, kTileLength, kTileLengthLog2,
                                  kTileWidth, kTileWidthLog2, kRowTilesPerStep>
        <<<(batch_size + kWarpsPerBlock - 1) / kWarpsPerBlock, kThreadBlockSize,
           kWarpsPerBlock * smem_elems_per_warp * sizeof(float), stream>>>(
            (const float*)input, (float*)output, batch_size, num_rows, num_cols,
            num_rows_after_padding, num_cols_after_padding, smem_elems_per_warp,
            output_size, num_row_steps, num_col_steps, smem_stride,
            smem_stride_acc, padding_size);
  } else {
    // GENERIC VERSION IS UNFINISHED: Use FP32 instead for now
    dotBasedInteractFP32Fwd(input, bottom_mlp_output, output, batch_size,
...
inline void dotBasedInteractTF32Bwd(const void* input,
  ...
  uint interaction_ugrad_size_with_padding =
      ((interaction_ugrad_size - 1) / 8 + 1) * 8;
  ...
  uint shared_mem_per_warp_size_elems =
      interaction_ugrad_2D_size_elems + input_size_elems + output_size_elems;
  ...
  if (float4_predicate) {
    dotBasedInteractTF32BwdKernel<
        kWarpsPerBlock, kNumThreads, kWarpSize, kWarpSizeLog2, kFragARows,
        kFragBCols, kTileLength, kTileLengthLog2, kTileWidth, kTileWidthLog2>
        <<<num_blocks, kNumThreads, shared_mem_size_bytes, stream>>>(
            (const float*)input, (const float*)upstream_grad, (float*)grad,
            (float*)bottom_mlp_grad, batch_size, num_rows, num_cols,
            ...
            input_size_elems, input_stride, shared_mem_per_warp_size_elems,
            num_k_steps, num_n_steps);
  } else {
    // GENERIC VERSION IS UNFINISHED: Use FP32 instead for now
    dotBasedInteractFP32Bwd(input, upstream_grad, grad, bottom_mlp_grad,
                            batch_size, num_rows, num_cols, stream);
  }
}
