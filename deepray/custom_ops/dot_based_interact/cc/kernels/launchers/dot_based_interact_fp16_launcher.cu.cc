...
#include "../cuda_kernels/dot_based_interact_fp16.cu.cc"

inline void dotBasedInteractFP16Fwd(const void* input,
                                    const void* bottom_mlp_output, void* output,
                                    uint batch_size, uint num_rows,
                                    uint num_cols, cudaStream_t stream) {
  ...
  uint num_rows_after_padding =
      kTileDim << 1;  // TODO here we assume that kTileDim*2 >= num_rows.
  ...
  const uint K_BLOCKS =
      8;  // todo what is this? is this 128//16, ie num_col_tiles?
  const uint M_BLOCKS =
      2;  // todo what is this? is this 32//16, ie num_row_tiles?
  const uint SKEW_HALF = ((K_BLOCKS % 2) == 0) ? 8 : 0;
  const uint SMEM_STRIDE =
      (K_BLOCKS * 16 + SKEW_HALF);  // num_cols padded for tensor cores, then
                                    // padded again for alignment
  // multiple of 2 to guarantee 256-bit alignment for start of the row, at least
  // 16 to safeload a tile
  const uint smem_rows_per_warp =
      M_BLOCKS << 4;  // is this m_blocks * tile dim? how many rows we multiply
                      // in a single pass
  ...
  uint output_size =
      ((raw_output_size - 1) / 8 + 1) * 8;  // round up to multiple of 8
  ...
  if (float4_predicate) {
    // aligned
    dotBasedInteractFwdKernelFP16<warps_per_threadblock, threadblock_size,
                                  M_BLOCKS, K_BLOCKS, SMEM_STRIDE,
                                  SMEM_STRIDE_ACC, kWarpSize, kWarpSizeLog2,
                                  kTileDim, kTileDimLog2, true>
        <<<(batch_size + warps_per_threadblock - 1) / warps_per_threadblock,
           threadblock_size,
           warps_per_threadblock * smem_elems_per_warp * sizeof(__half),
           stream>>>((const __half*)input, (half*)output, batch_size, num_rows,
                     num_cols, num_rows_after_padding, num_cols_after_padding,
                     smem_elems_per_warp, smem_rows_per_warp, output_size,
                     num_row_steps, num_col_steps, padding_size);
  } else {
    // not aligned
    dotBasedInteractFwdKernelFP16<warps_per_threadblock, threadblock_size,
                                  M_BLOCKS, K_BLOCKS, SMEM_STRIDE,
                                  SMEM_STRIDE_ACC, kWarpSize, kWarpSizeLog2,
                                  kTileDim, kTileDimLog2, false>
        <<<(batch_size + warps_per_threadblock - 1) / warps_per_threadblock,
           threadblock_size,
           warps_per_threadblock * smem_elems_per_warp * sizeof(__half),
           stream>>>((const __half*)input, (half*)output, batch_size, num_rows,
...
inline void dotBasedInteractFP16Bwd(const void* input,
                                    const void* upstream_grad, void* grad,
                                    void* bottom_mlp_grad, uint batch_size,
  ...
  uint interaction_ugrad_2D_size_bytes =
      interaction_ugrad_2D_size_elems * sizeof(half);

  // 1D ugrad size
  uint interaction_ugrad_size = num_rows * (num_rows - 1) >> 1;
  uint interaction_ugrad_size_with_padding =
      ((interaction_ugrad_size - 1) / 8 + 1) * 8;  // round up to multiple of 8
  ...
  uint output_size_elems =
      kTileDim * kTileDim * kRowTilesPerStep * kColTilesPerStep;
  ...
  uint shared_mem_per_warp_size_byte =
      input_size_bytes + staging_area_size_bytes;
  ...
  if (float4_predicate) {
    // aligned
    dotBasedInteractBwdKernelFP16<kWarpsPerBlock, kNumThreads, kRowTilesPerStep,
                                  kColTilesPerStep, kWarpSize, kWarpSizeLog2,
                                  kTileDim, kTileDimLog2, true>
        <<<num_blocks, kNumThreads, shared_mem_size_bytes, stream>>>(
            (const half*)input, (const half*)upstream_grad, (half*)grad,
            (half*)bottom_mlp_grad, batch_size, num_rows, num_cols,
            num_rows_after_padding, num_cols_after_padding, sample_size,
            interaction_ugrad_size, interaction_ugrad_size_with_padding,
            interaction_ugrad_2D_size_elems, interaction_ugrad_2D_stride,
            input_size_elems, input_stride, num_row_steps, num_col_steps,
