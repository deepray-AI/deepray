...
void set_params_fprop(FMHA_fprop_params& params,  // NOLINT
  ...
  params.q_ptr =
      reinterpret_cast<void*>(const_cast<T*>(q->tensor<T, 3>().data()));
  params.k_ptr =
      reinterpret_cast<void*>(const_cast<T*>(k->tensor<T, 3>().data()));
  params.v_ptr =
      reinterpret_cast<void*>(const_cast<T*>(v->tensor<T, 3>().data()));
