...
from deepray.custom_ops.distributed_embeddings.python.layers import dist_model_parallel as dmp
...
class TestHelperMixedin():
  ...
  def gen_inputs(self, table_sizes, input_to_table_map=None, mp_input_ids=None):
    ...
    global_inputs = [
        tf.random.uniform(shape=[self.global_batch], minval=0, maxval=table_sizes[i][0], dtype=tf.int64)
        for i in input_to_table_map
    ]
    for t in global_inputs:
      hvd.broadcast(t, root_rank=0)
    local_batch = self.global_batch // self.hvd_size
    dp_inputs = [t[self.hvd_rank * local_batch:(self.hvd_rank + 1) * local_batch] for t in global_inputs]
