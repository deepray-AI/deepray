...
class ParquetTabularDatasetOp::Dataset : public DatasetBase {
 public:
  Dataset(OpKernelContext* ctx, const string& filename, const int64 batch_size,
    ...
    size_t num_outputs = field_names.size();
...
ParquetTabularDatasetOp::ParquetTabularDatasetOp(OpKernelConstruction* ctx)
  ...
  OP_REQUIRES_OK(ctx, ctx->GetAttr("field_ragged_ranks", &field_ragged_ranks_));
...
REGISTER_KERNEL_BUILDER(Name("ParquetTabularDatasetV1").Device(DEVICE_CPU),
                        ParquetTabularDatasetOp);

ALLOW_STATEFUL_OP_FOR_DATASET_FUNCTIONS("ParquetTabularDatasetV1");
