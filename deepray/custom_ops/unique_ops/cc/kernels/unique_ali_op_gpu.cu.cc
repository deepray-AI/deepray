...
struct SegmentIndicatorFunctor {
  ...
  SegmentIndicatorFunctor(const TKey* sorted_input_ptr)
...
template <typename T, typename TIndex>
void MoveSparseValues(const GPUDevice& d, const TIndex* keys,
                      const TIndex* idxs, const T* values, const int64 size,
                      T* out) {
  GpuLaunchConfig config = GetGpuLaunchConfig(size, d);
...
void CompareAdjacent(const GPUDevice& d, const T* in, const int64 size,
                     TIndex* out) {
  GpuLaunchConfig config = GetGpuLaunchConfig(size, d);
  CompareAdjacentKernel<T>
      <<<config.block_count, config.thread_per_block, 0, d.stream()>>>(in, size,
                                                                       out);
}
template <typename T, typename TIndex>
 ...
 public:
  ...
  void ComputeAsync(OpKernelContext* ctx, DoneCallback done) override {
    ...
    OP_REQUIRES_OK_ASYNC(
        ctx,
        ctx->allocate_temp(DataTypeToEnum<T>::value, {N}, &keys_sort_tensor),
        done);
    OP_REQUIRES_OK_ASYNC(ctx,
                         ctx->allocate_temp(DataTypeToEnum<TIndex>::value, {N},
...

    {
      ...
      cub::DeviceScan::InclusiveSum(NULL, temp_storage_bytes,
                                    segment_indicator_iter, output_indices, N,
                                    cu_stream);
      OP_REQUIRES_OK_ASYNC(
          ctx,
          ctx->allocate_temp(
    ...
    OP_REQUIRES_ASYNC(ctx, stream, errors::Internal("No GPU stream available."),
                      done);
