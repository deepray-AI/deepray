...
def _gather_tree(step_ids, parent_ids, max_sequence_lengths, end_token):
  ...
  mask = tf.expand_dims(tf.transpose(tf.sequence_mask(max_sequence_lengths, maxlen=max_time)), -1)
  ...
  with tf.control_dependencies([assert_op]):
    rev_step_ids = tf.reverse_sequence(step_ids, max_sequence_lengths, seq_axis=0, batch_axis=1)
    rev_parent_ids = tf.reverse_sequence(parent_ids, max_sequence_lengths, seq_axis=0, batch_axis=1)
  ...
  output_ids = tf.reverse_sequence(output_ids, max_sequence_lengths, seq_axis=0, batch_axis=1)
...
def gather_tree_from_array(t: TensorLike, parent_ids: TensorLike, sequence_length: TensorLike) -> tf.Tensor:
  ...
  in_bound_steps = tf.transpose(tf.sequence_mask(sequence_length, maxlen=max_time), perm=[2, 0, 1])
...
def _check_ndims(t):
  if t.shape.ndims is None:
    raise ValueError("Expected tensor (%s) to have known rank, but ndims == None." % t)
...
class BeamSearchDecoderMixin:
  ...
  def _maybe_sort_array_beams(self, t, parent_ids, sequence_length):
    ...
    if not _check_static_batch_beam_maybe(t.element_shape, tf.get_static_value(self._batch_size), self._beam_width):
      return t
    t = t.stack()
    with tf.control_dependencies([_check_batch_beam(t, self._batch_size, self._beam_width)]):
  ...
  def step(self, time, inputs, state, training=None, name=None):
    ...
    with tf.name_scope(name or "BeamSearchDecoderStep"):
      ...
      inputs = tf.nest.map_structure(lambda inp: self._merge_batch_beams(inp, s=inp.shape[2:]), inputs)
      cell_state = tf.nest.map_structure(self._maybe_merge_batch_beams, cell_state, self._cell.state_size)
      cell_outputs, next_cell_state = self._cell(inputs, cell_state, training=training)
      cell_outputs = tf.nest.map_structure(lambda out: self._split_batch_beams(out, out.shape[1:]), cell_outputs)
      next_cell_state = tf.nest.pack_sequence_as(cell_state, tf.nest.flatten(next_cell_state))
      next_cell_state = tf.nest.map_structure(self._maybe_split_batch_beams, next_cell_state, self._cell.state_size)
...
def _get_scores(
  ...
  coverage_penalty = tf.reduce_sum(tf.math.log(tf.minimum(accumulated_attention_probs, 1.0)), 2)
