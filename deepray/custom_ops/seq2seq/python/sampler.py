...
class TrainingSampler(Sampler):
  ...
  def initialize(self, inputs, sequence_length=None, mask=None):
    ...
    if sequence_length is not None and mask is not None:
      raise ValueError("sequence_length and mask can't be provided at the same time.")
    if sequence_length is not None:
      ...
      with tf.control_dependencies([_check_sequence_is_right_padded(mask, self.time_major)]):
        self.sequence_length = tf.math.reduce_sum(tf.cast(mask, tf.int32), axis=axis, name="sequence_length")
    else:
      ...
      self.sequence_length = tf.fill([self.batch_size], max_seq_len, name="sequence_length")

    self.zero_inputs = tf.nest.map_structure(lambda inp: tf.zeros_like(inp[0, :]), inputs)
...
class ScheduledOutputTrainingSampler(TrainingSampler):
  ...
  def initialize(self, inputs, sequence_length=None, mask=None, auxiliary_inputs=None):
    if auxiliary_inputs is None:
      maybe_concatenated_inputs = inputs
    else:
      inputs = tf.convert_to_tensor(inputs)
      auxiliary_inputs = tf.convert_to_tensor(auxiliary_inputs)
      maybe_concatenated_inputs = tf.nest.map_structure(lambda x, y: tf.concat((x, y), -1), inputs, auxiliary_inputs)
      if not self.time_major:
        auxiliary_inputs = tf.nest.map_structure(_transpose_batch_time, auxiliary_inputs)
    if auxiliary_inputs is not None:
      self._auxiliary_input_tas = tf.nest.map_structure(_unstack_ta, auxiliary_inputs)
    else:
      self._auxiliary_input_tas = None

    return super().initialize(maybe_concatenated_inputs, sequence_length=sequence_length, mask=mask)
  ...
  def next_inputs(self, time, outputs, state, sample_ids):
    ...
    def maybe_sample():
      ...
      def maybe_concatenate_auxiliary_inputs(outputs_, indices=None):
        ...
        next_time = time + 1
        auxiliary_inputs = tf.nest.map_structure(lambda ta: ta.read(next_time), self._auxiliary_input_tas)
        if indices is not None:
          auxiliary_inputs = tf.gather_nd(auxiliary_inputs, indices)
        return tf.nest.map_structure(lambda x, y: tf.concat((x, y), -1), outputs_, auxiliary_inputs)
      ...
      inputs_not_sampling = tf.gather_nd(base_next_inputs, where_not_sampling)
      sampled_next_inputs = maybe_concatenate_auxiliary_inputs(self.next_inputs_fn(outputs_sampling), where_sampling)
...
class GreedyEmbeddingSampler(Sampler):
  ...
  def initialize(self, embedding, start_tokens=None, end_token=None):
    ...
    self.start_tokens = tf.convert_to_tensor(start_tokens, dtype=tf.int32, name="start_tokens")
    self.end_token = tf.convert_to_tensor(end_token, dtype=tf.int32, name="end_token")
...
def bernoulli_sample(probs=None, logits=None, dtype=tf.int32, sample_shape=(), seed=None):
...
def _unstack_ta(inp):
  return tf.TensorArray(dtype=inp.dtype, size=tf.shape(inp)[0], element_shape=inp.shape[1:]).unstack(inp)
...
def _check_sequence_is_right_padded(mask, time_major):
  ...
  right_padded_mask = tf.sequence_mask(sequence_length, maxlen=max_seq_length, dtype=tf.bool)
