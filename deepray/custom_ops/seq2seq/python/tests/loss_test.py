...
def get_test_data():
  ...
  sequence_length = 3
  number_of_classes = 5
  logits = [tf.constant(i + 0.5, shape=[batch_size, number_of_classes]) for i in range(sequence_length)]
  logits = tf.stack(logits, axis=1)
  targets = [tf.constant(i, tf.int32, shape=[batch_size]) for i in range(sequence_length)]
...
def test_keras_compatibility():
  ...
  def return_logits(x):
    ...
    logits_batch = tf.tile(tf.expand_dims(logits_single_row, 0), [tf.shape(x)[0], 1, 1])
    return logits_batch

  inp = tf.keras.layers.Input(shape=(sequence_length,))
  out = tf.keras.layers.Lambda(return_logits, output_shape=(sequence_length, number_of_classes))(inp)
  ...
  h = model.fit(x, targets, sample_weight=weights, batch_size=batch_size, steps_per_epoch=1)
