...
def test_step_with_training_helper_output_layer(cell_class, use_output_layer):
  ...
  np.testing.assert_equal(np.asanyarray([False, False, False, False, True]), first_finished)
  np.testing.assert_equal(np.asanyarray([False, False, False, True, True]), step_finished)
  assert output_dtype.sample_id == step_outputs.sample_id.dtype
  np.testing.assert_equal(np.argmax(step_outputs.rnn_output, -1), step_outputs.sample_id)
...
def test_step_with_training_helper_masked_input(use_mask):
  ...
  if use_mask is None:
    (first_finished, first_inputs, first_state) = my_decoder.initialize(input_t, initial_state=initial_state)
  elif use_mask:
...
def test_step_with_greedy_embedding_helper():
  ...
  np.testing.assert_equal(np.asanyarray([False, False, False, False, False]), first_finished)
...
def test_step_with_scheduled_embedding_training_helper():
  ...
  cell = tf.keras.layers.LSTMCell(vocabulary_size)
  sampler = sampler_py.ScheduledEmbeddingTrainingSampler(sampling_probability=half, time_major=False)
  initial_state = cell.get_initial_state(batch_size=batch_size, dtype=tf.float32)
  my_decoder = basic_decoder.BasicDecoder(cell=cell, sampler=sampler)
  (first_finished, first_inputs, first_state) = my_decoder.initialize(
      input_t,
...
def test_step_with_scheduled_output_training_helper(sampling_probability, use_next_inputs_fn, use_auxiliary_inputs):
  sequence_length = [3, 4, 3, 1, 0]
  batch_size = 5
  max_time = 8
  input_depth = 7
  ...
  if use_auxiliary_inputs:
    ...
    auxiliary_inputs = np.random.randn(batch_size, max_time, auxiliary_input_depth).astype(np.float32)
  else:
...
def test_step_with_inference_helper_categorical():
  ...
  start_inputs = tf.one_hot(np.ones(batch_size, dtype=np.int32) * start_token, vocabulary_size)
  ...
  initial_state = cell.get_initial_state(batch_size=batch_size, dtype=tf.float32)
  my_decoder = basic_decoder.BasicDecoder(cell=cell, sampler=sampler)
  (first_finished, first_inputs, first_state) = my_decoder.initialize(start_inputs, initial_state=initial_state)
