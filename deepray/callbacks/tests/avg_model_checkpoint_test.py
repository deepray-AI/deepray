...
from deepray.callbacks import AverageModelCheckpoint
from deepray.optimizers import MovingAverage
...
def get_data_and_model(optimizer="moving_avg"):
  ...
  hidden_layer = keras.layers.Dense(NUM_HIDDEN, input_dim=INPUT_DIM, activation="relu")(inputs)
...
def test_invalid_save_freq(tmp_path):
  ...
  with pytest.raises(ValueError, match="Unrecognized save_freq"):
    AverageModelCheckpoint(update_weights=True, filepath=test_model_filepath, save_freq=save_freq)
