...
from deepray.losses import sparsemax_loss, SparsemaxLoss
...
def test_sparsemax_loss_of_inf(dtype):
  ...
  np.testing.assert_equal(np.asanyarray([np.nan, np.nan, np.nan, np.nan]), tf_loss_pos)

  _, tf_loss_mix = _tf_sparsemax_loss(z_mix, q, dtype)
  np.testing.assert_equal(np.asanyarray([np.nan, np.nan, np.nan, np.nan]), tf_loss_mix)
...
def test_constant_add(dtype):
  ...
  test_utils.assert_allclose_according_to_type(tf_loss_zpc, tf_loss_z, float_atol=5e-6, float_rtol=5e-6)
