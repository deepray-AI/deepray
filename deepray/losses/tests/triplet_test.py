...
from deepray.losses import triplet
...
def test_keras_model_compile_semihard():
  model = tf.keras.models.Sequential([tf.keras.layers.Input(shape=(784,)), tf.keras.layers.Dense(10)])
  model.compile(loss="Deepray>triplet_semihard_loss", optimizer="adam")
...
def test_keras_model_compile_hard():
  model = tf.keras.models.Sequential([tf.keras.layers.Input(shape=(784,)), tf.keras.layers.Dense(10)])
  model.compile(loss="Deepray>triplet_hard_loss", optimizer="adam")
