...
from deepray.image import distort_image_ops
...
def _adjust_hue_in_yiq_np(x_np, delta_h):
  ...
  tyiq = np.array([[0.299, 0.587, 0.114], [0.596, -0.274, -0.322], [0.211, -0.523, 0.312]]).astype(x_v.dtype)
  ...
  hue_rotation = np.array([[1.0, 0.0, 0.0], [0.0, u, -w], [0.0, w, u]]).astype(x_v.dtype)
...
@pytest.mark.parametrize("shape", ([2, 2, 3], [4, 2, 3], [2, 4, 3], [2, 5, 3], [1000, 1, 3]))
@pytest.mark.parametrize("style", ("all_random", "rg_same", "rb_same", "gb_same", "rgb_same"))
...
def test_adjust_random_hue_in_yiq(shape, style, dtype):
  ...
  test_utils.assert_allclose_according_to_type(y_tf, y_np, atol=1e-4, rtol=2e-4, half_rtol=0.8)
...
def test_invalid_rank_hsv():
  ...
  with pytest.raises((tf.errors.InvalidArgumentError, ValueError), match="input must be at least 3-D"):
...
def test_adjust_hsv_in_yiq_unknown_shape():
  fn = tf.function(distort_image_ops.adjust_hsv_in_yiq
                  ).get_concrete_function(tf.TensorSpec(shape=None, dtype=tf.float64))
  for shape in (2, 3, 3), (4, 2, 3, 3):
    image_np = np.random.rand(*shape) * 255.0
    image_tf = tf.constant(image_np)
    np.testing.assert_allclose(_adjust_hue_in_yiq_np(image_np, 0), fn(image_tf), rtol=2e-4, atol=1e-4)


def test_random_hsv_in_yiq_unknown_shape():
  fn = tf.function(distort_image_ops.random_hsv_in_yiq
                  ).get_concrete_function(tf.TensorSpec(shape=None, dtype=tf.float32))
  for shape in (2, 3, 3), (4, 2, 3, 3):
...
def test_invalid_rank_value():
  ...
  if tf.executing_eagerly():
    ...
    with pytest.raises(ValueError, match="Shape must be at least rank 3 but is rank 2"):
      _adjust_value_in_yiq_tf(x_np, scale)
...
def test_adjust_random_saturation_in_yiq(shape, style, dtype):
  ...
  test_utils.assert_allclose_according_to_type(y_tf, y_baseline, atol=1e-4, rtol=2e-4, half_rtol=0.8)
