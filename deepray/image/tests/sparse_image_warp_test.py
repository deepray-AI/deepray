...
from deepray.image import sparse_image_warp
from deepray.image.sparse_image_warp import _get_boundary_locations
from deepray.image.sparse_image_warp import _get_grid_locations
from deepray.utils.resource_loader import get_path_to_datafile
...
def assert_zero_shift(order, regularization, num_boundary_points):
  ...
  control_point_locations = tf.constant(np.float32(np.expand_dims(control_point_locations, 0)))
...
def assert_move_single_pixel(order, num_boundary_points, type_to_use):
  ...
  control_point_locations = tf.constant(np.float32(np.expand_dims(control_point_locations, 0)), dtype=type_to_use)
  ...
  np.testing.assert_allclose(warped_image[0, 4, 5, :], input_image[0, 4, 4, :], atol=1e-5, rtol=1e-5)

  # Test that there is no flow at the corners.
  for i in (0, image_height - 1):
    for j in (0, image_width - 1):
      np.testing.assert_allclose(flow[0, i, j, :], np.zeros([2]), atol=1e-5, rtol=1e-5)


def load_image(image_file):
  image = tf.image.decode_png(tf.io.read_file(image_file), dtype=tf.dtypes.uint8, channels=4)[:, :, 0:3]
  return image
...
def test_smiley_face():
  ...
  control_points = tf.constant(np.expand_dims(np.float32(control_points[:, [1, 0]]), 0))
  control_point_displacements = tf.constant(np.expand_dims(np.float32(control_point_displacements[:, [1, 0]]), 0))
...
def test_that_backprop_runs():
  ...
  control_point_displacements = tf.constant(np.float32(np.expand_dims(control_point_displacements, 0)))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ShapeConfig([None, None, None, None], [None, 1, 2], [None, 1, 2], (1, 9, 12, 3)),
    ],
)
@pytest.mark.parametrize("interpolation_order", [1, 2, 3])
@pytest.mark.parametrize("num_boundary_points", [1, 2, 3])
def test_partially_or_fully_unknown_shape(shape, interpolation_order, num_boundary_points):
  control_point_locations = np.asarray([3.0, 3.0]).reshape(1, 1, 2).astype(np.float32)
  control_point_displacements = (np.asarray([0.25, -0.5]).reshape(1, 1, 2).astype(np.float32))
  fn = tf.function(sparse_image_warp).get_concrete_function(
      image=tf.TensorSpec(shape=shape.image, dtype=tf.float32),
      source_control_point_locations=tf.TensorSpec(shape=shape.source_control_point_locations, dtype=tf.float32),
      dest_control_point_locations=tf.TensorSpec(shape=shape.dest_control_point_locations, dtype=tf.float32),
      interpolation_order=interpolation_order,
      num_boundary_points=num_boundary_points,
  )
