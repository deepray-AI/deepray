...
from deepray.image import interpolate_spline
...
class _InterpolationProblem:
  ...
  def get_problem(self, optimizable=False, extrapolate=True, dtype="float32"):
    ...
    init_points = np.random.uniform(size=[batch_size, num_training_points, self.DATA_DIM])

    init_points = init_points.astype(dtype)
    train_points = (tf.Variable(init_points) if optimizable else tf.constant(init_points))
    train_values = self.test_function(train_points)

    query_points_np = np.random.uniform(size=[batch_size, num_query_points, self.DATA_DIM])
    query_points_np = query_points_np.astype(dtype)
    if not extrapolate:
      query_points_np = np.clip(query_points_np, np.min(init_points), np.max(init_points))
...
class _QuadraticPlusSinProblem1D(_InterpolationProblem):
  ...
  def test_function(self, x):
    """Takes a tensor, evaluates the test function, and returns a
        tensor."""
    return tf.reduce_mean(tf.pow((x - 0.5), 3) - 0.25 * x + 10 * tf.sin(x * 10), 2, keepdims=True)
...
class _QuadraticPlusSinProblemND(_InterpolationProblem):
  ...
  def test_function(self, x):
    """Takes a tensor, evaluates the test function, and returns a
        tensor."""
    return tf.reduce_sum(tf.square(x - 0.5) + 0.25 * x + 1 * tf.sin(x * 15), 2, keepdims=True)
...
def test_1d_linear_interpolation():
  ...
  with tf.name_scope("interpolator"):
    interp = interpolate_spline(train_points, train_values, query_points, interpolation_order).numpy()
    ...
    scipy_interp_function = sc_interpolate.interp1d(train_points, train_values, kind="linear")
    ...
    np.testing.assert_allclose(train_values, scipy_interpolation_on_train, atol=tol, rtol=tol)
...
def test_1d_interpolation():
  ...
  for order in (1, 2, 3):
    for reg_weight in (0, 0.01):
      interp = interpolate_spline(train_points, train_values, query_points, order, reg_weight)
