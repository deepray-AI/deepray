...
import numpy as np
from deepray.image.cutout_ops import cutout, random_cutout
from deepray.image.utils import to_4D_image
...
def test_mask_applied():
  ...
  np.testing.assert_equal(np.sum(result_image) + total_expected_masked_count, np.sum(test_image))
