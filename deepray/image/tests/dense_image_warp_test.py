...
from deepray.image import dense_image_warp
from deepray.image import interpolate_bilinear
...
def test_interpolate_small_grid_ij():
  ...
  query_points = tf.constant([[0.0, 0.0], [1.0, 0.0], [2.0, 0.5], [1.5, 1.5], [3.0, 2.0]], shape=[1, 5, 2])
...
def test_interpolate_small_grid_xy():
  ...
  query_points = tf.constant([[0.0, 0.0], [0.0, 1.0], [0.5, 2.0], [1.5, 1.5], [2.0, 3.0]], shape=[1, 5, 2])
  expected_results = np.reshape(np.array([0.0, 3.0, 6.5, 6.0, 11.0]), [1, 5, 1])
...
def test_interpolate_small_grid_batched():
  grid = tf.constant([[[0.0, 1.0], [3.0, 4.0]], [[5.0, 6.0], [7.0, 8.0]]], shape=[2, 2, 2, 1])
  query_points = tf.constant([[[0.0, 0.0], [1.0, 0.0], [0.5, 0.5]], [[0.5, 0.0], [1.0, 0.0], [1.0, 1.0]]])
  expected_results = np.reshape(np.array([[0.0, 3.0, 2.0], [6.0, 7.0, 8.0]]), [2, 3, 1])
...
def test_unknown_shape():
  query_points = tf.constant([[0.0, 0.0], [0.0, 1.0], [0.5, 2.0], [1.5, 1.5]], shape=[1, 4, 2])
...
def _check_interpolation_correctness(shape, image_type, flow_type, call_with_unknown_shapes=False, num_probes=5):
...
def test_size_exception():
  ...
  shape = [1, 2, 1, 1]
  with pytest.raises(tf.errors.InvalidArgumentError, match="Grid width must be at least 2."):
