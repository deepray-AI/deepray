...
from deepray.image import color_ops
...
def test_equalize_with_skimage(bins):
  ...
  assert (
      tf.reduce_mean(tf.math.abs(tf.reshape(tf.math.subtract(equalized, skiequalized), [-1]))) <= 0.015
  ...
  # https://github.com/tensorflow/deepray/pull/2381
...
def test_sharpness_with_PIL(factor):
  ...
  sharpened = np.stack([ImageEnhance.Sharpness(Image.fromarray(i)).enhance(factor) for i in image])
  np.testing.assert_allclose(color_ops.sharpness(tf.constant(image), factor).numpy(), sharpened, atol=1)
