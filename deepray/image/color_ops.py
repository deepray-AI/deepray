...
from deepray.image.utils import to_4D_image, from_4D_image
...
@tf.function
def equalize(image: TensorLike, bins: int = 256, name: Optional[str] = None) -> tf.Tensor:
  """Equalize image(s)
...
def _sharpness_image(image: TensorLike, factor: Number) -> tf.Tensor:
  ...
  degenerate = tf.nn.depthwise_conv2d(image, kernel, strides=[1, 1, 1, 1], padding="VALID", dilations=[1, 1])
