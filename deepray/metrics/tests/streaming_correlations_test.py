...
from deepray.metrics import KendallsTauB
from deepray.metrics import KendallsTauC
from deepray.metrics import PearsonsCorrelation
from deepray.metrics import SpearmansRank
from deepray.testing.serialization import check_metric_serialization
...
class TestStreamingCorrelations:
  ...
  def test_keras_binary_classification_model(self, correlation_type):
    ...
    initial_correlation = self.scipy_corr[correlation_type](model(x)[:, 0], y[:, 0])[0]

    history = model.fit(x, y, epochs=1, verbose=0, batch_size=32, validation_data=(x, y))

    # the training should increase the correlation metric
    metric_history = history.history["val_" + metric.name]
    assert np.all(metric_history[-1] > initial_correlation)
