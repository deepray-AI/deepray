...
class CorrelationBase(keras.metrics.Metric):
  ...
  def __init__(
    ...
    self.m = self.add_weight("m", (self.actual_cutpoints - 1, self.preds_cutpoints - 1), dtype=tf.int64)
  ...
  def update_state(self, y_true, y_pred, sample_weight=None):
    ...
    ncol = tf.tensor_scatter_nd_add(self.ncol, tf.expand_dims(j, axis=-1), tf.ones_like(j))
  ...
  def reset_state(self):
    ...
    self.m.assign(tf.zeros((self.actual_cutpoints - 1, self.preds_cutpoints - 1), tf.int64))
...
class KendallsTauBase(CorrelationBase):
  ...
  def _compute_variables(self):
    ...
    # Ties in x.
    t = (tf.cast(tf.math.reduce_sum(tf.math.square(self.nrow)), tf.float32) - sum_m_squard) / 2.0
    # Ties in y.
    u = (tf.cast(tf.math.reduce_sum(tf.math.square(self.ncol)), tf.float32) - sum_m_squard) / 2.0
    # Ties in both.
...
class PearsonsCorrelation(CorrelationBase):
  ...
  def result(self):
    ...
    col_bins = (self.preds_cuts[1:] - self.preds_cuts[:-1]) / 2.0 + self.preds_cuts[:-1]
    row_bins = (self.actual_cuts[1:] - self.actual_cuts[:-1]) / 2.0 + self.actual_cuts[:-1]
    ...
    col_var = tf.reduce_sum(ncol * tf.square(col_bins)) - n_col * tf.square(col_mean)
    row_var = tf.reduce_sum(nrow * tf.square(row_bins)) - n_row * tf.square(row_mean)
