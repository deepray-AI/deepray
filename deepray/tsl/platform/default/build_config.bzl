...
load("//deepray/tsl/platform:build_config_root.bzl", "if_static")
load(
    "//deepray/tsl:tsl.bzl",
    "clean_dep",
    "if_not_windows",
...
def pyx_library(
    ...
    for src in pyx_srcs:
        ...
        native.cc_binary(
            ...
            srcs = [stem + ".cpp"],
            deps = cc_deps + ["@//third_party/python_runtime:headers"],
            linkshared = 1,
            testonly = testonly,
            copts = copts,
        )
...
def cc_proto_library(
    ...
    if use_grpc_plugin:
        cc_libs += select({
            clean_dep("//deepray/tsl:linux_s390x"): ["//external:grpc_lib_unsecure"],
...
def tf_additional_lib_hdrs():
    return [
        clean_dep("//deepray/tsl/platform/default:casts.h"),
        clean_dep("//deepray/tsl/platform/default:context.h"),
        clean_dep("//deepray/tsl/platform/default:cord.h"),
        clean_dep("//deepray/tsl/platform/default:dynamic_annotations.h"),
        clean_dep("//deepray/tsl/platform/default:integral_types.h"),
        clean_dep("//deepray/tsl/platform/default:logging.h"),
        clean_dep("//deepray/tsl/platform/default:mutex.h"),
        clean_dep("//deepray/tsl/platform/default:mutex_data.h"),
        clean_dep("//deepray/tsl/platform/default:notification.h"),
        clean_dep("//deepray/tsl/platform/default:stacktrace.h"),
        clean_dep("//deepray/tsl/platform/default:status.h"),
        clean_dep("//deepray/tsl/platform/default:tracing_impl.h"),
        clean_dep("//deepray/tsl/platform/default:unbounded_work_queue.h"),
    ] + select({
        clean_dep("//deepray/tsl:windows"): [
            clean_dep("//deepray/tsl/platform/windows:intrinsics_port.h"),
            clean_dep("//deepray/tsl/platform/windows:stacktrace.h"),
            clean_dep("//deepray/tsl/platform/windows:subprocess.h"),
            clean_dep("//deepray/tsl/platform/windows:wide_char.h"),
            clean_dep("//deepray/tsl/platform/windows:windows_file_system.h"),
        ],
        "//conditions:default": [
            clean_dep("//deepray/tsl/platform/default:posix_file_system.h"),
            clean_dep("//deepray/tsl/platform/default:subprocess.h"),
        ],
    })

def tf_additional_all_protos():
    return [clean_dep("//deepray/core:protos_all")]

def tf_protos_all():
    return if_static(
        extra_deps = [
            clean_dep("//deepray/core/protobuf:conv_autotuning_proto_cc_impl"),
            clean_dep("//deepray/core:protos_all_cc_impl"),
            clean_dep("//deepray/tsl/protobuf:autotuning_proto_cc_impl"),
            clean_dep("//deepray/tsl/protobuf:protos_all_cc_impl"),
        ],
        otherwise = [clean_dep("//deepray/core:protos_all_cc")],
    )

def tf_protos_profiler_service():
    return [
        clean_dep("//deepray/tsl/profiler/protobuf:profiler_analysis_proto_cc_impl"),
        clean_dep("//deepray/tsl/profiler/protobuf:profiler_service_proto_cc_impl"),
        clean_dep("//deepray/tsl/profiler/protobuf:profiler_service_monitor_result_proto_cc_impl"),
    ]

def tf_protos_grappler_impl():
    return [clean_dep("//deepray/core/grappler/costs:op_performance_data_cc_impl")]

def tf_protos_grappler():
    return if_static(
        extra_deps = tf_protos_grappler_impl(),
        otherwise = [clean_dep("//deepray/core/grappler/costs:op_performance_data_cc")],
    )
...
def tf_additional_core_deps():
    return select({
        clean_dep("//deepray/tsl:android"): [],
        clean_dep("//deepray/tsl:ios"): [],
        clean_dep("//deepray/tsl:linux_s390x"): [],
        "//conditions:default": [
            clean_dep("//deepray/tsl/platform/cloud:gcs_file_system"),
        ],
    })
...
def tf_lib_proto_parsing_deps():
    return [
        ...
        clean_dep("//deepray/tsl/platform/default/build_config:proto_parsing"),
    ]
    ...
    native.cc_test(
        ...
        deps = deps + if_tsl_link_protobuf(
            ...
            [
                ...
                # granularly
                clean_dep("//deepray/tsl/protobuf:error_codes_proto_impl_cc_impl"),
                clean_dep("//deepray/tsl/protobuf:histogram_proto_cc_impl"),
                clean_dep("//deepray/tsl/protobuf:status_proto_cc_impl"),
                clean_dep("//deepray/tsl/profiler/protobuf:xplane_proto_cc_impl"),
                clean_dep("//deepray/tsl/profiler/protobuf:profiler_options_proto_cc_impl"),
            ],
        ),
        **kwargs
    )
...
def tf_portable_proto_lib():
    return ["//deepray/core:protos_all_cc_impl"]
...
def tf_windows_aware_platform_deps(name):
    return select({
        "//deepray/tsl:windows": [
            "//deepray/tsl/platform/windows:" + name,
        ],
        "//conditions:default": [
            "//deepray/tsl/platform/default:" + name,
        ],
    })

def tf_platform_deps(name, platform_dir = "//deepray/tsl/platform/"):
    return [platform_dir + "default:" + name]

def tf_testing_deps(name, platform_dir = "//deepray/tsl/platform/"):
    return tf_platform_deps(name, platform_dir)

def tf_stream_executor_deps(name, platform_dir = "//deepray/tsl/platform/"):
    return tf_platform_deps(name, platform_dir)

def tf_platform_alias(name, platform_dir = "//deepray/tsl/platform/"):
    return [platform_dir + "default:" + name]

def tf_logging_deps():
    return [clean_dep("//deepray/tsl/platform/default:logging")]

def tf_resource_deps():
    return [clean_dep("//deepray/tsl/platform/default:resource")]
...
def if_llvm_system_z_available(then, otherwise = []):
    return select({
        clean_dep("//deepray/tsl:linux_s390x"): then,
