...
def if_static(extra_deps, otherwise = [], macos = []):
    ret = {
        str(Label("//deepray/tsl:framework_shared_object")): otherwise,
        "//conditions:default": extra_deps,
    }
    if macos:
        ret[str(Label("//deepray/tsl:macos_with_framework_shared_object"))] = macos
    return select(ret)

def if_static_and_not_mobile(extra_deps, otherwise = []):
    return select({
        str(Label("//deepray/tsl:framework_shared_object")): otherwise,
        str(Label("//deepray/tsl:android")): otherwise,
        str(Label("//deepray/tsl:ios")): otherwise,
        "//conditions:default": extra_deps,
    })
