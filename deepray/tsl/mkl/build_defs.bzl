...
def if_mkl(if_true, if_false = []):
    ...
    """
    return select({
        "@//deepray/tsl/mkl:build_with_mkl_aarch64": if_true,
        "@//deepray/tsl:linux_x86_64": if_true,
        "@//deepray/tsl:windows": if_true,
...
def if_mkl_ml(if_true, if_false = []):
    ...
    """
    return select({
        "@//third_party/mkl_dnn:build_with_mkl_opensource": if_false,
        "@//deepray/tsl/mkl:build_with_mkl": if_true,
        "//conditions:default": if_false,
    })
...
def if_mkl_lnx_x64(if_true, if_false = []):
    ...
    """
    return select({
        "@//deepray/tsl/mkl:build_with_mkl_lnx_x64": if_true,
        "//conditions:default": if_false,
    })
...
def if_enable_mkl(if_true, if_false = []):
    ...
    """
    return select({
        "@//deepray/tsl/mkl:enable_mkl": if_true,
        "//conditions:default": if_false,
    })
...
def mkl_deps():
    ...
    """
    return select({
        "@//deepray/tsl/mkl:build_with_mkl_aarch64": ["@mkl_dnn_acl_compatible//:mkl_dnn_acl"],
        "@//deepray/tsl:linux_x86_64": ["@mkl_dnn_v1//:mkl_dnn"],
        "@//deepray/tsl:windows": ["@mkl_dnn_v1//:mkl_dnn"],
        "//conditions:default": [],
    })
