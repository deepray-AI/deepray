...
load(
    "//deepray/tsl/platform:rules_cc.bzl",
    "cc_binary",
    "cc_library",
    "cc_shared_library",
)
...
load(
    "//deepray/tsl/mkl:build_defs.bzl",
    "if_enable_mkl",
    "if_mkl",
)
load(
...
def if_tsl_link_protobuf(if_true, if_false = []):
    return select({
        ...
        clean_dep("//deepray/tsl:tsl_protobuf_header_only"): if_false,
    })
...
def if_libtpu(if_true, if_false = []):
    ...
    return select({
        ...
        clean_dep("//deepray/tsl:with_tpu_support"): if_true,
...
def if_macos(a, otherwise = []):
    return select({
        clean_dep("//deepray/tsl:macos"): a,
        "//conditions:default": otherwise,
    })

def if_windows(a, otherwise = []):
    return select({
        clean_dep("//deepray/tsl:windows"): a,
        "//conditions:default": otherwise,
    })
...
def if_xla_available(if_true, if_false = []):
    return select({
        clean_dep("//deepray/tsl:with_xla_support"): if_true,
        "//conditions:default": if_false,
    })

def if_android_arm(a):
    return select({
        clean_dep("//deepray/tsl:android_arm"): a,
        "//conditions:default": [],
    })
...
def if_linux_x86_64(a):
    return select({
        clean_dep("//deepray/tsl:linux_x86_64"): a,
        "//conditions:default": [],
    })

def if_ios_x86_64(a):
    return select({
        clean_dep("//deepray/tsl:ios_x86_64"): a,
        "//conditions:default": [],
    })

def if_no_default_logger(a):
    return select({
        clean_dep("//deepray/tsl:no_default_logger"): a,
        "//conditions:default": [],
    })
...
def if_nccl(if_true, if_false = []):
    return select({
        clean_dep("//deepray/tsl:no_nccl_support"): if_false,
        clean_dep("//deepray/tsl:windows"): if_false,
        "//conditions:default": if_true,
    })
...
def tsl_copts(
    ...
    return (
        ...
        select({
            clean_dep("//deepray/tsl:framework_shared_object"): [],
            "//conditions:default": ["-DTENSORFLOW_MONOLITHIC_BUILD"],
        }) +
        select({
            clean_dep("//deepray/tsl:android"): android_copts,
            clean_dep("//deepray/tsl:emscripten"): [],
            clean_dep("//deepray/tsl:macos"): [],
            clean_dep("//deepray/tsl:windows"): get_win_copts(is_external),
            clean_dep("//deepray/tsl:ios"): [],
            clean_dep("//deepray/tsl:no_lgpl_deps"): ["-D__TENSORFLOW_NO_LGPL_DEPS__", "-pthread"],
...
def tf_openmp_copts():
    ...
    return select({
        ...
        # "//deepray/tsl/mkl:build_with_mkl_lnx_openmp": ["-fopenmp"],
        # "//deepray/tsl/mkl:build_with_mkl_windows_openmp": ["/openmp"],
        # copybara:uncomment_end_and_comment_begin
...
def tsl_grpc_cc_dependencies():
    return [clean_dep("//deepray/tsl:grpc++")]
...
def tsl_pybind_extension_opensource(
    ...
    native.py_library(
        name = name,
        data = select({
            clean_dep("//deepray/tsl:windows"): [pyd_file],
