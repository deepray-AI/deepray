...
load("//deepray:tensorflow.default.bzl", "if_indexing_source_code")
load("//deepray/python/tools/api/generator:api_init_files.bzl", "TENSORFLOW_API_INIT_FILES")
...
def gen_api_init_files(
        ...
        package_deps = [
            "//deepray/python:no_contrib",
            "//deepray/python:modules_with_exports",
            "//deepray/lite/python:analyzer",
            "//deepray/lite/python:lite",
            "//deepray/lite/python/authoring",
        ],
    ...
    native.py_binary(
        ...
        srcs = ["//deepray/python/tools/api/generator:create_python_api.py"],
        main = "//deepray/python/tools/api/generator:create_python_api.py",
        python_version = "PY3",
        srcs_version = "PY3",
        visibility = ["//visibility:public"],
        deps = package_deps + [
            "//deepray/python:util",
            "//deepray/python/tools/api/generator:doc_srcs",
        ],
    )
    ...
    native.genrule(
        ...
        visibility = [
            "//deepray:__pkg__",
            "//deepray/tools/api/tests:__pkg__",
        ],
    )
