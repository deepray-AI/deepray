...
class ConditionalGradient(KerasLegacyOptimizer):
  ...
  def __init__(
    ...
    **kwargs,
    ...
    if ord not in supported_norms:
      raise ValueError("'ord' must be a supported matrix norm in %s, got '%s' instead" % (supported_norms, ord))
    self.ord = ord
  ...
  def _prepare_local(self, var_device, var_dtype, apply_state):
    ...
    apply_state[(var_device, var_dtype)]["learning_rate"] = tf.identity(self._get_hyper("learning_rate", var_dtype))
    apply_state[(var_device, var_dtype)]["lambda_"] = tf.identity(self._get_hyper("lambda_", var_dtype))
    apply_state[(var_device, var_dtype)]["epsilon"] = tf.convert_to_tensor(self.epsilon, var_dtype)
  ...
  def _resource_apply_dense(self, grad, var, apply_state=None):
    ...
    if self.ord == "fro":
      norm = tf.convert_to_tensor(self._frobenius_norm(grad), name="norm", dtype=var.dtype.base_dtype)
      s = grad / (norm + epsilon)
    else:
