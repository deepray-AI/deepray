...
from deepray.optimizers import Lookahead
...
def run_sparse_sample(iterations, optimizer, seed=0x2019):
  ...
  grad_0 = tf.IndexedSlices(tf.constant([np.random.standard_normal()]), tf.constant([0]), tf.constant([2]))
  grad_1 = tf.IndexedSlices(tf.constant([np.random.standard_normal()]), tf.constant([1]), tf.constant([2]))
  ...
  for k in [5, 10, 100]:
    for alpha in [0.3, 0.7]:
      ...
      for val, quick, slow in zip(vals, quick_vars, slow_vars):
        ...
        np.testing.assert_allclose(expected.numpy(), slow.numpy(), rtol=1e-06, atol=1e-06)
