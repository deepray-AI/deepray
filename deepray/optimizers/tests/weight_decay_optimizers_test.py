...
from deepray.optimizers import weight_decay_optimizers
...
def do_test(
  ...
  if do_sparse:
    ...
    grads0 = tf.IndexedSlices(tf.constant(grads0_np), tf.constant(grads0_np_indices), tf.constant([2]))
  ...
  for _ in range(3):
    ...
    var0_np, np_slot_vars0 = update_fn(var0_np, grads0_np, np_slot_vars0, **optimizer_kwargs)
    var1_np, np_slot_vars1 = update_fn(var1_np, grads1_np, np_slot_vars1, **optimizer_kwargs)
...
def do_test_sparse_repeated_indices(dtype, optimizer, **optimizer_kwargs):
  ...
  np.testing.assert_allclose(aggregated_update_var.numpy(), repeated_index_update_var.numpy())
  for _ in range(3):
...
def sgdw_update_numpy(param, grad_t, slot_vars, learning_rate, momentum, weight_decay):
  ...
  m = slot_vars.get("m", 0)
  lr, momentum, wd = (v() if callable(v) else v for v in (learning_rate, momentum, weight_decay))
...
def test_exclude_weight_decay_adamw():
  optimizer = weight_decay_optimizers.AdamW(learning_rate=1e-4, weight_decay=1e-4, exclude_from_weight_decay=["var1"])
...
def test_keras_fit_with_schedule():
  ...
  model = tf.keras.models.Sequential([tf.keras.layers.Dense(2)])
  loss = tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True)
  wd_schedule = tf.keras.optimizers.schedules.ExponentialDecay(WEIGHT_DECAY, decay_steps=10, decay_rate=0.9)
  optimizer = weight_decay_optimizers.AdamW(learning_rate=1e-4, weight_decay=wd_schedule)
  model.compile(optimizer=optimizer, loss=loss, metrics=["accuracy"])
  x, y = np.random.uniform(size=(2, 4, 1))
  model.fit(x, y, epochs=1)
...
def test_weight_decay_with_piecewise_constant_decay_schedule():
  ...
  optimizer = weight_decay_optimizers.SGDW(learning_rate=1e-2, weight_decay=wd_schedule)
  model.compile(optimizer=optimizer, loss=loss, metrics=["accuracy"])
  x, y = np.random.uniform(size=(2, 4, 1))
...
def test_exclude_weight_decay_sgdw():
  optimizer = weight_decay_optimizers.SGDW(learning_rate=0.01, weight_decay=1e-4, exclude_from_weight_decay=["var1"])
  var0 = tf.Variable([], name="var0")
  var1 = tf.Variable([], name="var1")
  var1_weight = tf.Variable([], name="var1_weight")
...
def test_optimizer_sparse(dtype, optimizer):
  do_test_sparse_repeated_indices(dtype, optimizer, learning_rate=0.001, momentum=0.9, weight_decay=WEIGHT_DECAY)
