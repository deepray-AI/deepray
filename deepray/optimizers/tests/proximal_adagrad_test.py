...
from deepray.optimizers import ProximalAdagrad
...
def run_sample(iterations, expected, optimizer, sparse=False, rtol=1e-7, atol=0.0):
  ...
  if sparse:
    grad_0 = tf.IndexedSlices(tf.constant([0.1], dtype=tf.float32), tf.constant([0]), tf.constant([2]))
    grad_1 = tf.IndexedSlices(tf.constant([0.02], dtype=tf.float32), tf.constant([1]), tf.constant([2]))
  else:
