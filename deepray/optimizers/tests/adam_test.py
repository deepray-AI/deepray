...
def adam_sparse_update_numpy_amsgrad(
  ...
  param_t_slice = param[indices] - (lr_t * (m_t_slice / (np.sqrt(v_hat_t_slice) + epsilon)))
...
class AdamOptimizerTest(test.TestCase, parameterized.TestCase):
  ...
  def testSparseDevicePlacement(self):
    ...
    for index_dtype in [dtypes.int32, dtypes.int64]:
      with ops.Graph().as_default(), self.cached_session(use_gpu=test.is_gpu_available()):
    ...
    for i, dtype in enumerate([dtypes.half, dtypes.float32, dtypes.float64]):
      with self.cached_session():
        ...
        for t in range(3):
          ...
          var0_np, m0, v0, v0hat = adam_update_numpy_amsgrad(var0_np, grads0_np, t, m0, v0, v0hat)
          var1_np, m1, v1, v1hat = adam_update_numpy_amsgrad(var1_np, grads1_np, t, m1, v1, v1hat)
  ...
  def testSparseWithAmsgrad(self):
    ...
    for dtype in [dtypes.float32, dtypes.float64]:
      with self.cached_session(use_gpu=test.is_gpu_available()):
        ...
        if not context.executing_eagerly():
          repeated_update = opt_repeated.apply_gradients([(grad_repeated_index, repeated_index_update_var)])
          aggregated_update = opt_aggregated.apply_gradients([(grad_aggregated, aggregated_update_var)])
        self.evaluate(variables.global_variables_initializer())
        ...
        for t in range(3):
          ...
          var0_np, m0, v0, v0hat = adam_sparse_update_numpy_amsgrad(var0_np, indices_np, grads0_np, t, m0, v0, v0hat)
  ...
  def testBasicWithLearningRateDecay(self):
    ...
    for i, dtype in enumerate([dtypes.half, dtypes.float32, dtypes.float64]):
      with ops.Graph().as_default(), self.cached_session():
        ...
        opt = adam.Adam(learning_rate=learning_rate, beta_1=beta_1, beta_2=beta_2, epsilon=epsilon, decay=decay)
        update = opt.apply_gradients(zip([grads0, grads1], [var0, var1]))
        ...
        for t in range(3):
          ...
          var0_np, m0, v0 = adam_update_numpy(var0_np, grads0_np, t, m0, v0, lr=lr_np)
          var1_np, m1, v1 = adam_update_numpy(var1_np, grads1_np, t, m1, v1, lr=lr_np)
  ...
  def testBasicWithLearningRateInverseTimeDecay(self):
    ...
    for i, dtype in enumerate([dtypes.half, dtypes.float32, dtypes.float64]):
      with ops.Graph().as_default(), self.cached_session():
        ...
        learning_rate = 0.001
        decay = 0.5
        lr_schedule = learning_rate_schedule.InverseTimeDecay(learning_rate, decay_steps=1.0, decay_rate=decay)
        beta_1 = 0.9
        beta_2 = 0.999
        epsilon = 1e-7

        opt = adam.Adam(learning_rate=lr_schedule, beta_1=beta_1, beta_2=beta_2, epsilon=epsilon)
        update = opt.apply_gradients(zip([grads0, grads1], [var0, var1]))
  ...
  def testSlotsUniqueEager(self):
    ...
    self.assertEqual(self.evaluate(opt.variables()[0]), self.evaluate(opt.iterations))
  ...
  def testSetWeightsFromV1AdamWithoutMinimize(self):
    ...
    self.evaluate(variables.global_variables_initializer())
    self.assertEqual(self.evaluate(keras_v1_iteration), self.evaluate(keras_v2_iteration))
  ...
  def testSparseDevicePlacement(self):
    ...
    for index_dtype in [dtypes.int32, dtypes.int64]:
      with ops.Graph().as_default(), self.cached_session(use_gpu=test.is_gpu_available()):
        ...
        indices = constant_op.constant([0, 1], dtype=index_dtype)
        g_sum = lambda: math_ops.reduce_sum(array_ops.gather(var, indices))  # pylint: disable=cell-var-from-loop
        optimizer = tf_adam.NonFusedAdam(3.0)
        minimize_op = optimizer.minimize(g_sum, var_list=[var])
        self.evaluate(variables.global_variables_initializer())
        minimize_op.run()
  ...
  def testSparseRepeatedIndices(self):
    ...
    for dtype in [dtypes.half, dtypes.float32, dtypes.float64]:
      with ops.Graph().as_default(), self.cached_session():
        ...
        repeated_update = tf_adam.NonFusedAdam().apply_gradients([(grad_repeated_index, repeated_index_update_var)])
        aggregated_update = tf_adam.NonFusedAdam().apply_gradients([(grad_aggregated, aggregated_update_var)])
        self.evaluate(variables.global_variables_initializer())
        self.assertAllClose(aggregated_update_var, self.evaluate(repeated_index_update_var))
        for _ in range(3):
          repeated_update.run()
  ...
  def doTestBasic(self, use_callable_params=False):
    for i, dtype in enumerate([dtypes.half, dtypes.float32, dtypes.float64]):
      with self.cached_session():
        ...
        opt = tf_adam.NonFusedAdam(learning_rate=learning_rate)
        if not context.executing_eagerly():
          update = opt.apply_gradients(zip([grads0, grads1], [var0, var1]))
        ...
        for t in range(3):
          ...
          # Validate updated params
          self.assertAllCloseAccordingToType(var0_np, self.evaluate(var0), rtol=1e-4, atol=1e-4)
          self.assertAllCloseAccordingToType(var1_np, self.evaluate(var1), rtol=1e-4, atol=1e-4)
  ...
  def testBasicWithAmsgrad(self):
    for i, dtype in enumerate([dtypes.half, dtypes.float32, dtypes.float64]):
      with self.cached_session():
        ...
        opt = tf_adam.NonFusedAdam(amsgrad=True)
        if not context.executing_eagerly():
          update = opt.apply_gradients(zip([grads0, grads1], [var0, var1]))
  ...
  def testSparseWithAmsgrad(self):
    ...
    for dtype in [dtypes.float32, dtypes.float64]:
      with self.cached_session():
        ...
        grad_aggregated = indexed_slices.IndexedSlices(grads0_np, indices, constant_op.constant([2, 1]))
        opt_repeated = tf_adam.NonFusedAdam(amsgrad=True)
        opt_aggregated = tf_adam.NonFusedAdam(amsgrad=True)
        if not context.executing_eagerly():
          repeated_update = opt_repeated.apply_gradients([(grad_repeated_index, repeated_index_update_var)])
          aggregated_update = opt_aggregated.apply_gradients([(grad_aggregated, aggregated_update_var)])
        self.evaluate(variables.global_variables_initializer())
  ...
  def testBasicWithLearningRateDecay(self):
    ...
    for i, dtype in enumerate([dtypes.half, dtypes.float32, dtypes.float64]):
      with ops.Graph().as_default(), self.cached_session():
        ...
        opt = tf_adam.NonFusedAdam(
            learning_rate=learning_rate, beta_1=beta_1, beta_2=beta_2, epsilon=epsilon, decay=decay
        )
        update = opt.apply_gradients(zip([grads0, grads1], [var0, var1]))
  ...
  def testBasicWithLearningRateInverseTimeDecay(self):
    ...
    for i, dtype in enumerate([dtypes.half, dtypes.float32, dtypes.float64]):
      with ops.Graph().as_default(), self.cached_session():
        ...
        opt = tf_adam.NonFusedAdam(learning_rate=lr_schedule, beta_1=beta_1, beta_2=beta_2, epsilon=epsilon)
        update = opt.apply_gradients(zip([grads0, grads1], [var0, var1]))
  ...
  def testTensorLearningRate(self):
    ...
    for dtype in [dtypes.half, dtypes.float32, dtypes.float64]:
      with ops.Graph().as_default(), self.cached_session():
        ...
        grads0 = constant_op.constant(grads0_np)
        grads1 = constant_op.constant(grads1_np)
        opt = tf_adam.NonFusedAdam(constant_op.constant(0.001))
        update = opt.apply_gradients(zip([grads0, grads1], [var0, var1]))
        self.evaluate(variables.global_variables_initializer())
