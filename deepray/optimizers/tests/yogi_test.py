...
from deepray.optimizers import yogi
...
def yogi_update_numpy(
  ...
  if l1reg > 0:
    param_t = (param_t - l1reg * per_coord_lr * np.sign(param_t)) / (1 + l2reg * per_coord_lr)
    print(param_t.dtype)
...
def do_test_sparse(beta1=0.0, l1reg=0.0, l2reg=0.0):
  for dtype in _dtypes_to_test(use_gpu=test_utils.is_gpu_available()):
    ...
    for t in range(1, 4):
      ...
      var0_np, m0, v0 = yogi_update_numpy(var0_np, grads0_np, t, m0, v0, beta1=beta1, l1reg=l1reg, l2reg=l2reg)
      var1_np, m1, v1 = yogi_update_numpy(var1_np, grads1_np, t, m1, v1, beta1=beta1, l1reg=l1reg, l2reg=l2reg)
