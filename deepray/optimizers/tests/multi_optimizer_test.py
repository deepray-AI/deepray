...
class CompositeOptimizerTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def test_composite_optimizer(self, optimizer1_type, optimizer2_type):
    ...
    var1 = tf.Variable(values1, name="var1")
    var2 = tf.Variable(values2, name="var2")
    var3 = tf.Variable(values3, name="var3")
  ...
  #   var1 = tf.Variable([0.1, 0.2, 1.0], name="var1")
  #   var2 = tf.Variable([-5.1, 0.1, 0], name="var2")
  #   var3 = tf.Variable([-2.1, 1.3, 0 / 3], name="var3")
  ...
  #   composite_optimizer = MultiOptimizer(
  #       [
  #           (tf.keras.optimizers.legacy.Adam(), "var1"),
  #           (tf.keras.optimizers.legacy.Adagrad(), "var1,var2"),
  #       ],
  ...
  #       [
  #           (tf.keras.optimizers.legacy.Adam(), "var1"),
  #           (tf.keras.optimizers.legacy.Adagrad(), "var2"),
  #       ],
  #       default_optimizer=tf.keras.optimizers.Adam()
  #   )
  ...
  #     composite_optimizer = MultiOptimizer(
  #         [
  #             # (tf.keras.optimizers.legacy.Adam(), lambda: model.trainable_variables[:1]),
  #             # (tf.keras.optimizers.legacy.Adagrad(), lambda: model.trainable_variables[1:]),
  #         ],
  #         default_optimizer=tf.keras.optimizers.Adam()
  #     )
  #     model.compile(optimizer=composite_optimizer, loss=tf.keras.losses.MSE)
  #     return model
  #
  #   batch_size = 16
  #   num_of_batches = 8
  ...
  #   self.assertAllClose(new_model.optimizer.variables(), model.optimizer.variables())
  #
  #   model_pred = new_model.predict(training_dataset)
  #
  #   with tempfile.TemporaryDirectory() as tmp:
  #     path = os.path.join(tmp, "model_with_composite_optimizer")
  #     new_model.save(path, include_optimizer=False, options=tf.saved_model.SaveOptions(namespace_whitelist=["Addons"]))
  #     loaded_model = tf.keras.models.load_model(path)
  #     loaded_pred = loaded_model.predict(training_dataset)
  #
  #   self.assertEqual(model.layers[0].get_config(), loaded_model.layers[0].get_config())
