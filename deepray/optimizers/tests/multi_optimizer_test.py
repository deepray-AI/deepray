...
def test_fit_layer_optimizer(device, serialize, tmpdir):
  model = tf.keras.Sequential([tf.keras.Input(shape=[1]), tf.keras.layers.Dense(1), tf.keras.layers.Dense(1)])
...
def test_list_of_layers():
  ...
  weights_before_train = [[weight.numpy() for weight in layer.weights] for layer in model.layers]
  ...
  weights_after_train = [[weight.numpy() for weight in layer.weights] for layer in model.layers]

  assert_list_allclose(weights_before_train[0], weights_after_train[0])

  for layer_before, layer_after in zip(weights_before_train[1:], weights_after_train[1:]):
...
def test_pretrained_model():
  ...
  resnet_weights_before_train = [weight.numpy() for weight in resnet.trainable_weights]
...
def test_serialization():
  ...
  if Version(tf.__version__) >= Version("2.13"):
    ...
    old_config["optimizer_specs"][0]["optimizer"] = old_config["optimizer_specs"][0]["optimizer"].__class__
    old_config["optimizer_specs"][1]["optimizer"] = old_config["optimizer_specs"][1]["optimizer"].__class__

    new_config["optimizer_specs"][0]["optimizer"] = new_config["optimizer_specs"][0]["optimizer"].__class__
    new_config["optimizer_specs"][1]["optimizer"] = new_config["optimizer_specs"][1]["optimizer"].__class__
...
def test_serialization_after_training(tmpdir):
  ...
  for old_optimizer_spec, new_optimizer_spec in zip(old_config["optimizer_specs"], new_config["optimizer_specs"]):
    assert old_optimizer_spec["weights"] == new_optimizer_spec["weights"]
    assert (old_optimizer_spec["optimizer"].get_config() == new_optimizer_spec["optimizer"].get_config())
