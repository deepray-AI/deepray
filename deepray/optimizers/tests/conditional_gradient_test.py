...
from deepray.optimizers import conditional_gradient as cg_lib
...
def test_like_dist_belief_nuclear_cg01():
  ...
  for i in range(num_samples):
    ...
    np.testing.assert_allclose(np.array(db_out[i]), var0.numpy(), rtol=1e-6, atol=1e-6)
...
def test_basic_frobenius(dtype, use_resource):
  ...
  cg_opt = cg_lib.ConditionalGradient(learning_rate=learning_rate, lambda_=lambda_, ord=ord)
...
def test_minimize_sparse_resource_variable_nuclear():
  ...
  for dtype in _dtypes_with_checking_system(use_gpu=test_utils.is_gpu_available(), system=platform.system()):
    var0 = tf.Variable([[1.0, 2.0]], dtype=dtype)
...
def test_like_dist_belief_frobenius_cg01():
  ...
  for i in range(num_samples):
    grads0 = tf.constant(db_grad[i])
    cg_opt.apply_gradients(zip([grads0], [var0]))
    np.testing.assert_allclose(np.array(db_out[i]), var0.numpy(), rtol=1e-06, atol=1e-06)
...
def test_serialization():
  ...
  lambda_ = 0.1
  ord = "nuclear"
  optimizer = cg_lib.ConditionalGradient(learning_rate=learning_rate, lambda_=lambda_, ord=ord)
