...
from deepray.core.utils import common
from deepray.core.utils import tpu_summaries
...
class DummyTrainer(tf.Module):
  ...
  @train_function_with_summaries(input_signature=[tf.TensorSpec((), dtype=tf.int32)])
...
class TpuSummariesTest(tf.test.TestCase):
  ...
  def test_train_with_tpu_summary_optimization(self):
    output = self._validate_tpu_summary_optimization(self.trainer.train_with_tpu_summary_optimization)
    self.assertEqual(output, self.trainer.step_counter.numpy())

  def test_train_with_tpu_summary_optimization_no_decorator(self):
    optimized = train_function_with_summaries(self.trainer.train_with_tpu_summary_optimization_no_decorator)
