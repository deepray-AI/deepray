...
def create_tf_while_loop_fn_with_state(step_fn):
  ...
  def loop_fn_with_state(iterator, num_steps, state, reduce_fn):
    ...
    def _get_relaxed_shape_structure(s):
      ...
      return tf.nest.pack_sequence_as(state, [_get_relaxed_tensor_shape(t) for t in tf.nest.flatten(s)])
    ...
    for _ in tf.range(num_steps):
      ...
      with tf.name_scope(""):
        ...
        # and concat to `state`.
        tf.autograph.experimental.set_loop_options(shape_invariants=[(state, _get_relaxed_shape_structure(state))])
        outputs = step_fn(iterator)
        state = reduce_fn(state, outputs)
    return state
