...
class Trainer(Module):
  ...
  def __init__(self,
    ...
    if not self.use_horovod or hvd.rank() == 0:
      ...
      self.eval_summary_writer = tf.summary.create_file_writer(
        os.path.join(self.summary_dir, 'eval'))
    ...
    if FLAGS.init_checkpoint:
      self.sub_manager = tf.train.CheckpointManager(self.sub_model_checkpoint, os.path.join(FLAGS.model_dir, 'sub_ckpt'), max_to_keep=3)
  ...
  def fit(self,
    ...
    if FLAGS.keras_use_ctl:
      ...
      return self.run_customized_training_loop(train_input,
                                               eval_input)
    else:
      ...
      if FLAGS.use_horovod:
        callbacks += [
          ...
          de.keras.callbacks.DEHvdBroadcastGlobalVariablesCallback(root_rank=0) if FLAGS.use_dynamic_embedding else hvd.callbacks.BroadcastGlobalVariablesCallback(0),
        ]
  ...
  def run_customized_training_loop(self,
                                   train_input=None,
                                   eval_input=None,
                                   ):
      ...
      while self.steps_per_epoch <= 0 or self._step_epoch < self.steps_per_epoch:
        ...
        try:
          if steps == 1:
            ...
            tmp_logs = self._train_steps(train_iterator, tf.convert_to_tensor(steps, dtype=tf.int32), num_accumulation_steps)
    ...
    if not self.use_horovod or hvd.rank() == 0:
      ...
      if dllogging and self._perf_wo_n != 0:
        dllogging.logger.log(step=(), data={"throughput_train": self._perf_wo / self._perf_wo_n}, verbosity=Verbosity.DEFAULT)
  ...
  def _replicated_step(self, inputs, first_batch=False):
    ...
    if self.use_horovod and not FLAGS.use_dynamic_embedding:
      tape = hvd.DistributedGradientTape(tape, sparse_as_dense=True, compression=Compression.fp16 if self.use_float16 else Compression.none)
    # Run backwards pass.
    ...
    if self.metric_container: self.metric_container.update_state(y_true=labels,
  ...
  def forward(self, inputs):
    ...
    # For reporting, the metric takes the mean of losses.
    if self.metric_container: self.metric_container.update_state(y_true=labels, y_pred=model_outputs, sample_weight=sample_weight)

  def step(self, num_grad_accumulates):
    gradients = self.accum_gradients.gradients
    if self.use_horovod:
      gradients = [None if g is None else hvd.allreduce(g / tf.cast(num_grad_accumulates, g.dtype),
                                                        compression=Compression.fp16 if self.use_float16 else Compression.none) for g in gradients]
    else:
  ...
  def test_step(self, iterator):
    ...
    def _test_step_fn(inputs):
      ...
      loss = self.loss_container(labels, model_outputs, sample_weight=sample_weight)
      if self.metric_container: self.metric_container.update_state(labels, model_outputs, sample_weight=sample_weight)
