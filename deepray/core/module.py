...
class Module():
  ...
  def on_epoch_begin(self, epoch):
    ...
    if self.metric_container: self.metric_container.reset_state()
  ...
  def on_batch_end(self, logs, steps, t0):
    ...
    if self.steps_per_epoch > 0:
      training_status = 'Train Step: %d/%d / time=%.3f sec' % (self.current_step, self.steps_per_epoch * self.epochs, elapse_time)
    else:
    ...
    if not self.use_horovod or hvd.rank() == 0:
      if self.use_float16:
        logging.info('Step: %d Lr %g Loss scale %g' % (self.current_step, self.optimizer._optimizer._decayed_lr('float32'), self.optimizer.loss_scale))
  ...
  def save_model_to_export(self, epoch=None):
    ...
    if FLAGS.use_dynamic_embedding:
      ...
      tf.keras.models.save_model(self.model, model_save_dir, overwrite=True, include_optimizer=False, save_traces=False,
                                 options=options,
                                 signatures={
  ...
  def save_model_to_serving(self):  # 4 zero init
    ...
    options = tf.saved_model.SaveOptions(namespace_whitelist=['TFRA'])
    tf.keras.models.save_model(zero_model, zero_model_save_path, overwrite=True, include_optimizer=False, save_traces=False,
                               options=options,
                               signatures={
                                 'serving_default': serve.get_concrete_function(*arg_specs, **kwarg_specs)
                               })
