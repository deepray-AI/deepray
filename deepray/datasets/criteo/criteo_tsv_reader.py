...
class CriteoTsvReader(DataPipeLine):
  ...
  def _generate_synthetic_data(
      self, ctx: tf.distribute.InputContext, batch_size: int, is_training=True
  ) -> tf.data.Dataset:
  ...
  def build_dataset(
    ...
    if FLAGS.sharding and context and context.num_input_pipelines > 1:
      filenames = filenames.shard(context.num_input_pipelines, context.input_pipeline_id)
    ...
    dataset = indices.interleave(
        map_func=make_dataset, cycle_length=FLAGS.interleave_cycle, num_parallel_calls=tf.data.experimental.AUTOTUNE
    )
