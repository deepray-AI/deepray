...
class ParquetPipeLine(DataPipeLine):
  ...
  def build_dataset(self, input_file_pattern,
    ...
    dataset = parquet_dataset_ops.ParquetDataset(
      ...
      fields=[parquet_dataset_ops.DataFrame.Field(k, dtype, ragged_rank=1 if length != 1 else 0) for k, dtype, length in self.feature_map[["name", "dtype", "length"]].values]
    ...
    if FLAGS.shuffle_buffer:
      dataset = dataset.apply(tf.data.experimental.shuffle_and_repeat(buffer_size=FLAGS.shuffle_buffer, count=FLAGS.epochs))
    else:
