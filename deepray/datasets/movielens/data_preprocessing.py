...
def read_dataframe(
  ...
  grouped = df.groupby(rconst.USER_COLUMN)
  df = grouped.filter(
    lambda x: len(x) >= rconst.MIN_NUM_RATINGS)  # type: pd.DataFrame

  original_users = df[rconst.USER_COLUMN].unique()
  original_items = df[rconst.ITEM_COLUMN].unique()
  ...
  df[rconst.USER_COLUMN] = df[rconst.USER_COLUMN].apply(
    lambda user: user_map[user])
  df[rconst.ITEM_COLUMN] = df[rconst.ITEM_COLUMN].apply(
  ...
  assert df[rconst.USER_COLUMN].max() == num_users - 1
  assert df[rconst.ITEM_COLUMN].max() == num_items - 1
  ...
  df.sort_values(by=rconst.TIMESTAMP_COLUMN, inplace=True)
  df.sort_values([rconst.USER_COLUMN, rconst.TIMESTAMP_COLUMN],
                 inplace=True,
                 kind="mergesort")
...
def _filter_index_sort(raw_rating_path: Text,
  ...
  if valid_cache:
    ...
    grouped = df.groupby(rconst.USER_COLUMN, group_keys=False)
    eval_df, train_df = grouped.tail(1), grouped.apply(lambda x: x.iloc[:-1])

    data = {
      rconst.TRAIN_USER_KEY:
        train_df[rconst.USER_COLUMN].values.astype(rconst.USER_DTYPE),
      rconst.TRAIN_ITEM_KEY:
        train_df[rconst.ITEM_COLUMN].values.astype(rconst.ITEM_DTYPE),
      rconst.EVAL_USER_KEY:
        eval_df[rconst.USER_COLUMN].values.astype(rconst.USER_DTYPE),
      rconst.EVAL_ITEM_KEY:
        eval_df[rconst.ITEM_COLUMN].values.astype(rconst.ITEM_DTYPE),
      rconst.USER_MAP:
        user_map,
      rconst.ITEM_MAP:
        item_map,
...
def instantiate_pipeline(dataset,
  ...
  raw_rating_path = os.path.join(data_dir, dataset, rconst.RATINGS_FILE)
  ...
  num_users, num_items = rconst.DATASET_TO_NUM_USERS_AND_ITEMS[dataset]
