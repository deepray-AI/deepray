...
def read_dataframe(raw_rating_path: Text) -> Tuple[Dict[int, int], Dict[int, int], pd.DataFrame]:
  ...
  df[rconst.USER_COLUMN] = df[rconst.USER_COLUMN].apply(lambda user: user_map[user])
  df[rconst.ITEM_COLUMN] = df[rconst.ITEM_COLUMN].apply(lambda item: item_map[item])
  ...
  df.sort_values([rconst.USER_COLUMN, rconst.TIMESTAMP_COLUMN], inplace=True, kind="mergesort")
