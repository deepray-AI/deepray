...
from deepray.datasets.movielens import constants
...
def _download_and_clean(dataset, data_dir):
  ...
  if dataset not in constants.DATASETS:
    raise ValueError("dataset {} is not in {{{}}}".format(
      dataset, ",".join(constants.DATASETS)))

  data_subdir = os.path.join(data_dir, dataset)

  expected_files = ["{}.zip".format(dataset), constants.RATINGS_FILE, constants.MOVIES_FILE]
...
def _regularize_1m_dataset(temp_dir):
  ...
  _transform_csv(
    ...
    names=constants.RATING_COLUMNS, skip_first=False, separator="::")

  _transform_csv(
    input_path=os.path.join(working_dir, "movies.dat"),
    output_path=os.path.join(temp_dir, constants.MOVIES_FILE),
    names=constants.MOVIE_COLUMNS, skip_first=False, separator="::")
...
def _regularize_20m_dataset(temp_dir):
  ...
  _transform_csv(
    ...
    output_path=os.path.join(temp_dir, constants.RATINGS_FILE),
    names=constants.RATING_COLUMNS, skip_first=True, separator=",")

  _transform_csv(
    input_path=os.path.join(working_dir, "movies.csv"),
    output_path=os.path.join(temp_dir, constants.MOVIES_FILE),
    names=constants.MOVIE_COLUMNS, skip_first=True, separator=",")
...
def download(dataset, data_dir):
  if dataset:
    ...
    _ = [_download_and_clean(d, data_dir) for d in constants.DATASETS]


def ratings_csv_to_dataframe(data_dir, dataset):
  with tf.io.gfile.GFile(os.path.join(data_dir, dataset, constants.RATINGS_FILE)) as f:
...
def csv_to_joint_dataframe(data_dir, dataset):
  ...
  with tf.io.gfile.GFile(os.path.join(data_dir, dataset, constants.MOVIES_FILE)) as f:
    movies = pd.read_csv(f, encoding="utf-8")

  df = ratings.merge(movies, on=constants.ITEM_COLUMN)
  df[constants.RATING_COLUMN] = df[constants.RATING_COLUMN].astype(np.float32)
...
def integerize_genres(dataframe):
  ...
  dataframe[constants.GENRE_COLUMN] = dataframe[constants.GENRE_COLUMN].apply(_map_fn)
...
#     name="dataset",
#     default=None,
#     enum_values=constants.DATASETS,
