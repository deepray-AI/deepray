  ...
  def build_dataset(self, input_file_pattern,
    ...
    if is_training:
      ...
      with gzip.open(self.paths[1], "rb") as imgpath:
        x = np.frombuffer(imgpath.read(), np.uint8, offset=16).reshape(
          len(y), 28, 28
        )
    else:
    ...
    dataset = tf.data.Dataset.from_tensor_slices((tf.cast(x[..., tf.newaxis] / 255.0, tf.float32), tf.cast(y, tf.int64)))
