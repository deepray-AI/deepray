...
class AmazonBooks2014(TFRecordDataset):
  ...
  def parser(self, record):
    ...
    user_features = {
      f_name: tf.reshape(sample[f_name], [-1]) for f_name in USER_FEATURES
    }

    target_item_features = {
      f_name: tf.reshape(sample[f_name], [-1]) for f_name in TARGET_ITEM_FEATURES
    }

    padded_positive = {
      f_name: tf.reshape(sample[f_name], [-1, self.feature_map.loc[self.feature_map['name'] == f_name, 'length'].values[0]])
    ...
    long_sequence_features = {
      f_name: val[:, :self._max_seq_length] for f_name, val in padded_positive.items()
    }
    ...
    short_neg_sequence_features = {
      f_name: val[:, self._max_seq_length:] for f_name, val in padded_negative.items()
    }
