...
class DummyData:
  ...
  def __init__(self):
    ...
    self.memory = np.random.randn(self.batch, self.timestep, self.memory_size).astype(np.float32)
    self.memory_length = np.random.randint(low=1, high=self.timestep + 1, size=(self.batch,))
...
def test_save_load_layer(attention_cls):
  ...
  encoder_output = tf.keras.layers.LSTM(dummy_data.memory_size, return_sequences=True)(encoder_input)
...
def test_manual_memory_reset(attention_cls):
  ...
  def _compute_score(batch_size=None):
    ...
    attention.setup_memory(memory, memory_sequence_length=dummy_data.memory_length[:batch_size])
    assert attention.values.shape.as_list() == list(memory.shape)
    assert attention.keys.shape.as_list() == list(memory.shape)[:-1] + [dummy_data.units]
...
def test_memory_re_setup(attention_cls):
  ...
  x = np.random.randint(vocab, size=(num_batches * dummy_data.batch, dummy_data.timestep))
  x_test = np.random.randint(vocab, size=(num_batches * dummy_data.batch, dummy_data.timestep))
...
class ResultSummary(collections.namedtuple("ResultSummary", ("shape", "dtype", "mean"))):
  pass
...
class DummyData2:
  ...
  def __init__(self):
    ...
    self.encoder_outputs = np.random.randn(self.batch, self.encoder_timestep, self.encoder_dim)
    self.encoder_sequence_length = np.random.randint(1, high=self.encoder_timestep, size=(self.batch,)).astype(np.int32)
    self.decoder_inputs = np.random.randn(self.batch, self.decoder_timestep, self.units)
    self.decoder_sequence_length = np.random.randint(self.decoder_timestep, size=(self.batch,)).astype(np.int32)
...
def test_custom_attention_layer():
  ...
  attention_layer = tf.keras.layers.Dense(dummy_data.units * 2, use_bias=False, activation=tf.math.tanh)
  attention_wrapper = wrapper.AttentionWrapper(cell, attention_mechanism, attention_layer=attention_layer)
...
def _test_with_attention(
  ...
  decoder_inputs = np.random.randn(batch_size, decoder_max_time, input_depth).astype(np.float32)
  encoder_outputs = np.random.randn(batch_size, encoder_max_time, encoder_output_depth).astype(np.float32)
  ...
  for creator, depth in zip(create_attention_mechanisms, attention_mechanism_depths):
    ...
    if create_query_layer:
      create_attention_kwargs["query_layer"] = tf.keras.layers.Dense(depth, kernel_initializer="ones", use_bias=False)
  ...
  if cell._attention_layers is not None:
    for layer in cell._attention_layers:
      layer.kernel_initializer = tf.compat.v1.keras.initializers.glorot_uniform(seed=1337)
  ...
  assert (batch_size, expected_time, attention_depth) == tuple(final_outputs.rnn_output.shape.as_list())
  ...
  tf.nest.map_structure(assert_allclose_or_equal, expected_final_output, final_output_info)
  tf.nest.map_structure(assert_allclose_or_equal, expected_final_state, final_state_info)
