...
def test_dynamic_decode_rnn(time_major, maximum_iterations):
  ...
  def _decode(inputs, sequence_length):
    ...
    initial_state = cell.get_initial_state(batch_size=batch_size_t, dtype=inputs.dtype)
    return my_decoder(inputs, initial_state=initial_state, sequence_length=sequence_length)
...
def test_dynamic_decode_tflite_conversion():
  ...
  with pytest.raises(tf.errors.InvalidArgumentError, match="batch size"):
    ...
    _decode.get_concrete_function(tf.TensorSpec([2], dtype=tf.int32), tf.TensorSpec([], dtype=tf.int32))
...
def test_dynamic_decode_rnn_with_training_helper_matches_dynamic_rnn(use_sequence_length,):
  ...
  # state.
  np.testing.assert_allclose(final_decoder_outputs.rnn_output, final_rnn_outputs[:, 0:max_out, :])
  if use_sequence_length:
...
def test_dynamic_decode_rnn_with_scheduled_embedding_training_sampler():
  ...
  cell = tf.keras.layers.LSTMCell(cell_depth)
  sampler = sampler_py.ScheduledEmbeddingTrainingSampler(sampling_probability=tf.constant(1.0), embedding_fn=embedding)
  my_decoder = basic_decoder.BasicDecoder(cell=cell, sampler=sampler)
  ...
  final_outputs, _, _ = my_decoder(inputs, initial_state=initial_state, sequence_length=sequence_length)
