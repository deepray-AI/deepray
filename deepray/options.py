...
try:
    _TF_DEEPRAY_PY_OPS = bool(int(os.environ["TF_DEEPRAY_PY_OPS"]))
except KeyError:
    if platform.system() == "Linux":
        _TF_DEEPRAY_PY_OPS = False
    else:
        _TF_DEEPRAY_PY_OPS = True
...
by setting the enviornment variable `TF_DEEPRAY_PY_OPS=1`:
```bash
TF_DEEPRAY_PY_OPS=1 python my_script.py
```
...
def enable_custom_kernel():
    ...
    It has the same effect as setting environment variable `TF_DEEPRAY_PY_OPS=0`.
    """
    global _TF_DEEPRAY_PY_OPS
    _TF_DEEPRAY_PY_OPS = False
...
def disable_custom_kernel():
    ...
    It has the same effect as setting environment variable `TF_DEEPRAY_PY_OPS=1`.
    """
    global _TF_DEEPRAY_PY_OPS
    _TF_DEEPRAY_PY_OPS = True
