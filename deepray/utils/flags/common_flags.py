...
def define_common_flags():
  ...
  flags.DEFINE_string('input_meta_data_path', None,
  ...
  flags.DEFINE_string("eval_script", None,
  ...
  flags_core.define_data(dataset=True,
                         data_dir=True,
                         download_if_missing=True, )
  flags_core.define_device(tpu=False)
  flags_core.define_benchmark(benchmark=True, )

  flags.DEFINE_string(name="date",
                      default=(datetime.datetime.now() - datetime.timedelta(days=1)).strftime("%Y-%m-%d"),
                      help="")
  flags.DEFINE_string(name="restore_date",
                      default=None,
                      help="")
  flags.DEFINE_string(name="start_date",
                      default=None,
                      help="")
  flags.DEFINE_string(name="end_date",
                      default=None,
                      help="")
  flags.DEFINE_string(name="fine_tune",
                      default=None,
                      help="")
  flags.DEFINE_string(name="warmup_path",
                      default=None,
                      help="")
  flags.DEFINE_list("stage_one_mlp_dims",
                    default="200",
                    help="MLP hidden dimensions for stage one (excluding classification output).")
  flags.DEFINE_list("stage_two_mlp_dims",
                    default="200,80",
                    help="MLP hidden dimensions for stage two (excluding classification output).")
  flags.DEFINE_list("aux_mlp_dims",
                    default="100,50",
  ...
  flags.DEFINE_float("dropout_rate", default=-1, help="Dropout rate for all the classification MLPs (default: -1, disabled).", )
