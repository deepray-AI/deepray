...
class SummaryManager(metaclass=SingletonType):
  """A utility class for managing summary writing."""

  def __init__(self, summary_dir, global_step=None):
    ...
    if not tf.io.gfile.exists(self._summary_dir):
      tf.io.gfile.makedirs(self._summary_dir)
  ...
  def summary_writer(self, relative_path=""):
    ...
    Args:
      relative_path: The current path in which to write summaries, relative to
        ...
        root directory.
    """
    if self.summary_writers and relative_path in self.summary_writers:
  ...
  def flush(self):
    ...
    if self._enabled:
      tf.nest.map_structure(tf.summary.flush, self.summary_writers)
  ...
  def _write_summaries(self, summary_dict, relative_path=""):
    for name, value in summary_dict.items():
      if isinstance(value, dict):
        self._write_summaries(value, relative_path=os.path.join(relative_path, name))
      else:
