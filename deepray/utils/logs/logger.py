...
class BaseBenchmarkLogger(object):
  ...
  def log_evaluation_result(self, eval_results):
    ...
    if not isinstance(eval_results, dict):
      tf.compat.v1.logging.warning("eval_results should be dictionary for logging. Got %s", type(eval_results))
      return
  ...
  def log_run_info(self, model_name, dataset_name, run_params, test_id=None):
    tf.compat.v1.logging.info("Benchmark run: %s", _gather_run_info(model_name, dataset_name, run_params, test_id))
  ...
  def log_metric(self, name, value, unit=None, global_step=None, extras=None):
    ...
    if metric:
      try:
        ...
        tf.compat.v1.logging.warning("Failed to dump metric to log file: name %s, value %s, error %s", name, value, e)
  ...
  def log_run_info(self, model_name, dataset_name, run_params, test_id=None):
    ...
    with tf.io.gfile.GFile(os.path.join(self._logging_dir, BENCHMARK_RUN_LOG_FILE_NAME), "w") as f:
      try:
        ...
        tf.compat.v1.logging.warning("Failed to dump benchmark run info to log file: %s", e)
...
def _process_metric_to_json(name, value, unit=None, global_step=None, extras=None):
  ...
  if not isinstance(value, numbers.Number):
    tf.compat.v1.logging.warning("Metric value to log should be a number. Got %s", type(value))
    return None
...
def _collect_tensorflow_info(run_info):
  run_info["tensorflow_version"] = {"version": tf.version.VERSION, "git_hash": tf.version.GIT_VERSION}
