...
class Demo(tf.keras.Model):
  def __init__(self, embedding_size, is_training=True, *args, **kwargs):
    ...
    self.features_dict = {}
    for key, dtype, emb_size, length in self.feature_map.loc[self.feature_map["ftype"] == "Categorical"][["name", "dtype", "dim", "length"]].values:
      self.features_dict[key] = EmbeddingLayer(embedding_size=emb_size,
                                               key_dtype=dtype,
