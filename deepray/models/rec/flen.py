...
class FLEN(Model):
  def __init__(self, field_info, embedding_dim=16):
    ...
    for name, ftype, dtype, voc_size, length in self.feature_map[(self.feature_map['ftype'] == "Categorical")][["name", "ftype", "dtype", "voc_size", "length"]].values:
      self.embedding_layers[name] = Embedding(
        embedding_dim=embedding_dim,
        vocabulary_size=voc_size + 1,
  ...
  def call(self, inputs, training=False):
    ...
    field_embedding = [
      tf.reduce_mean(
        tf.stack([embedding[name] for name in names], axis=-1),
        axis=-1) for field, names in self.field_info.items()
    ]
