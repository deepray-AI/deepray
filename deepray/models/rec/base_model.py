...
Feature = namedtuple('Feature',
                     ['name', 'code', 'dtype', 'len', 'default_value',
                      'use_hash', 'ids', 'vocab', 'boundaries',
                      'emb_name', 'emb_size', 'emb_dim', 'emb_reg_l1', 'emb_split',
                      'emb_reg_l2', 'emb_init', 'emb_mask', 'emb_dynamic',
                      'trainable', 'combiner',
                      'group'])
...
class BaseModel():
  ...
  def get_target(self):
    ...
    for key, value in self.conf.target.items():
      ...
      target_label_table[key] = tf.lookup.StaticHashTable(initializer, default_value=0,
                                                          name=key + "_target_lookup")
    ...
    for key, value in self.conf.evaluate_target.items():
      ...
      initializer = tf.lookup.KeyValueTensorInitializer(
        ...
        value_dtype=tf.int64,
        name=key + "_target_lookup_1")
      eva_target_label_table[key] = tf.lookup.StaticHashTable(initializer, default_value=0,
                                                              name=key + "_target_lookup")
  ...
  def get_feature_meta(self):
    ...
    for line in f.readlines():
      ...
      if fea_len == 1 and fea_dtype in ('int64', 'float32') and fea_name not in fea_tag_dict:
        if fea_gpercentile.startswith('[') or (fea_gpercentile not in ['', '\"\"'] and not fea_gpercentile.startswith('{')):
        ...
        if fea_bpercentile.startswith('[') or (fea_bpercentile not in ['', '\"\"'] and not fea_bpercentile.startswith('{')):
  ...
  def make_feature(self, f=None, **kwargs):
    if not f:
      f = Feature(None, 0, tf.int64, 1, -3,
                  False, False, None, None,
                  None, 0, 1, 0.00001, None,
                  0.00001, 'truncated_normal', False, False,
                  True, 'mean',
                  None)
    copy = f._asdict()
    copy.update(kwargs)
  ...
  def input_from_features(self):
    ...
    for code, fname, dtype, length in self.feature_map[["code", "name", "dtype", "length"]].values:
      if code not in input_dict:
        input_dict[code] = tf.keras.Input(
          shape=(int(length),),
          name=code,
          dtype=dtype)
    return input_dict
  ...
  def dense_from_columns(self, features, emb_dict):  # 用于非id特征
    ...
    embedding_outs = tf.split(embedding_outs_concat, num_or_size_splits=split_dims_final, axis=1, name=f"split_not_id_fea")
    ...
    for i, embedding in enumerate(embedding_outs):
      if is_sequence_feature[i]:
        ...
        simple_fea_embeddings = tf.split(embedding, num_or_size_splits=[1] * split_dims_final[i], axis=1, name=f"split_simple_fea_embeddings_{i}")
      ...
      if denses:
        ...
        for i, fea in enumerate(feas):
          ...
          if fea.len > 1 and fea.combiner:
            fea_dense = Pooling(combiner=fea.combiner, name=f"{fea.combiner}_{fea.name}")(fea_dense, mask=masks[fea.code])
  ...
  def build_features(self):
    ...
    features = dict()
    for code, fname, dtype, length, def_valu in self.feature_map[["code", "name", "dtype", "length", "def_valu"]].values:
      if 'int' in dtype:
        try:
      ...
      features[fname] = self.make_feature(name=fname, code=code, dtype=dtype, len=int(length), default_value=def_valu,
                                          use_hash=use_hash, vocab=vocab, boundaries=boundaries,
                                          emb_name=emb_name, emb_size=emb_size, emb_dynamic=emb_dynamic,
