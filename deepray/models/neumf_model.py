...
from deepray.datasets.movielens import stat_utils
...
def neumf_model_fn(features, labels, mode, params):
  ...
  users = features[rconst.USER_COLUMN]
  items = features[rconst.ITEM_COLUMN]
...
class ConstructModel(tf.keras.layers.Layer):
  def __init__(self, params: Dict[Text, Any]):
    ...
    self.mlp_reg_layers = params["mlp_reg_layers"]
    ...
    self.emb_layer = tf.keras.layers.Embedding(num_items,
                                               self.mf_dim + self.model_layers[0] // 2,
                                               embeddings_initializer=embedding_initializer,
                                               embeddings_regularizer=tf.keras.regularizers.l2(mf_regularization),
                                               input_length=1,
                                               name="embedding_item")
    self.user_layer = tf.keras.layers.Embedding(num_users,
                                                self.mf_dim + self.model_layers[0] // 2,
                                                embeddings_initializer=embedding_initializer,
                                                embeddings_regularizer=tf.keras.regularizers.l2(mf_regularization),
                                                input_length=1,
    ...
    self.prediction_layer = tf.keras.layers.Dense(1,
                                                  ...
                                                  name=rconst.RATING_COLUMN)

    self.kernel = []
    num_layer = len(self.model_layers)  # Number of layers in the MLP
    for layer in xrange(1, num_layer):
      self.kernel.append(tf.keras.layers.Dense(self.model_layers[layer],
  ...
  def call(self, user_input: tf.Tensor, item_input: tf.Tensor, *args, **kwargs):
    ...
    embedding_user = self.user_layer(user_input)

    embedding_item = self.emb_layer(item_input)
    ...
    for layer in range(1, num_layer):
      mlp_vector = self.kernel[layer](mlp_vector)
