...
load(
    "//deepray/core/platform:build_config_root.bzl",
    "if_dynamic_kernels",
    "if_static",
...
load(
    "//deepray/core/platform:rules_cc.bzl",
    "cc_binary",
    "cc_library",
    "cc_shared_library",
    "cc_test",
...
def if_v2(a):
    return select({
        clean_dep("//deepray:api_version_2"): a,
        "//conditions:default": [],
    })

def if_not_v2(a):
    return select({
        clean_dep("//deepray:api_version_2"): [],
...
def if_xla_available(if_true, if_false = []):
    return select({
        clean_dep("//deepray:with_xla_support"): if_true,
...
def tf_portable_full_lite_protos(full, lite):
    return select({
        "//deepray:mobile_lite_protos": lite,
        "//deepray:mobile_full_protos": full,
...
def if_no_default_logger(a):
    return select({
        clean_dep("//deepray:no_default_logger"): a,
        "//conditions:default": [],
    })

def if_android_x86(a):
    return select({
        clean_dep("//deepray:android_x86"): a,
        clean_dep("//deepray:android_x86_64"): a,
        "//conditions:default": [],
    })

def if_android_arm(a):
    return select({
        clean_dep("//deepray:android_arm"): a,
        "//conditions:default": [],
    })

def if_android_arm64(a):
    return select({
        clean_dep("//deepray:android_arm64"): a,
        "//conditions:default": [],
    })

def if_android_mips(a):
    return select({
        clean_dep("//deepray:android_mips"): a,
        "//conditions:default": [],
    })

def if_not_android(a):
    return select({
        clean_dep("//deepray:android"): [],
        "//conditions:default": a,
    })

def if_not_android_mips_and_mips64(a):
    return select({
        clean_dep("//deepray:android_mips"): [],
        clean_dep("//deepray:android_mips64"): [],
        "//conditions:default": a,
    })

def if_android(a):
    return select({
        clean_dep("//deepray:android"): a,
        "//conditions:default": [],
    })
...
def if_android_or_ios(a):
    return select({
        clean_dep("//deepray:android"): a,
        clean_dep("//deepray:ios"): a,
        "//conditions:default": [],
    })

def if_emscripten(a):
    return select({
        clean_dep("//deepray:emscripten"): a,
        "//conditions:default": [],
    })

def if_chromiumos(a, otherwise = []):
    return select({
        clean_dep("//deepray:chromiumos"): a,
        "//conditions:default": otherwise,
    })

def if_macos(a, otherwise = []):
    return select({
        clean_dep("//deepray:macos"): a,
        "//conditions:default": otherwise,
    })
...
def if_ios_x86_64(a):
    return select({
        clean_dep("//deepray:ios_x86_64"): a,
        "//conditions:default": [],
    })

def if_mobile(a):
    return select({
        clean_dep("//deepray:mobile"): a,
        "//conditions:default": [],
    })

def if_not_mobile(a):
    return select({
        clean_dep("//deepray:mobile"): [],
        "//conditions:default": a,
    })
...
def if_windows(a, otherwise = []):
    return select({
        clean_dep("//deepray:windows"): a,
        "//conditions:default": otherwise,
    })

def if_windows_cuda(a, otherwise = []):
    return select({
        clean_dep("//deepray:is_cuda_enabled_and_windows"): a,
        "//conditions:default": otherwise,
    })

def if_not_fuchsia(a):
    return select({
        clean_dep("//deepray:fuchsia"): [],
        "//conditions:default": a,
    })

def if_linux_x86_64(a):
    return select({
        clean_dep("//deepray:linux_x86_64"): a,
        "//conditions:default": [],
    })

def if_override_eigen_strong_inline(a):
    return select({
        clean_dep("//deepray:override_eigen_strong_inline"): a,
        "//conditions:default": [],
    })
...
def if_zendnn(if_true, if_false = []):
    return select({
        clean_dep("//deepray:linux_x86_64"): if_true,
        "//conditions:default": if_false,
    })
...
def if_libtpu(if_true, if_false = []):
    ...
    return select({
        ...
        clean_dep("//deepray:with_tpu_support"): if_true,
...
def if_with_tpu_support(if_true, if_false = []):
    ...
    return select({
        "//deepray:with_tpu_support": if_true,
        "//conditions:default": if_false,
    })

def if_registration_v2(if_true, if_false = []):
    return select({
        "//deepray:registration_v2": if_true,
        "//conditions:default": if_false,
    })
...
def lrt_if_needed():
    lrt = ["-lrt"]
    return select({
        clean_dep("//deepray:linux_aarch64"): lrt,
        clean_dep("//deepray:linux_x86_64"): lrt,
        clean_dep("//deepray:linux_ppc64le"): lrt,
        "//conditions:default": [],
    })
...
def tf_copts(
    ...
    return (
        ...
        select({
            clean_dep("//deepray:framework_shared_object"): [],
            "//conditions:default": ["-DTENSORFLOW_MONOLITHIC_BUILD"],
        }) +
        select({
            clean_dep("//deepray:android"): android_copts,
            clean_dep("//deepray:emscripten"): [],
            clean_dep("//deepray:macos"): [],
            clean_dep("//deepray:windows"): get_win_copts(is_external),
            clean_dep("//deepray:ios"): [],
            clean_dep("//deepray:no_lgpl_deps"): ["-D__TENSORFLOW_NO_LGPL_DEPS__", "-pthread"],
...
def tf_openmp_copts():
    ...
    return select({
        ...
        # copybara:uncomment_end_and_comment_begin
        "@//third_party/mkl:build_with_mkl_lnx_openmp": ["-fopenmp"],
        "@//third_party/mkl:build_with_mkl_windows_openmp": ["/openmp:llvm"],
        # copybara:comment_end
        "//conditions:default": [],
    })
...
def tf_features_nolayering_check_if_ios():
    return select({
        clean_dep("//deepray:ios"): ["-layering_check"],
        "//conditions:default": [],
    })
...
def tf_gen_op_libs(
    ...
    for n in op_lib_names:
        cc_library(
            ...
            deps = deps + [clean_dep("//deepray/core:framework")],
...
def tf_binary_additional_data_deps():
    return if_static(
        extra_deps = [],
        macos = [
            clean_dep("//deepray:libtensorflow_framework.dylib"),
            clean_dep("//deepray:libtensorflow_framework.%s.dylib" % VERSION_MAJOR),
            clean_dep("//deepray:libtensorflow_framework.%s.dylib" % VERSION),
        ],
        otherwise = [
            clean_dep("//deepray:libtensorflow_framework.so"),
            clean_dep("//deepray:libtensorflow_framework.so.%s" % VERSION_MAJOR),
            clean_dep("//deepray:libtensorflow_framework.so.%s" % VERSION),
        ],
    )
...
def tf_binary_pybind_deps():
    return select({
        clean_dep("//deepray:macos"): [
            clean_dep(
                "//deepray/python:_pywrap_tensorflow_internal_macos",
            ),
        ],
        clean_dep("//deepray:windows"): [
            clean_dep(
                "//deepray/python:_pywrap_tensorflow_internal_windows",
            ),
        ],
        "//conditions:default": [
            clean_dep(
                "//deepray/python:_pywrap_tensorflow_internal_linux",
            ),
        ],
    })
...
def tf_shared_library_deps():
    return select({
        clean_dep("//deepray:macos_with_framework_shared_object"): [
            clean_dep("//deepray:libtensorflow.dylib"),
            clean_dep("//deepray:libtensorflow.%s.dylib" % VERSION_MAJOR),
            clean_dep("//deepray:libtensorflow.%s.dylib" % VERSION),
        ],
        clean_dep("//deepray:macos"): [],
        clean_dep("//deepray:windows"): [
            clean_dep("//deepray:tensorflow.dll"),
            clean_dep("//deepray:tensorflow_dll_import_lib"),
        ],
        clean_dep("//deepray:framework_shared_object"): [
            clean_dep("//deepray:libtensorflow.so"),
            clean_dep("//deepray:libtensorflow.so.%s" % VERSION_MAJOR),
            clean_dep("//deepray:libtensorflow.so.%s" % VERSION),
        ],
        "//conditions:default": [],
...
def tf_binary_dynamic_kernel_dsos():
    return if_dynamic_kernels(
        extra_deps = [
            ...
            # "//deepray/core/kernels:libtfkernel_all_kernels.so",
        ],
        otherwise = [],
    )
...
def tf_cc_shared_object(
    ...
    if name not in flat_names:
        native.filegroup(
            name = name,
            srcs = select({
                clean_dep("//deepray:windows"): [":%s.dll" % (name)],
                clean_dep("//deepray:macos"): [":lib%s%s.dylib" % (name, longsuffix)],
...
def tf_cc_shared_library_opensource(
        ...
        linkstatic = True,
        framework_so = [clean_dep("//deepray:libtensorflow_framework_import_lib")],
        soversion = None,
    ...
    if name not in [item for sublist in names for item in sublist]:
        native.filegroup(
            name = name,
            srcs = select({
                clean_dep("//deepray:windows"): [":%s" % get_versioned_shared_library_name_windows(name, soversion)],
                clean_dep("//deepray:macos"): [":%s" % get_versioned_shared_library_name_macos(name, soversion)],
...
def tf_cc_binary(
    ...
    for name_os in names:
        cc_binary(
            ...
            deps = deps + tf_binary_dynamic_kernel_deps(kernels) + mkl_dep + if_static(
                extra_deps = [],
                otherwise = [
                    clean_dep("//deepray:libtensorflow_framework_import_lib"),
                ],
            ),
            tags = tags,
    ...
    if name not in names:
        native.filegroup(
            name = name,
            srcs = select({
                "//deepray:windows": [":%s.dll" % name],
                "//deepray:macos": [":lib%s.dylib" % name],
...
def tf_gen_op_wrapper_cc(
        ...
        out_ops_file,
        pkg = "",
        op_gen = clean_dep("//deepray/cc:cc_op_gen_main"),
        deps = None,
...
def tf_gen_op_wrappers_cc(
        ...
        pkg = "",
        deps = [
            clean_dep("//deepray/cc:ops"),
            clean_dep("//deepray/cc:scope"),
            clean_dep("//deepray/cc:const_op"),
        ],
        deps_internal = [],
        op_gen = clean_dep("//deepray/cc:cc_op_gen_main"),
        include_internal_ops = 0,
    ...
    cc_library(
        ...
        deps = deps + if_not_android([
            clean_dep("//deepray/core:core_cpu"),
            clean_dep("//deepray/core:framework"),
            clean_dep("//deepray/core:lib"),
            clean_dep("//deepray/core:ops"),
            clean_dep("//deepray/core:protos_all_cc"),
        ]) + if_android([
            clean_dep("//deepray/core:portable_tensorflow_lib"),
        ]),
        copts = tf_copts(),
        alwayslink = 1,
        visibility = visibility,
    ...
    cc_library(
        ...
        copts = tf_copts(),
        alwayslink = 1,
        visibility = [clean_dep("//deepray:internal")],
        compatible_with = compatible_with,
    )
...
generate_op_reg_offsets = rule(
    attrs = {
        ...
        "_offset_counter": attr.label(
            ...
            executable = True,
            allow_files = True,
            default = "//deepray/python/framework:offset_counter",
        ),
    },
...
def tf_gen_op_wrapper_py(
    ...
    if not deps:
        deps = [str(Label("//deepray/core:" + name + "_op_lib"))]
    tf_cc_binary(
        name = tool_name,
        ...
        deps = ([
            clean_dep("//deepray/core:framework"),
            clean_dep("//deepray/python/framework:python_op_gen_main"),
        ] + deps),
        testonly = testonly,
    )
    ...
    generate_op_reg_offsets(
        ...
        deps = select({
            clean_dep("//deepray:api_indexable"): deps,
            "//conditions:default": [],
        }),
    ...
    native.py_library(
        ...
        visibility = visibility,
        deps = [
            clean_dep("//deepray/python:framework_for_generated_wrappers_v2"),
        ],
...
def tf_cc_shared_test(
    ...
    cc_test(
        ...
        dynamic_deps = if_static(
            extra_deps = [],
            macos = ["//deepray:libtensorflow_framework.%s.dylib" % VERSION],
            otherwise = ["//deepray:libtensorflow_framework.so.%s" % VERSION],
        ),
...
def tf_gpu_cc_test(
    ...
    tf_cc_test(
        ...
        deps = deps + if_cuda_or_rocm([
            clean_dep("//deepray/core:gpu_runtime"),
        ]),
        **kwargs
    )
...
def tf_gpu_kernel_library(
    ...
    cuda_library(
        ...
        copts = copts,
        deps = deps + if_cuda([
            clean_dep("//deepray/tsl/cuda:cudart_stub"),
        ]) + if_cuda_or_rocm([
            clean_dep("//deepray/core:gpu_lib"),
        ]),
        alwayslink = 1,
        **kwargs
    )
...
def tf_kernel_library(
    ...
    cuda_deps = [clean_dep("//deepray/core:gpu_lib")]
    if gpu_srcs:
    ...
    kwargs["tags"] = kwargs.get("tags", []) + [
        "req_dep=%s" % clean_dep("//deepray/core:gpu_lib"),
...
def tf_custom_op_library_additional_deps():
    return [
        ...
        # clean_dep("//deepray/core:framework_headers_lib"),
    ]
...
def tf_custom_op_library_additional_deps_impl():
    return [
        ...
        clean_dep("//deepray/core:framework"),
        clean_dep("//deepray/core:reader_base"),
    ]
...
def tf_custom_op_library(
    ...
    deps = deps + if_cuda_or_rocm([
        clean_dep("//deepray/core:stream_executor_headers_lib"),
    ]) + if_cuda([
        "@local_config_cuda//cuda:cuda_headers",
        "@local_config_cuda//cuda:cudart_static",
    ]) + if_windows([
        clean_dep("//deepray/python:pywrap_tensorflow_import_lib"),
...
def pywrap_tensorflow_macro_opensource(
    ...
    if not version_script:
        version_script = select({
            "//deepray:macos": clean_dep("//deepray:tf_exported_symbols.lds"),
            "//conditions:default": clean_dep("//deepray:tf_version_script.lds"),
        })
    ...
    srcs += select({
        clean_dep("//deepray:macos"): [clean_dep("//deepray:libtensorflow_cc.%s.dylib" % VERSION_MAJOR)],
        clean_dep("//deepray:windows"): [],
        "//conditions:default": [clean_dep("//deepray:libtensorflow_cc.so.%s" % VERSION_MAJOR)],
    })
    ...
    _create_symlink("bfloat16.so", "//deepray/tsl/python/lib/core:bfloat16.so")
    ...
    native.py_library(
        ...
        data = select({
            clean_dep("//deepray:windows"): [
                ":" + cc_library_pyd_name,
                ":bfloat16.so",
                "//deepray/tsl/python/lib/core:bfloat16.so",
            ],
            "//conditions:default": [
...
def py_test(deps = [], data = [], kernels = [], exec_properties = None, test_rule = native.py_test, **kwargs):
    ...
    test_rule(
        ...
        deps = select({
            ...
            clean_dep("//deepray:no_tensorflow_py_deps"): [],
        }),
        data = data + select({
            "//conditions:default": kernels,
            clean_dep("//deepray:no_tensorflow_py_deps"): ["//deepray/tools/pip_package:win_pip_package_marker"],
        }),
...
def tf_py_test(
    ...
    if xla_enable_strict_auto_jit:
        xla_enabled = True
        xla_test_true_list.append("//deepray/python/framework:is_xla_test_true")
    if xla_enabled:
    ...
    for to_add in [
        "//deepray/python:extra_py_tests_deps",
    ]:
...
def tf_generate_proto_text_sources(name, srcs_relative_dir, srcs, protodeps = [], deps = [], visibility = None, compatible_with = None):
    ...
    native.genrule(
        name = name + "_srcs",
        srcs = srcs + protodeps + [clean_dep("//deepray/tools/proto_text:placeholder.txt")],
        ...
        tools = [
            clean_dep("//deepray/tools/proto_text:gen_proto_text_functions"),
        ],
        compatible_with = compatible_with,
    )
...
def tf_version_info_genrule(name, out, compatible_with = None):
    ...
    _local_genrule(
        ...
        out = out,
        compatible_with = compatible_with,
        exec_tool = "//deepray/tools/git:gen_git_source",
        srcs = [
...
def tf_py_build_info_genrule(name, out):
    _local_genrule(
        name = name,
        out = out,
        exec_tool = "//deepray/tools/build_info:gen_build_info",
        arguments =
            "--raw_generate \"$@\" " +
            " --key_value" +
...
def tf_python_pybind_extension_opensource(
    ...
    extended_deps += [] if dynamic_deps else if_windows([], ["//deepray:libtensorflow_framework_import_lib"]) + tf_binary_pybind_deps()
...
def if_mlir(if_true, if_false = []):
    return select({
        str(Label("//deepray:with_mlir_support")): if_true,
        "//conditions:default": if_false,
    })

def tf_enable_mlir_bridge():
    return select({
        str(Label("//deepray:enable_mlir_bridge")): [
            "//deepray/python:is_mlir_bridge_test_true",
        ],
        str(Label("//deepray:disable_mlir_bridge")): [
            "//deepray/python:is_mlir_bridge_test_false",
        ],
        "//conditions:default": [],
    })
...
def replace_with_portable_tf_lib_when_required(non_portable_tf_deps, use_lib_with_runtime = False):
    portable_tf_lib = "//deepray/core:portable_tensorflow_lib_lite"

    return select({
        "//deepray:android": [portable_tf_lib],
        "//deepray:ios": [portable_tf_lib],
