...
from deepray.activations import sparsemax
...
def test_sparsemax_against_numpy_axis(dtype):
  ...
  test_utils.assert_allclose_according_to_type(np_sparsemax, tf_sparsemax_out, half_atol=5e-3)
...
def test_sparsemax_of_nan(dtype):
  ...
  z_nan = np.asarray([[0, np.nan, 0], [0, np.nan, np.nan], [np.nan, np.nan, np.nan]]).astype(dtype)
...
def test_sparsemax_of_inf(dtype):
  ...
  z_neg = np.asarray([[0, -np.inf, 0], [0, -np.inf, -np.inf], [-np.inf, -np.inf, -np.inf]]).astype(dtype)
  z_pos = np.asarray([[0, np.inf, 0], [0, np.inf, np.inf], [np.inf, np.inf, np.inf]]).astype(dtype)
  z_mix = np.asarray([[0, np.inf, 0], [0, np.inf, -np.inf], [-np.inf, np.inf, -np.inf]]).astype(dtype)

  tf_sparsemax_neg = sparsemax(z_neg)
  np.testing.assert_equal(np.array([[0.5, 0, 0.5], [1, 0, 0], [np.nan, np.nan, np.nan]]), tf_sparsemax_neg)
...
def test_constant_add(dtype):
  ...
  test_utils.assert_allclose_according_to_type(tf_sparsemax_zpc, tf_sparsemax_z, half_atol=5e-3)
...
def test_two_dimentional(dtype):
  ...
  test_utils.assert_allclose_according_to_type(1 - p0_expected, tf_sparsemax_out[:, 1])
...
def test_permutation(dtype):
  ...
  for i in range(test_obs):
    ...
    test_utils.assert_allclose_according_to_type(p_expected, tf_sparsemax_out, half_atol=5e-3)
...
def test_gradient_against_estimate(dtype):
  ...
  (jacob_sym,), (jacob_num,) = tf.test.compute_gradient(lambda logits: sparsemax(logits), [z], delta=1e-6)
