...
from deepray.text import skip_gram_ops
...
def test_skip_gram_sample_skips_0():
  ...
  tokens, labels = text.skip_gram_sample(input_tensor, min_skips=0, max_skips=0, emit_self_as_target=False)
  assert 0 == len(tokens)
  assert 0 == len(labels)
...
def test_skip_gram_sample_non_string_input():
  ...
  expected_tokens, expected_labels = _split_tokens_labels([(1, 2), (2, 1), (2, 3), (3, 2)])
  np.testing.assert_equal(np.asanyarray(expected_tokens), tokens.numpy())
  np.testing.assert_equal(np.asanyarray(expected_labels), labels.numpy())
...
def test_skip_gram_sample_errors():
  ...
  for min_skips, max_skips in invalid_skips:
    with pytest.raises(tf.errors.InvalidArgumentError):
      text.skip_gram_sample(input_tensor, min_skips=min_skips, max_skips=max_skips)
  ...
  with pytest.raises(ValueError):
    text.skip_gram_sample(dummy_input, vocab_freq_table=None, vocab_subsampling=1e-5)
  with pytest.raises(ValueError):
    text.skip_gram_sample(dummy_input, vocab_freq_table=None, corpus_size=100)
  with pytest.raises(ValueError):
    text.skip_gram_sample(dummy_input, vocab_freq_table=None, vocab_subsampling=1e-5, corpus_size=100)
...
def _text_vocab_subsample_vocab_helper(vocab_freq_file, vocab_min_count, vocab_freq_dtype, corpus_size=None):
  # The outputs are non-deterministic, so set random seed to help ensure
  # that the outputs remain constant for testing.
  tf.random.set_seed(42)
...
def _make_text_vocab_freq_file(tmp_dir):
  ...
  with open(filepath, "w") as f:
    writer = csv.writer(f)
    writer.writerows([["and", 40], ["life", 8], ["the", 30], ["to", 20], ["universe", 2]])
  return filepath
