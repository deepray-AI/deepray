...
def test_crf_sequence_score(dtype):
  ...
  for sequence_lengths, inputs, tag_indices in zip(sequence_lengths_list, inputs_list, tag_indices_list):
    ...
    expected_sequence_score = calculate_sequence_score(inputs, transition_params, tag_indices, sequence_lengths)
    test_utils.assert_allclose_according_to_type(sequence_score, expected_sequence_score)
...
def test_viterbi_decode(dtype):
  ...
  actual_max_sequence, actual_max_score = text.viterbi_decode(inputs[:sequence_lengths], transition_params)
...
def test_crf_constrained_decode(dtype):
  ...
  for sequence_lengths, inputs, tag_bitmap in zip(sequence_lengths_list, inputs_list, tag_bitmap_list):
    ...
    test_utils.assert_allclose_according_to_type(actual_max_score, expected_max_score, 1e-6, 1e-6)
    assert list(actual_max_sequence[:sequence_lengths]) == list(expected_max_sequence[:sequence_lengths])
...
def test_different_dtype():
  ...
  loss, _ = text.crf_log_likelihood(inputs=inputs, tag_indices=tags, sequence_lengths=seq_lens)
...
def test_crf_decode_forward_mask(potentials, sequence_length):
  ...
  backpointers, _ = text.crf_decode_forward(inputs, initial_state, transition_params, sequence_length_less_one)
