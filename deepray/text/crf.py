...
def crf_log_norm(inputs: TensorLike, sequence_lengths: TensorLike, transition_params: TensorLike) -> tf.Tensor:
  ...
  def _single_seq_fn():
    ...
    log_norm = tf.where(tf.less_equal(sequence_lengths, 0), tf.zeros_like(log_norm), log_norm)
    return log_norm
...
def crf_unary_score(tag_indices: TensorLike, sequence_lengths: TensorLike, inputs: TensorLike) -> tf.Tensor:
  ...
  unary_scores = tf.reshape(tf.gather(flattened_inputs, flattened_tag_indices), [batch_size, max_seq_len])

  masks = tf.sequence_mask(sequence_lengths, maxlen=tf.shape(tag_indices)[1], dtype=unary_scores.dtype)
...
def crf_binary_score(tag_indices: TensorLike, sequence_lengths: TensorLike, transition_params: TensorLike) -> tf.Tensor:
  ...
  masks = tf.sequence_mask(sequence_lengths, maxlen=tf.shape(tag_indices)[1], dtype=binary_scores.dtype)
...
class CrfDecodeForwardRnnCell(tf.keras.layers.AbstractRNNCell):
  ...
  def get_config(self) -> dict:
    config = {"transition_params": tf.squeeze(self._transition_params, 0).numpy().tolist()}
  ...
  def from_config(cls, config: dict) -> "CrfDecodeForwardRnnCell":
    config["transition_params"] = np.array(config["transition_params"], dtype=np.float32)
    return cls(**config)
